...
func deleteProviderIgnoreNotFound(ctx context.Context, t *testing.T, client *v1.Client, workspace, name string) {
	t.Helper()
	_, err := client.Providers().Delete(ctx, workspace, name)
...
func attachProviderRetry(ctx context.Context, t *testing.T, client *v1.Client, workspace, sandboxName, providerName string) *v1.AttachProviderResult {
	...
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		...
		if attempt%10 == 0 {
			t.Logf("attach provider still conflicted after %d attempts: %v", attempt, err)
		}
		if attempt == maxAttempts {
			t.Fatalf("attach provider still conflicted after %d attempts: %v", attempt, err)
		}
		select {
		case <-ctx.Done():
...
func detachProviderRetry(ctx context.Context, t *testing.T, client *v1.Client, workspace, sandboxName, providerName string) *v1.DetachProviderResult {
	...
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		...
		if attempt%10 == 0 {
			t.Logf("detach provider still conflicted after %d attempts: %v", attempt, err)
		}
		if attempt == maxAttempts {
			t.Fatalf("detach provider still conflicted after %d attempts: %v", attempt, err)
		}
		select {
		case <-ctx.Done():
...
func TestProviderCredentialsAvailableAsEnvVar(t *testing.T) {
	...
	require.NoError(t, err)
	requireSandboxResponse(t, sandbox, "default", sandboxName)
	t.Cleanup(func() {
		cleanupCtx, cleanupCancel := context.WithTimeout(context.Background(), 30*time.Second)
		defer cleanupCancel()
		detachProviderBestEffort(cleanupCtx, client, "default", sandboxName, providerName)
		_, err := client.Sandboxes().Delete(cleanupCtx, "default", sandboxName)
		require.NoError(t, err)
	})
...
func TestAttachDetachProviderUpdatesCredentials(t *testing.T) {
	...
	requireSandboxResponse(t, attachResult.Sandbox, "default", sandboxName)
	require.Equal(t, sandbox.ID, attachResult.Sandbox.ID)
	require.Contains(t, attachResult.Sandbox.Spec.Providers, providerName)
	...
	requireSandboxResponse(t, detachResult.Sandbox, "default", sandboxName)
	require.Equal(t, sandbox.ID, detachResult.Sandbox.ID)
	require.NotContains(t, detachResult.Sandbox.Spec.Providers, providerName)
