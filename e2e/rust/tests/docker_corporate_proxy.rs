...
//! Cross-layer E2E coverage for corporate forward-proxy egress from Docker
//! sandboxes (issue #3545).
//!
//! The Docker counterpart of `podman_corporate_proxy.rs`. It drives the whole
//! chain end to end:
//!
//! gateway TOML → Docker driver config → companion supervisor archive →
...
//! 3. Credentials arrive through the supervisor-volume file — the proxy answers
...
//! 6. A TLS-intercepting proxy CA is trusted by both inspected supervisor
//!    upstream TLS and a raw workload TLS connection.
//! 7. An incoherent setting is fatal at gateway startup rather than
...
const ALLOWED_MARKER: &str = "docker-corp-proxy-e2e-allowed-upstream";
const DENIED_MARKER: &str = "docker-corp-proxy-e2e-denied-upstream";
const BYPASS_MARKER: &str = "docker-corp-proxy-e2e-bypass-upstream";
const READY_MARKER: &str = "docker-corp-proxy-e2e-workload-done";

/// Ports the fixtures bind and the workload addresses them by.
struct FixturePorts {
    proxy: u16,
    allowed: u16,
    denied: u16,
...
fn intercepting_proxy_script(port: u16) -> String {
    let pki = tls_proxy_pki_preamble();
    format!(
        r"
import select, socket, ssl, subprocess, tempfile, threading, os
{HOST_REWRITE}
{pki}
...
server.bind(('0.0.0.0', {port}))
server.listen(64)
log('intercepting-proxy-listening')
while True:
...
/// Workload variant that uses the system/default trust store.
///
/// The inspected endpoint proves the supervisor accepts the corporate
/// re-signing CA for upstream TLS. The `tls: skip` endpoint proves the same CA
/// reached the workload trust bundle because Python verifies it directly.
fn trusted_workload_script(ports: &FixturePorts) -> String {
    let (inspected, raw, denied) = (ports.allowed, ports.bypass, ports.denied);
    format!(
        r"
import json, ssl, time, urllib.request
...
print('INSPECTED_RESULT ' + json.dumps(
    fetch('https://{HOST_ALIAS}:{inspected}/', 6)), flush=True)
print('RAW_RESULT ' + json.dumps(
    fetch('https://{HOST_ALIAS}:{raw}/', 6)), flush=True)
print('DENIED_RESULT ' + json.dumps(
    fetch('https://{HOST_ALIAS}:{denied}/', 1)), flush=True)
print('{READY_MARKER}', flush=True)
"
...
/// Allow one auto-inspected TLS endpoint and one raw `tls: skip` endpoint.
fn interception_policy_yaml(ports: &FixturePorts) -> String {
    let (inspected, raw) = (ports.allowed, ports.bypass);
    format!(
        r#"version: 1
...
network_policies:
  docker_corporate_proxy_interception_e2e:
    name: docker_corporate_proxy_interception_e2e
    endpoints:
      - host: {HOST_ALIAS}
        port: {inspected}
        enforcement: enforce
...
impl GatewayProxyConfig {
    ...
    /// Point the Docker driver at a corporate proxy and wait for the gateway to
    /// come back healthy.
    async fn apply(
        proxy_url: &str,
...
fn assert_proxied_egress(output: &str, proxy_logs: &str, ports: &FixturePorts) {
    ...
    // The CONNECT target is a validated resolved address, not the hostname,
    // and it arrived authenticated -- which is only possible if the credential
    // staged into the overlay reached the supervisor.
    assert!(
        proxy_logs.lines().any(|line| {
            line.starts_with("CONNECT ")
                && line.contains(&format!(":{} auth=ok", ports.allowed))
                && !line.contains(HOST_ALIAS)
        }),
...
#[tokio::test]
#[serial(docker_corporate_proxy)]
async fn docker_corporate_proxy_routes_approved_tls_egress() {
    ...
    // ── Point the Docker driver at the corporate proxy ────────────────────
...
#[tokio::test]
#[serial(docker_corporate_proxy)]
async fn docker_corporate_proxy_trusts_ca_bundle_for_https_proxy() {
    ...
    assert!(
        proxy_logs.lines().any(|line| {
            line.starts_with("CONNECT ")
                && line.contains(&format!(":{} ok", ports.allowed))
                && !line.contains(HOST_ALIAS)
        }),
...
#[tokio::test]
#[serial(docker_corporate_proxy)]
async fn docker_corporate_proxy_trusts_intercepted_destination_tls() {
    if !should_run("TLS-intercepting corporate proxy test") {
        return;
    }

    let ports = FixturePorts::pick();
    let proxy = HostPythonFixture::start(&intercepting_proxy_script(ports.proxy), ports.proxy)
        .await
        .expect("start TLS-intercepting corporate proxy");
    let _inspected = HostPythonFixture::start(
        &tls_upstream_script(ALLOWED_MARKER, ports.allowed),
        ports.allowed,
    )
    .await
    .expect("start inspected TLS upstream");
    let _denied = HostPythonFixture::start(
        &tls_upstream_script(DENIED_MARKER, ports.denied),
        ports.denied,
    )
    .await
    .expect("start denied TLS upstream");
    let _raw = HostPythonFixture::start(
        &tls_upstream_script(BYPASS_MARKER, ports.bypass),
        ports.bypass,
    )
    .await
    .expect("start raw workload TLS upstream");

    let ca_pem = ca_cert_from_logs(&proxy.logs().expect("read intercepting proxy logs"))
        .expect("recover intercepting proxy CA");
    let (_ca_file, ca_path) = temp_file_with(&ca_pem, "intercepting proxy CA bundle");
    let mut gateway_config = GatewayProxyConfig::apply(
        &format!("http://{HOST_ALIAS}:{}", ports.proxy),
        None,
        Some(&ca_path),
    ...
    .await
    .expect("apply TLS-intercepting proxy configuration");

    let (_policy, policy_path) = temp_file_with(
        &interception_policy_yaml(&ports),
    ...
    assert!(
        ...
        "raw workload TLS should trust the corporate CA through the combined bundle:\n{output}"
    );
    assert!(
        proxy_logs.contains(&format!(":{} mitm=ok", ports.allowed)),
        "intercepting proxy should see inspected upstream TLS:\n{proxy_logs}"
    );
    assert!(
        proxy_logs.contains(&format!(":{} mitm=ok", ports.bypass)),
        "intercepting proxy should see raw workload TLS:\n{proxy_logs}"
    );
    assert!(
        !proxy_logs.contains(&format!(":{}", ports.denied)) && !output.contains(DENIED_MARKER),
        "policy-denied destination must not reach the intercepting proxy:\n{proxy_logs}\n{output}"
    );
...
#[tokio::test]
#[serial(docker_corporate_proxy)]
async fn docker_corporate_proxy_rejects_incoherent_configuration() {
