...
pub async fn exercise_refresh_handles(github_key: Option<&str>) -> Result<(), String> {
    ...
    let tools = {
        ...
        ImageGuard::build("github-app-tools", &dockerfile, context.path())?
    };
...
if [ "$api_ok:$git_ok" = 1:1 ] && [ "$(cat /sandbox/repo/README.md)" = 'GitHub App installation token E2E' ]; then
  ...
  echo mixed-result
        ...
        format!(
            r#"if /usr/bin/python3.12 -c 'import os, base64, urllib.request; token = os.environ["REFRESH_E2E_ACCESS_TOKEN"]; auths = ["Bearer " + token, "Basic " + base64.b64encode(("x-access-token:" + token).encode()).decode()]; [urllib.request.urlopen(urllib.request.Request("https://{resource_host}:{resource_port}/probe", headers=dict(Authorization=auth)), timeout=5).read() for auth in auths]'; then echo ok; else echo failed; fi"#
        )
    };
