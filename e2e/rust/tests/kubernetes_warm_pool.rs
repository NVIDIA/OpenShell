...
use openshell_e2e::harness::cli::{
    run_cli, sandbox_names, wait_for_sandbox_exec_contains, wait_for_sandbox_phase,
};
...
async fn run_warm_pool_e2e(ctx: &mut TestContext) -> Result<(), String> {
    ...
    let default_template = create_and_assert_template(
        ctx,
        &template_name("default", &template_suffix),
        TemplateCase::Default,
    )
    .await?;
    let env_template = create_and_assert_template(
        ctx,
        &template_name("env", &template_suffix),
        TemplateCase::Env,
    )
    .await?;
    let cpu_template = create_and_assert_template(
        ctx,
        &template_name("cpu", &template_suffix),
        TemplateCase::Cpu,
    )
    .await?;

    create_and_assert_claimed_default_command(
        ctx,
        &format!("wps-{sandbox_suffix}"),
        &default_template,
    )
    .await?;
...
async fn create_and_assert_claimed_default_command(
    ctx: &mut TestContext,
    sandbox_name: &str,
    ...
    if code != 0 {
        return Err(format!(
            "detached scratch sandbox create for {sandbox_name} failed with exit {code}; output:\n{output}"
        ));
    }
    ...
    if claim["spec"]["warmPoolRef"]["name"].as_str() != Some(&template.warm_pool_name) {
        return Err(format!(
            "scratch sandbox {sandbox_name} did not use warm pool {}: {claim}",
            template.warm_pool_name
        ));
    }
    assert_claim_delete_shutdown_policy(sandbox_name, &claim)?;
    Ok(())
}
...
async fn create_and_assert_claimed(
    ...
    wait_for_sandbox_phase(sandbox_name, "Completed", Duration::from_secs(120)).await?;

    Ok(())
}

fn assert_claim_delete_shutdown_policy(sandbox_name: &str, claim: &Value) -> Result<(), String> {
    if claim["spec"]["lifecycle"]["shutdownPolicy"].as_str() != Some("Delete") {
        return Err(format!(
            "SandboxClaim for {sandbox_name} does not use Delete shutdown policy: {claim}"
        ));
    }
    Ok(())
}
...
fn assert_template_invariants(template: &Value, case: TemplateCase) -> Result<(), String> {
    if template["spec"]["podTemplate"]["metadata"]["labels"][LABEL_MANAGED_BY].as_str()
        != Some("openshell")
    {
        return Err(format!(
            "generated SandboxTemplate must label warm pods with {LABEL_MANAGED_BY}=openshell: {template}"
        ));
    }
...
fn template_name(case: &str, suffix: &str) -> String {
    let name = format!("wp-{case}-{suffix}");
    assert!(
        name.len() <= MAX_ROUTABLE_NAME_LEN,
        "generated template name {name:?} exceeds {MAX_ROUTABLE_NAME_LEN} characters"
    );
    name
}

#[test]
fn generated_template_names_fit_routable_limit() {
    for case in ["default", "env", "cpu"] {
        assert!(template_name(case, "12345678").len() <= MAX_ROUTABLE_NAME_LEN);
    }
}
