...
test_release_root_override_changes_repo() {
  ...
  SHELL="/bin/sh" run_install_with_env \
...
test_release_root_override_rejects_malformed_repo() {
  ...
  SHELL="/bin/sh" run_install_expect_failure \
...
test_tagged_release_fails_without_checksum_manifest() {
  ...
  if SHELL="/bin/sh" run_install_with_checksum_state_expect_failure manifest-missing; then
...
test_tagged_release_verifies_archive_checksum_against_manifest() {
  ...
  if SHELL="/bin/sh" run_install_with_checksum_state manifest-present; then
...
test_tagged_release_fails_when_checksum_mismatches_manifest() {
  ...
  if SHELL="/bin/sh" run_install_with_checksum_state_expect_failure checksum-mismatch; then
...
test_accepts_selection_flags() {
  ...
  SHELL="/bin/sh" run_install_with_args \
    --tool opencode \
...
test_no_env_scripts_created;      echo ""
