...
| Gateway logs `Agent Sandbox extension APIs are unavailable` and Kubernetes creates cold sandboxes | Agent Sandbox was installed with `manifest.yaml` only while OpenShell warm pooling defaults are enabled | Apply Agent Sandbox `extensions.yaml`, or set `server.warmPooling.enabled=false`; then restart the gateway so the driver revalidates `SandboxClaim`, `SandboxTemplate`, and `SandboxWarmPool` APIs |
