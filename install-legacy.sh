...
#
# Install the legacy OpenShell CLI tarball.
#
# Usage:
#   curl -LsSf https://raw.githubusercontent.com/NVIDIA/OpenShell/main/install-legacy.sh | sh
#
# Or run directly:
#   ./install-legacy.sh
#
# Environment variables:
#   OPENSHELL_VERSION     - Release tag to install (default/latest: v0.0.36)
...
usage() {
  cat <<EOF
install-legacy.sh - Install the legacy OpenShell CLI tarball

USAGE:
    curl -LsSf https://raw.githubusercontent.com/NVIDIA/OpenShell/main/install-legacy.sh | sh
    ./install-legacy.sh [OPTIONS]

OPTIONS:
    --help    Print this help message

ENVIRONMENT VARIABLES:
    OPENSHELL_VERSION       Release tag to install (default/latest: ${LEGACY_LATEST_VERSION})
    OPENSHELL_INSTALL_DIR   Directory to install into (default: ~/.local/bin)

EXAMPLES:
    # Install the latest legacy tarball release (${LEGACY_LATEST_VERSION})
    curl -LsSf https://raw.githubusercontent.com/NVIDIA/OpenShell/main/install-legacy.sh | sh

    # Install a specific version
    curl -LsSf https://raw.githubusercontent.com/NVIDIA/OpenShell/main/install-legacy.sh | OPENSHELL_VERSION=v0.0.9 sh

    # Install to /usr/local/bin
    curl -LsSf https://raw.githubusercontent.com/NVIDIA/OpenShell/main/install-legacy.sh | OPENSHELL_INSTALL_DIR=/usr/local/bin sh
EOF
}
...
resolve_version() {
  _requested="${OPENSHELL_VERSION:-latest}"

  if [ "$_requested" = "latest" ]; then
    info "using legacy latest version ${LEGACY_LATEST_VERSION}"
    echo "$LEGACY_LATEST_VERSION"
    return 0
  fi
