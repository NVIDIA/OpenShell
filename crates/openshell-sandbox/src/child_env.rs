...
pub fn standard_sbin_path_repair_enabled(policy: &SandboxPolicy, cdi_active: bool) -> bool {
    cdi_active && policy_has_standard_sbin_path(policy)
}
...
fn policy_has_standard_sbin_path(policy: &SandboxPolicy) -> bool {
    policy
        ...
        .any(|path| {
            STANDARD_SBIN_PATHS
                .iter()
                .any(|dir| Path::new(dir).starts_with(path) || path.starts_with(dir))
        })
}
...
mod tests {
    ...
    #[test]
    fn standard_sbin_repair_requires_cdi_context_and_policy_path() {
        let policy = policy_with_read_only(["/usr/sbin/nvidia-smi"]);

        assert!(standard_sbin_path_repair_enabled(&policy, true));
        assert!(!standard_sbin_path_repair_enabled(&policy, false));
    }
    ...
    #[test]
    fn standard_sbin_repair_accepts_read_only_ancestor() {
        let policy = policy_with_read_only(["/usr"]);
    ...
    #[test]
    fn bashrc_startup_snippet_is_idempotent() {
