...
fn inject_static_credential(
    ...
    let Some(value) = cred.env_vars.iter().find_map(|env_var| {
        let placeholder = format!("{}{env_var}", crate::secrets::PLACEHOLDER_PREFIX_PUBLIC);
        ctx.secret_resolver
            .as_ref()
    ...
    match cred.auth_style.trim().to_ascii_lowercase().as_str() {
        "" | "bearer" | "header" => {
            let modified_raw_header = inject_token_grant_header(&req.raw_header, cred, value)?;
            let provider_key = ocsf_message_field(provider_key);
            ocsf_emit!(
                HttpActivityBuilder::new(crate::ocsf_ctx())
                    .activity(ActivityId::Other)
...
mod tests {
    ...
    fn static_credential_ctx(
        ...
        let (_, resolver) = crate::secrets::SecretResolver::from_provider_env(
