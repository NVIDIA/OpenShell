...
mod tests {
    ...
    /// Drift guard between the gateway's minted claims and the public SDK
    /// verifier, which deliberately keeps its own copy of the wire contract.
    #[test]
    fn sdk_verifier_accepts_gateway_minted_extension_tokens() {
        use openshell_sdk::extension::{
            ExtensionCallerKind as SdkCallerKind, GatewayJwtAuthenticator,
        };
        ...
        .expect("issuer");
        let gateway_auth = SandboxJwtAuthenticator::from_pem(
            mat.public_key_pem.as_bytes(),
            mat.kid.clone(),
            "gateway-a",
        )
        ...
        let jwks = serde_json::to_vec(gateway_auth.jwks()).expect("JWKS JSON");

        for verifier in [
            GatewayJwtAuthenticator::builder(gateway_auth.issuer(), audience.as_str())
                .ed25519_pem(mat.kid.clone(), mat.public_key_pem.as_bytes())
                .build()
                .expect("PEM verifier"),
            GatewayJwtAuthenticator::builder(gateway_auth.issuer(), audience.as_str())
                .jwks(&jwks)
                .build()
                .expect("published JWKS verifier"),
        ] {
            let gateway = issuer
                .mint_extension_token(
                    &audience,
                ...
                .expect("gateway token");
            let caller = verifier
                .authenticate(&gateway.token)
                .expect("SDK accepts gateway token");
            assert_eq!(caller.kind, SdkCallerKind::Gateway);
            assert_eq!(caller.subject, "openshell-gateway:gateway-a");
            assert_eq!(caller.sandbox_id, None);
            ...
            let caller = verifier
                ...
                .expect("SDK accepts supervisor token");
            assert_eq!(caller.kind, SdkCallerKind::Supervisor);
            assert_eq!(caller.subject, "spiffe://openshell/sandbox/sandbox-a");
            assert_eq!(caller.sandbox_id.as_deref(), Some("sandbox-a"));

            let bootstrap = issuer.mint("sandbox-a").expect("bootstrap token");
            assert!(
                verifier.authenticate(&bootstrap.token).is_err(),
                "sandbox bootstrap tokens must never authenticate at an extension"
            );
        }
    }
}
