...
pub const DEFAULT_PEER_TOKEN_CACHE_TTL: Duration = Duration::from_mins(1);
...
mod tests {
    ...
    async fn caching_resolver_reuses_a_verified_token() {
        ...
        let resolver = CachingGatewayPeerResolver::new(inner.clone(), Duration::from_mins(1));
    ...
    fn entry_lifetime_never_outlives_the_token() {
        let inner = Arc::new(FakeGatewayPeerResolver::returning(Ok(Some(identity()))));
        let resolver = CachingGatewayPeerResolver::new(inner, Duration::from_mins(1));
        ...
        assert_eq!(capped, Duration::from_mins(1));
