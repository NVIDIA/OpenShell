...
use crate::auth::workspace_authz::{
    AuthGrant, MinWorkspaceRole, authorize_workspace_selector, require_platform_admin,
};
use crate::credentials::SecretMaterialScope;
use crate::pagination::{MAX_PAGE_SIZE, Pagination};
use crate::persistence::{
    ObjectListQuery, ObjectType, PersistenceError, WriteCondition, current_time_ms,
};
use crate::storage_proto::{
    DelegatedIdentityCredential, DelegatedIdentityRefreshLease, SandboxDelegatedIdentityRecord,
    StoredRefreshMaterialDeletion,
};
use openshell_core::proto::datamodel::v1::{ObjectMeta, WorkspaceSelector};
use openshell_core::proto::{
    AuthorizeDelegatedIdentityRequest, AuthorizeDelegatedIdentityResponse, CredentialHandle,
    DelegatedIdentityAuthorizationGrant, DelegatedIdentityCredentialSummary,
    DelegatedIdentityRequest, DeleteDelegatedIdentityCredentialRequest,
    DeleteDelegatedIdentityCredentialResponse, ExtendSandboxDelegatedIdentityRequest,
    ExtendSandboxDelegatedIdentityResponse, GetDelegatedIdentityAuthorizationStatusRequest,
...
const REFRESH_LEASE_RENEW_INTERVAL: Duration = Duration::from_secs(30);
const REFRESH_STEP_TIMEOUT: Duration = Duration::from_secs(30);
const REFRESH_MIN_LEASE_LIFETIME_MS: i64 = 60_000;
...
static DELEGATED_IDENTITY_HTTP_CLIENT: LazyLock<Result<reqwest::Client, String>> =
    LazyLock::new(|| {
        ...
        reqwest::Client::builder()
            .timeout(REFRESH_STEP_TIMEOUT)
            .connect_timeout(REFRESH_STEP_TIMEOUT)
            .no_proxy()
...
pub async fn prepare_for_sandbox_create(
    ...
    let delegated_until_ms = delegated_until_from_duration(state, request.duration_ms)?;
...
pub async fn handle_status(
    ...
    let sandbox =
        authorized_sandbox_by_name(state, &principal, req.workspace_scope.as_ref(), &req.name)
            .await?;
    let record = sandbox_delegated_identity_record(state, &sandbox).await?;
    let delegation = record
        .as_ref()
...
pub async fn handle_extend(
    ...
    let user = require_user(&principal)?;
    let delegated_until_ms = delegated_until_from_duration(state, requested.duration_ms)?;
    let credential = get_user_credential(state, user).await?.ok_or_else(|| {
        Status::failed_precondition(
    ...
    state
        ...
        .update_message_cas::<SandboxDelegatedIdentityRecord, _>(record.object_id(), 0, |current| {
            if let Some(delegation) = current.delegated_identity.as_mut() {
                ...
                delegation.delegated_until_ms = delegated_until_ms;
                delegation.withdrawn_at_ms = 0;
            }
        })
        .await
...
pub async fn handle_list_credentials(
    ...
    let pagination = Pagination::new(
        req.page_size,
        &req.page_token,
        "ListDelegatedIdentityCredentials",
        &[],
    )?;
    let after = pagination.object_cursor()?;
    let page = state
        .store
        .list_message_page::<DelegatedIdentityCredential>(
...
async fn ensure_no_active_delegation_references(
    ...
    let records = state
        .store
        .collect_messages::<SandboxDelegatedIdentityRecord>(ObjectListQuery::AllWorkspaces)
        .await
        .map_err(|e| {
...
fn delegated_until_from_duration(
    state: &Arc<ServerState>,
    duration_ms: i64,
) -> Result<i64, Status> {
    if duration_ms <= 0 {
        return Err(Status::invalid_argument(
            "delegated_identity.duration_ms must be greater than zero",
        ));
    }
    ...
    Ok(current_time_ms().saturating_add(duration_ms))
}
...
async fn persist_staged_cleanup_handles(
    ...
    persist_staged_cleanup_handles_cas(
        state,
        credential_id,
        handles,
...
async fn persist_staged_cleanup_handles_cas(
    state: &Arc<ServerState>,
    credential_id: &str,
    handles: &HashMap<String, CredentialHandle>,
    ...
    loop {
        ...
        match result {
            ...
            Err(PersistenceError::Conflict { .. }) => {
                // Internal version 0 makes the next CAS reload and merge into the
                // latest credential. Keep the handles until a write succeeds.
...
async fn run_credential_cleanup_tick(state: &Arc<ServerState>) -> Result<(), Status> {
    ...
    loop {
        ...
        for credential in &page.messages {
...
async fn refresh_with_owned_lease(
    ...
    let renewal = async {
        let mut ticker = tokio::time::interval_at(
            tokio::time::Instant::now() + REFRESH_LEASE_RENEW_INTERVAL,
            REFRESH_LEASE_RENEW_INTERVAL,
        );
        ticker.set_missed_tick_behavior(tokio::time::MissedTickBehavior::Delay);
        loop {
            ticker.tick().await;
            if let Err(status) = renew_refresh_lease(state, &credential_id, owner_id).await {
                break Err::<(), Status>(status);
            }
        }
    };
    refresh_with_owned_lease_and_renewal(state, credential, owner_id, renewal).await
}

async fn refresh_with_owned_lease_and_renewal(
    state: &Arc<ServerState>,
    credential: DelegatedIdentityCredential,
    owner_id: &str,
    renewal: impl Future<Output = Result<(), Status>>,
) -> Result<ResolvedDelegatedAccessToken, Status> {
    let (failure_tx, failure_rx) = tokio::sync::watch::channel(None);
    let mut operation = Box::pin(refresh_with_owned_lease_inner(
        state,
        credential.clone(),
        owner_id,
        &failure_rx,
    ));
    let mut renewal = Box::pin(renewal);
    // No detached task: cancellation drops renewal along with the operation.
    let result = tokio::select! {
        result = &mut operation => result,
        result = &mut renewal => {
            if let Err(status) = result {
                failure_tx.send_replace(Some(status));
            }
            // Do not cancel a potentially partial credential write. The operation
            // observes this failure before exchange/commit and compensates staging.
            operation.await
        }
    };
    if result.is_err() {
    ...
    // Commit has cleared the lease; backend cleanup no longer needs renewal.
    resolved.credential =
        cleanup_pending_secret_deletions_best_effort(state, resolved.credential).await;
    Ok(resolved)
}
...
async fn renew_refresh_lease(
    ...
    tokio::time::timeout(REFRESH_STEP_TIMEOUT, async {
        loop {
            let current = state
                .store
                .get_message::<DelegatedIdentityCredential>(credential_id)
                ...
                .map_err(|error| {
                    crate::grpc::persistence_error_to_status(
                        error,
                        "reload delegated credential for lease renewal",
                    )
                })?
                .ok_or_else(|| {
                    Status::failed_precondition("delegated identity credential is missing")
            ...
            let result = state
                ...
                .update_message_cas::<DelegatedIdentityCredential, _>(
                    ...
                    current.get_resource_version(),
                    |credential| {
                        // The CAS helper reloads before running this closure; do
                        // not revive a lease that expired during that read.
                        if let Err(status) = ensure_refresh_lease_owner(credential, owner_id) {
                            renewal_error = Some(status);
                            return;
                        }
                        // Only extend this owner's lease; merge all other concurrent fields.
                        if let Some(lease) = credential.refresh_lease.as_mut() {
                            lease.expires_at_ms =
                                current_time_ms().saturating_add(REFRESH_LEASE_DURATION_MS);
                        }
                    },
                )
                .await;
            match result {
                Ok(renewed) => {
                    if let Some(status) = renewal_error {
                        return Err(status);
                    }
                    ensure_refresh_lease_owner(&renewed, owner_id)?;
                    // Reserve at least two request timeouts, including margin
                    // for a delayed CAS response, before allowing exchange.
                    if renewed.refresh_lease.as_ref().is_none_or(|lease| {
                        lease.expires_at_ms.saturating_sub(current_time_ms())
                            < REFRESH_MIN_LEASE_LIFETIME_MS
                    }) {
                        return Err(Status::deadline_exceeded(
                            "delegated credential refresh lease has insufficient lifetime",
                        ));
                    }
                    return Ok(());
                }
                ...
                Err(error) => {
                    return Err(crate::grpc::persistence_error_to_status(
                        error,
                        "renew delegated credential refresh lease",
                    ));
                }
            }
        }
    ...
    .await
    .map_err(|_| {
        Status::deadline_exceeded("delegated credential refresh lease renewal timed out")
    })?
}

fn ensure_refresh_renewal_healthy(
    failure: &tokio::sync::watch::Receiver<Option<Status>>,
) -> Result<(), Status> {
    failure
        .borrow()
        .as_ref()
        .map_or(Ok(()), |status| Err(status.clone()))
}
...
async fn refresh_with_owned_lease_inner(
    ...
    credential: DelegatedIdentityCredential,
    owner_id: &str,
    renewal_failure: &tokio::sync::watch::Receiver<Option<Status>>,
) -> Result<ResolvedDelegatedAccessToken, Status> {
    ensure_refresh_lease_owner(&credential, owner_id)?;
    let material = tokio::time::timeout(
        REFRESH_STEP_TIMEOUT,
        resolve_delegated_secret_material(state, &credential),
    )
    .await
    .map_err(|_| Status::deadline_exceeded("resolve delegated credential material timed out"))??;
    ...
    let scopes = credential
        ...
        .collect::<Vec<_>>();
    ensure_refresh_renewal_healthy(renewal_failure)?;
    // Reload and renew immediately before the irreversible exchange. A full
    // lease leaves margin beyond the HTTP client's 30-second request timeout.
    renew_refresh_lease(state, credential.object_id(), owner_id).await?;
    ensure_refresh_renewal_healthy(renewal_failure)?;
    ...
    let expires_at_ms = tokio::time::timeout(
        REFRESH_STEP_TIMEOUT,
        validate_delegated_access_token_subject_value(
            state,
    ...
    .await
    .map_err(|_| Status::deadline_exceeded("delegated access token validation timed out"))??;
    ensure_refresh_renewal_healthy(renewal_failure)?;
...
async fn commit_refreshed_material(
    ...
    let deadline = tokio::time::Instant::now() + REFRESH_STEP_TIMEOUT;
    loop {
        if tokio::time::Instant::now() >= deadline {
            return Err(Status::deadline_exceeded(
                "delegated credential refresh commit retries timed out",
            ));
        }
        ...
        .await
        .map_err(|_| {
            Status::deadline_exceeded("reload delegated credential before refresh commit timed out")
        })?
        .map_err(|error| {
            Status::internal(format!(
        ...
        .ok_or_else(|| Status::failed_precondition("delegated identity credential was deleted"))?;
        ensure_delegated_credential_usable(&current)?;
        ensure_refresh_renewal_healthy(renewal_failure)?;
        ensure_refresh_lease_owner(&current, owner_id)?;
        ...
        let result = state
            ...
            .update_message_cas::<DelegatedIdentityCredential, _>(
                ...
                current.get_resource_version(),
                |credential| {
                    // Re-check after the CAS helper's own read, before touching
                    // material. Version fencing alone does not check expiry.
                    if let Err(status) = ensure_refresh_renewal_healthy(renewal_failure)
                        .and_then(|()| ensure_refresh_lease_owner(credential, owner_id))
                    {
                        commit_error = Some(status);
                        return;
                    }
            ...
            .await;
        match result {
            Ok(updated) => return commit_error.map_or(Ok(updated), Err),
            Err(PersistenceError::Conflict { .. }) => {}
            Err(error) => {
                return Err(crate::grpc::persistence_error_to_status(
                    error,
...
fn ensure_refresh_lease_owner(
    ...
    if credential
        .refresh_lease
        .as_ref()
        .is_some_and(|lease| lease.owner_id == owner_id && lease.expires_at_ms > current_time_ms())
    {
        return Ok(());
    }
    Err(Status::aborted(
        "delegated credential refresh lease ownership changed or expired",
    ))
}
...
mod tests {
    ...
    use openshell_core::proto::{
        ...
        DelegatedIdentityRequest, DeleteDelegatedIdentityCredentialRequest,
        ExtendSandboxDelegatedIdentityRequest, GetDelegatedIdentityAuthorizationStatusRequest,
        GetSandboxDelegatedIdentityStatusRequest, ListDelegatedIdentityCredentialsRequest,
        RevokeDelegatedIdentityCredentialRequest, Sandbox, SandboxDelegatedIdentity, SandboxSpec,
        SandboxStatus, workspace_selector,
    };
    ...
    #[tokio::test]
    async fn delegation_duration_is_bounded_and_anchored_to_gateway_clock() {
        let state = test_server_state().await;
        let max_duration_ms = i64::try_from(state.config.max_delegated_identity_duration_secs)
            .expect("test maximum fits in i64")
            .saturating_mul(1000);
        let before = current_time_ms();

        let delegated_until_ms = super::delegated_until_from_duration(&state, max_duration_ms)
            .expect("the exact gateway maximum should be accepted");

        let after = current_time_ms();
        assert!(delegated_until_ms >= before.saturating_add(max_duration_ms));
        assert!(delegated_until_ms <= after.saturating_add(max_duration_ms));

        let zero = super::delegated_until_from_duration(&state, 0)
            .expect_err("a zero duration should be rejected");
        assert_eq!(zero.code(), Code::InvalidArgument);

        let too_long =
            super::delegated_until_from_duration(&state, max_duration_ms.saturating_add(1))
                .expect_err("a duration over the gateway maximum should be rejected");
        assert_eq!(too_long.code(), Code::FailedPrecondition);
    }
    ...
    #[tokio::test]
    async fn refresh_lease_renewal_preserves_latest_credential_fields() {
        let state = test_server_state().await;
        let credential = put_test_credential(&state, "delegated-lease-renewal").await;
        let original_expiry = current_time_ms() + 60_000;
        let current = state
            .store
            .update_message_cas::<DelegatedIdentityCredential, _>(
                credential.object_id(),
                credential.get_resource_version(),
                |credential| {
                    credential.refresh_lease = Some(DelegatedIdentityRefreshLease {
                        owner_id: "owner-a".to_string(),
                        expires_at_ms: original_expiry,
                    });
                    credential.last_refresh_at_ms = 1234;
                    credential
                        .metadata
                        .as_mut()
                        .unwrap()
                        .annotations
                        .insert("concurrent-change".to_string(), "preserved".to_string());
                    credential
                        .pending_secret_deletions
                        .push(StoredRefreshMaterialDeletion::default());
                },
            )
            .await
            .unwrap();

        super::renew_refresh_lease(&state, credential.object_id(), "owner-a")
            .await
            .expect("live owner should renew");
        let mut renewed = state
            ...
            .unwrap()
            .unwrap();
        assert!(renewed.get_resource_version() > current.get_resource_version());
        assert!(renewed.refresh_lease.as_ref().unwrap().expires_at_ms > original_expiry);
        // Apart from version and lease expiry, renewal must leave the record untouched.
        renewed.metadata.as_mut().unwrap().resource_version = current.get_resource_version();
        renewed.refresh_lease = current.refresh_lease.clone();
        assert_eq!(renewed, current);
    }

    #[tokio::test]
    async fn refresh_lease_renewal_cannot_revive_expiry_or_overwrite_takeover() {
        let state = test_server_state().await;
        let mut credential = put_test_credential(&state, "delegated-lease-lost").await;
        for (owner_id, expires_at_ms) in [
            ("owner-a", current_time_ms() - 1),
            ("owner-b", current_time_ms() + 120_000),
        ] {
            credential = state
                .store
                .update_message_cas::<DelegatedIdentityCredential, _>(
                ...
                .await
                .unwrap();
            let status = super::renew_refresh_lease(&state, credential.object_id(), "owner-a")
                .await
                .expect_err("expired or replaced lease cannot be renewed");
            assert_eq!(status.code(), Code::Aborted);
            let stored = state
                .store
                .get_message::<DelegatedIdentityCredential>(credential.object_id())
    ...
    #[tokio::test]
    async fn refresh_lease_renewal_rejects_revoked_credentials() {
        let state = test_server_state().await;
        let credential = put_test_credential(&state, "delegated-lease-revoked").await;
        let credential = state
            .store
            .update_message_cas::<DelegatedIdentityCredential, _>(
                credential.object_id(),
                ...
                |credential| {
                    credential.refresh_lease = Some(DelegatedIdentityRefreshLease {
                        owner_id: "owner-a".to_string(),
                        expires_at_ms: current_time_ms() + 120_000,
                    });
        ...
        let status = super::renew_refresh_lease(&state, credential.object_id(), "owner-a")
            .await
            .expect_err("revocation must prevent renewal");
        assert_eq!(status.code(), Code::FailedPrecondition);
        assert_eq!(
            state
                .store
    ...
    #[tokio::test]
    async fn live_refresh_renews_while_staging_and_compensates_renewal_failure() {
        for (takeover, renewal_failure) in [(false, false), (true, false), (false, true)] {
            let server = wiremock::MockServer::start().await;
            mount_test_oidc_issuer(&server).await;
            let issuer = server.uri();
            let state = test_server_state_with_oidc(issuer.clone()).await;
            let access_token = mint_test_access_token(&issuer, "alice", current_time_secs() + 3600);
            mount_refresh_token_rotation_response(&server, &access_token, "rotated-refresh").await;
            let credential =
                put_test_credential_for_subject(&state, "delegated-live-renewal", "alice").await;
            let credential = state
                .store
                .update_message_cas::<DelegatedIdentityCredential, _>(
                    credential.object_id(),
                    credential.get_resource_version(),
                    |credential| {
                        credential.issuer = issuer.clone();
                        credential.client_id = TEST_AUDIENCE.to_string();
                        credential.access_token_expires_at_ms = current_time_ms() - 1;
                    },
                )
                .await
                .unwrap();
            let credential_id = credential.object_id().to_string();
            let original_handles = credential.secret_material_handles.clone();
            let super::RefreshLeaseAttempt::Acquired {
                credential,
                owner_id,
            } = super::try_acquire_refresh_lease(&state, &credential)
                .await
                .unwrap()
            else {
                panic!("initial owner should acquire lease")
            };
            let (store_started, release_store) = state.credentials.gate_next_store();
            let (renew_now_tx, renew_now_rx) = tokio::sync::oneshot::channel();
            let (renewed_tx, renewed_rx) = tokio::sync::oneshot::channel();
            let task_state = Arc::clone(&state);
            let task_credential_id = credential_id.clone();
            let refresh = tokio::spawn(async move {
                let renewal = async {
                    renew_now_rx.await.expect("test should trigger renewal");
                    let result = if renewal_failure {
                        Err(tonic::Status::deadline_exceeded("test renewal timeout"))
                    } else {
                        super::renew_refresh_lease(&task_state, &task_credential_id, &owner_id)
                            .await
                    };
                    renewed_tx
                        .send(result.clone())
                        .expect("test should observe renewal");
                    result?;
                    std::future::pending::<Result<(), tonic::Status>>().await
                };
                super::refresh_with_owned_lease_and_renewal(
                    &task_state,
                    *credential,
                    &owner_id,
                    renewal,
            ...
            store_started
                .await
                .expect("refresh should reach replacement staging");
            let current = state
                .store
                .get_message::<DelegatedIdentityCredential>(&credential_id)
                .await
                .unwrap()
                .unwrap();
            let short_expiry = current_time_ms() + 10_000;
            let current = state
                .store
                .update_message_cas::<DelegatedIdentityCredential, _>(
                    &credential_id,
                    current.get_resource_version(),
                    |credential| {
                        let lease = credential.refresh_lease.as_mut().unwrap();
                        lease.expires_at_ms = short_expiry;
                        if takeover {
                            lease.owner_id = "owner-b".to_string();
                        }
                        credential
                            .metadata
                            .as_mut()
            ...
            renew_now_tx
                .send(())
                .expect("trigger renewal during staging");
            let renewal_result = renewed_rx.await.expect("renewal should complete");
            if takeover || renewal_failure {
                assert_eq!(
                    renewal_result.unwrap_err().code(),
                    if takeover {
                        Code::Aborted
                    } else {
                ...
                // A storage gate and shortened lease exercise renewal without
                // sleeping for the production lease duration.
                renewal_result.expect("live owner should renew while storage is blocked");
                let renewed = state
                    .store
                    .get_message::<DelegatedIdentityCredential>(&credential_id)
                    .await
                    .unwrap()
                    .unwrap();
                assert!(renewed.refresh_lease.as_ref().unwrap().expires_at_ms > short_expiry);
                assert!(matches!(
                    super::try_acquire_refresh_lease(&state, &renewed)
                        .await
                        .unwrap(),
                    super::RefreshLeaseAttempt::Held
                ));
            }
            release_store.send(()).expect("release replacement storage");
            ...
            if takeover || renewal_failure {
                ...
                assert_eq!(stored.secret_material_handles, original_handles);
                if takeover {
                    assert_eq!(stored.refresh_lease, current.refresh_lease);
                } else {
                    assert!(stored.refresh_lease.is_none());
                }
            } else {
                assert_eq!(result.unwrap().access_token, access_token);
                assert!(stored.refresh_lease.is_none());
                assert_ne!(stored.secret_material_handles, original_handles);
            }
            assert_eq!(
                state.credentials.stored_credential_count(),
                Some(original_handles.len())
            );
        }
    }
    ...
    #[tokio::test]
    async fn staged_cleanup_cas_conflict_preserves_concurrent_changes_and_retry_handles() {
        let state = test_server_state().await;
        let credential = put_test_credential(&state, "delegated-staged-cleanup-cas").await;
        let credential_id = credential.object_id();
        let staged_handles = super::store_staged_secret_material(
            &state,
            credential_id,
            &HashMap::from([
                ("access_token".to_string(), "staged-access".to_string()),
                ("refresh_token".to_string(), "staged-refresh".to_string()),
            ]),
        )
        .await
        .expect("stage material awaiting cleanup");
        let concurrent_handles = super::store_staged_secret_material(
            &state,
            credential_id,
            &HashMap::from([
                ("access_token".to_string(), "other-access".to_string()),
                ("refresh_token".to_string(), "other-refresh".to_string()),
            ]),
        )
        .await
        .expect("stage concurrent pending material");
        let updated = state
            .store
            .update_message_cas::<DelegatedIdentityCredential, _>(
                credential_id,
                credential.get_resource_version(),
                |current| {
                    current.revoked_at_ms = 1234;
                    super::enqueue_handle_deletions(
                        &mut current.pending_secret_deletions,
                        concurrent_handles.clone(),
                    );
                },
            )
            .await
            .expect("concurrent update after cleanup's initial read");
        assert!(updated.get_resource_version() > credential.get_resource_version());

        super::persist_staged_cleanup_handles_cas(
            &state,
            credential_id,
            &staged_handles,
            credential.get_resource_version(),
        ...
        .await
        .expect("stale cleanup CAS should retry against the latest credential");
        ...
        let stored = state
            ...
            .unwrap()
            .unwrap();
        assert_eq!(stored.revoked_at_ms, 1234);
        assert_eq!(
            stored.secret_material_handles,
            credential.secret_material_handles
        );
        assert_eq!(stored.pending_secret_deletions.len(), 4);
        for (material_key, handle) in staged_handles.iter().chain(concurrent_handles.iter()) {
            assert!(stored.pending_secret_deletions.iter().any(|deletion| {
                deletion.material_key == *material_key && deletion.handle.as_ref() == Some(handle)
            }));
        }

        super::run_credential_cleanup_tick(&state)
            .await
            .expect("worker should delete both recorded batches");
        let cleaned = state
            .store
            .get_message::<DelegatedIdentityCredential>(credential_id)
            .await
            .unwrap()
            .unwrap();
        assert!(cleaned.pending_secret_deletions.is_empty());
        assert_eq!(
            cleaned.secret_material_handles,
            credential.secret_material_handles
        );
        // The test driver counts individual values, not staged storage batches.
        assert_eq!(
            state.credentials.stored_credential_count(),
            Some(credential.secret_material_handles.len())
        );
        let active_material = state
            .credentials
            .resolve_secret_material(
                super::delegated_secret_scope(credential_id),
                &cleaned.secret_material_handles,
            )
            .await
            .expect("cleanup should preserve both active secret values");
        assert_eq!(
            active_material,
            HashMap::from([
                ("access_token".to_string(), "access".to_string()),
    ...
    #[tokio::test]
    async fn sandbox_delegated_identity_status_requires_named_workspace_selector() {
        let state = test_server_state().await;

        let missing = super::handle_status(
            &state,
            authed_request(GetSandboxDelegatedIdentityStatusRequest {
                name: "regular".to_string(),
        ...
        .await
        .expect_err("workspace selector must be explicit");
        assert_eq!(missing.code(), Code::InvalidArgument);

        let all_workspaces = super::handle_status(
            &state,
            authed_request(GetSandboxDelegatedIdentityStatusRequest {
                name: "regular".to_string(),
        ...
        .await
        .expect_err("all-workspaces selector must be rejected");
        assert_eq!(all_workspaces.code(), Code::InvalidArgument);
    }

    #[tokio::test]
    async fn delegated_identity_credential_list_uses_continuation_tokens() {
        let state = test_server_state().await;
        put_test_credential(&state, "delegated-identity-list-a").await;
        put_test_credential(&state, "delegated-identity-list-b").await;

        let first = super::handle_list_credentials(
            &state,
            authed_request(ListDelegatedIdentityCredentialsRequest {
                page_size: 1,
                page_token: String::new(),
            }),
        )
        .await
        .expect("first credential page")
        .into_inner();
        assert_eq!(first.credentials.len(), 1);
        assert!(!first.next_page_token.is_empty());

        let second = super::handle_list_credentials(
            &state,
            authed_request(ListDelegatedIdentityCredentialsRequest {
                page_size: 100,
        ...
        .await
        .expect("second credential page")
        .into_inner();
        assert_eq!(second.credentials.len(), 1);
        assert!(second.next_page_token.is_empty());
        assert_ne!(
            first.credentials[0].metadata.as_ref().map(|meta| &meta.id),
            second.credentials[0].metadata.as_ref().map(|meta| &meta.id)
        );
    }
