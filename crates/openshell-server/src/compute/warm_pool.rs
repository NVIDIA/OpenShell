...
use openshell_core::proto::compute::v1::{GetSandboxRequest, SyncWarmPoolsRequest, WarmPoolTarget};
...
pub async fn completed(
    ...
    completed_with_runtime_identity(runtime, expected, None).await
}

fn runtime_binding_is_persisted(
    runtime: &super::ComputeRuntime,
    sandbox: &openshell_core::proto::Sandbox,
) -> bool {
    sandbox.metadata.as_ref().is_some_and(|metadata| {
        metadata
            .annotations
            .get(super::COMPUTE_DRIVER_ANNOTATION)
            .is_some_and(|driver| driver == runtime.configured_driver_name())
            && metadata
                .annotations
                .get(super::COMPUTE_RUNTIME_IDENTITY_ANNOTATION)
                .is_some_and(|identity| !identity.is_empty())
    })
}

async fn completed_with_runtime_identity(
    runtime: &super::ComputeRuntime,
    expected: &openshell_core::proto::Sandbox,
    runtime_identity: Option<&str>,
) -> Result<(), Status> {
    if runtime.supports_sandbox_authentication() && runtime_identity.is_some_and(str::is_empty) {
    ...
    for _ in 0..4 {
        ...
        if runtime.supports_sandbox_authentication()
            && runtime_identity.is_none()
            && !runtime_binding_is_persisted(runtime, &sandbox)
        {
            return Err(Status::unavailable(
                "compute runtime identity persistence is pending",
            ));
        }
        if let Some(runtime_identity) = runtime_identity {
            let annotations = &mut sandbox
                ...
                .annotations;
            annotations.insert(
                super::COMPUTE_DRIVER_ANNOTATION.to_string(),
                runtime.configured_driver_name().to_string(),
            );
            annotations.insert(
                super::COMPUTE_RUNTIME_IDENTITY_ANNOTATION.to_string(),
...
async fn finish_if_inactive(
    ...
    {
        // A ready observation proves activation, but an authenticated driver
        // must repeat the idempotent claim so recovery can obtain and persist
        // the exact runtime binding before completing the allocation.
        if state.compute.supports_sandbox_authentication()
            && !runtime_binding_is_persisted(&state.compute, sandbox)
        {
            return Ok(false);
        }
...
pub async fn create(
    ...
    mut lifecycle_guard: Option<super::SandboxLifecycleGuard>,
    mut global_guard: Option<super::SandboxSyncGuard>,
) -> Result<openshell_core::proto::Sandbox, Status> {
    use crate::auth::sandbox_session::PersistedSandboxIdentity;
    if lifecycle_guard.is_none() {
        let (lifecycle, global) = state
            .compute
    ...
    loop {
        ...
        match Box::pin(state.compute.create_sandbox_authenticated_with_guards(
            sandbox.clone(),
    ...
    // Recovery enters the ordinary lifecycle path and acquires this sandbox's
    // mutation guard itself. Release the create guards after the driver call so
    // reconciliation cannot deadlock behind its own successful reservation.
    drop(global_guard.take());
...
async fn recover_with_retry(
    ...
    loop {
        ...
        match result {
            Ok(response) => {
                return completed_with_runtime_identity(
                    &state.compute,
                    &current,
                    Some(&response.into_inner().runtime_identity),
                )
                .await;
            }
            Err(error) if pair.is_none() && error.code() == Code::AlreadyExists => {
                ...
                let runtime_identity = existing_runtime_identity(state, &current).await?;
                return completed_with_runtime_identity(
                    &state.compute,
                    &current,
...
async fn existing_runtime_identity(
    state: &crate::ServerState,
    sandbox: &openshell_core::proto::Sandbox,
    ...
    let response = state
        ...
        .call(
            ...
            Some(&sandbox_id),
            |driver| async move { driver.get_sandbox(Request::new(request)).await },
        )
        .await
        .map_err(|error| {
            if error.code() == Code::NotFound {
                Status::unavailable("existing compute runtime is not yet observable")
            } else {
                error
            }
        })?
    ...
    let observed = response
        .sandbox
        .ok_or_else(|| Status::unavailable("compute driver omitted the existing runtime"))?;
    if observed.id != sandbox_id {
        return Err(Status::failed_precondition(
            "compute driver returned a different existing runtime",
        ));
    }
    if response.runtime_identity.is_empty() {
        return Err(Status::unavailable(
            "existing compute runtime identity is not yet observable",
        ));
    }
    Ok(response.runtime_identity)
}
...
mod tests {
    ...
    async fn a_new_gateway_recovers_the_persisted_pair_and_deletes_it_without_repooling() {
        ...
        let runtime = super::super::new_test_runtime_with_driver(
            store.clone(),
            "test-kubernetes",
            Arc::new(
                super::super::NoopTestDriver::authenticating_sandbox_with_runtime(
                    "logical",
                    "kubernetes://openshell/parent-uid/supervisor-pod-uid",
                ),
            ),
        );
        ...
        assert_eq!(
            recovered.metadata.as_ref().unwrap().annotations
                [super::super::COMPUTE_DRIVER_ANNOTATION],
            "test-kubernetes"
        );
        assert_eq!(
            recovered.metadata.as_ref().unwrap().annotations
                [super::super::COMPUTE_RUNTIME_IDENTITY_ANNOTATION],
            "kubernetes://openshell/parent-uid/supervisor-pod-uid"
        );
