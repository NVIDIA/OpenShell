...
use openshell_core::proto::compute::v1::{
    ...
    StartSandboxRequest, StopSandboxRequest, SupervisorSessionModel, ValidateSandboxCreateRequest,
...
impl ComputeRuntime {
    ...
    /// Authenticate a driver-native sandbox credential. Returns the
    /// authenticated sandbox ID and whether the driver reported a process-scoped
    /// caller (e.g. a proxy-pod agent pod), which narrows the minted token.
    pub(crate) async fn authenticate_sandbox(
        ...
        self.driver
            ...
            .map(|response| {
                ...
                (response.sandbox_id, response.scoped_process_caller)
            })
    }
...
mod tests {
    ...
    fn composed_phase_requires_a_session_by_default() {
        ...
        let composed = ComposedPhase::new(&status, false, false);
        assert_eq!(composed.phase, SandboxPhase::Provisioning);
        assert!(composed.backend_ready_without_session);

        let composed = ComposedPhase::new(&status, true, false);
        assert_eq!(composed.phase, SandboxPhase::Ready);
    }
    ...
    fn sessionless_sandboxes_are_not_ready_until_the_backend_says_so() {
        ...
        assert_eq!(
            ComposedPhase::new(&status, false, false).phase,
            SandboxPhase::Provisioning
        );
    }
