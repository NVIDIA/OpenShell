...
mod tests {
    ...
    #[test]
    fn start_sandbox_request_uses_optional_wire_field_six_for_durable_snapshot() {
        ...
        // Field 6 is length-delimited, hence its protobuf key is 0x32. The
        // legacy form does not emit it, preserving existing fields 1–5.
        assert!(!legacy_wire.contains(&0x32));
        assert!(snapshot_wire.contains(&0x32));
        assert_eq!(
