...
use openshell_core::supervisor_bootstrap::{
    SupervisorBootstrapActivationRequest, SupervisorBootstrapActivator, SupervisorBootstrapBinding,
...
impl SupervisorBootstrapActivator for GatewaySupervisorBootstrapActivator {
    async fn activate_registered_supervisor(
        ...
        let target = WarmPodActivationTarget {
            ...
            activation_guard: request.activation_guard,
        };
...
impl WarmPodActivationValidator for TrustDriverActivationRequest {
    async fn validate(
        ...
        Ok(ValidatedWarmPodActivation {
            ...
            activation_guard: target.activation_guard.clone(),
...
fn ensure_validated_activation_matches_pending(
    ...
    let SupervisorBootstrapBinding::WarmPending { activation_guard } = &pending.binding else {
        return Err(Status::permission_denied(
            "pending registration is already bound to a sandbox",
        ));
    };
    if validated.activation_guard != target.activation_guard
        || validated.activation_guard != *activation_guard
    {
        return Err(Status::permission_denied(
            "validated activation guard does not match pending registration",
        ));
    }
...
pub async fn mint_pod_activation(
    ...
    let mut startup_metadata = std::collections::HashMap::default();
    let main_process_spec = record
        .metadata
        .as_ref()
        .and_then(|metadata| metadata.annotations.get(MAIN_PROCESS_SPEC_ANNOTATION))
        .cloned()
        .or_else(|| {
            openshell_core::sandbox_env::MainProcessConfig::encode_public_spec(
                record.spec.as_ref(),
                false,
            )
            .ok()
        });
    if let Some(main_process_spec) = main_process_spec {
        startup_metadata.insert(MAIN_PROCESS_SPEC.to_string(), main_process_spec);
    }
    info!(
        sandbox_id = %sandbox_id,
        reason,
...
mod tests {
    ...
    async fn pending_warm_pod_receives_activation_token() {
        ...
        let main_process = received
            .startup_metadata
            .get(MAIN_PROCESS_SPEC)
            .expect("main process activation metadata");
        let decoded = openshell_core::sandbox_env::MainProcessConfig::decode(main_process).unwrap();
    ...
    async fn revalidation_metadata_must_match_pending_registration() {
        ...
        validated.activation_guard = "other-owner".to_string();
