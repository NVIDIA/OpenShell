...
use openshell_core::proto::compute::v1::{
    ...
    DriverSandbox, GatewayListenerRequirement, GatewayLocalInterfaceRequirement,
...
impl FakeComputeDriver {
    ...
    #[must_use]
    pub fn with_gateway_local_interface_requirement(self, reason: impl Into<String>) -> Self {
        self.with_state(|state| {
            state
                .gateway_listener_requirements
                .push(GatewayListenerRequirement {
                    reason: reason.into(),
                    selector: Some(Selector::LocalInterface(
                        GatewayLocalInterfaceRequirement {},
                    )),
                });
        });
        self
...
impl ComputeDriver for FakeComputeDriver {
    ...
    async fn get_gateway_listener_requirements(
        ...
        let response = self.with_state(|state| {
            ...
            state.gateway_listener_requirements_supported.then(|| {
                GetGatewayListenerRequirementsResponse {
                    requirements: state.gateway_listener_requirements.clone(),
                }
            })
        });
        response.map(Response::new).ok_or_else(|| {
            Status::unimplemented("gateway listener requirements are not implemented")
        })
    }
