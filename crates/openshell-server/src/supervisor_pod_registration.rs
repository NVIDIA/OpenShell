...
const ACTIVATED_TOMBSTONE_TTL: Duration = Duration::from_hours(1);
...
struct PendingRegistration {
    ...
    sender: mpsc::Sender<StdResult<SupervisorActivationMessage, Status>>,
    session_id: u64,
...
impl SupervisorPodRegistrationRegistry {
    ...
    pub(crate) fn activate_if_session(
        ...
        instance_id: &str,
        session_id: u64,
        activation: SupervisorActivationMessage,
    ) -> Result<(), Status> {
        let now = Instant::now();
        let pending = {
            let mut inner = self.inner.lock().expect("pending pod registry poisoned");
    ...
    fn remove_if_session(&self, instance_id: &str, session_id: u64) {
        ...
        if let Some(pending) = removed {
            debug!(
                driver = %pending.identity.driver,
                instance_id = %instance_id,
                pending_ms = pending.registered_at.elapsed().as_millis(),
...
pub struct PendingRegistrationStream {
    ...
    instance_id: String,
    session_id: u64,
    inner: ReceiverStream<StdResult<SupervisorActivationMessage, Status>>,
}

impl Stream for PendingRegistrationStream {
    type Item = StdResult<SupervisorActivationMessage, Status>;
...
mod tests {
    ...
    fn bootstrap_identity(instance_id: &str) -> SupervisorBootstrapIdentity {
        SupervisorBootstrapIdentity {
            ...
            binding:
                openshell_core::supervisor_bootstrap::SupervisorBootstrapBinding::WarmPending {
                    activation_guard: "owner-uid-a".to_string(),
                },
        }
    }

    fn activation() -> SupervisorActivationMessage {
