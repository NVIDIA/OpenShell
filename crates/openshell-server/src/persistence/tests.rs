...
#[tokio::test]
async fn delete_with_name_scoped_if_version_removes_matching_pair_only() {
    use super::WriteCondition;
    ...
    let deleted = store
        .delete_with_name_scoped_if_version("sandbox", "sandbox-id-1", 1, "sandbox_settings")
        .await
        .unwrap();
    ...
    assert!(
        store
            .get("sandbox_settings", "settings-id-1")
            .await
            .unwrap()
            .is_none()
    );
    assert_eq!(
        store
            .get("sandbox_settings", "settings-id-2")
            .await
            .unwrap()
            .unwrap()
            .payload,
    ...
    // A settings row that reuses the old name after parent removal must not
    // be selected by a later cleanup attempt for the absent parent.
    store
        .put(
            "sandbox_settings",
            "settings-id-reused",
            "sandbox-1",
            b"reused-name settings",
            None,
        )
        .await
        .unwrap();
    ...
    assert!(
        store
            .get("sandbox_settings", "settings-id-reused")
            .await
            .unwrap()
            .is_some()
    );
...
#[tokio::test]
async fn delete_with_name_scoped_if_version_conflict_preserves_both() {
    use super::WriteCondition;
    ...
    let result = store
        .delete_with_name_scoped_if_version("sandbox", "sandbox-id-1", 99, "sandbox_settings")
        .await;
...
#[tokio::test]
async fn delete_with_name_scoped_removes_matching_pair_only() {
    let store = test_store().await;

    store
        .put("sandbox", "sandbox-id-1", "sandbox-1", b"sandbox", None)
        .await
        .unwrap();
    store
        .put(
    ...
    assert!(
        store
            .delete_with_name_scoped("sandbox", "sandbox-id-1", "sandbox_settings")
            .await
            .unwrap()
    );
    assert!(
