...
",
    ...
    pub async fn delete_with_name_scoped_if_version(
        ...
        // Feed the sibling delete exclusively from the exact parent delete's
        // RETURNING output. A stale resource version therefore cannot remove
        // settings, and both deletes commit as one statement.
        let row = sqlx::query(
            r"
...
deleted_scoped AS (
    DELETE FROM objects
    WHERE object_type = $4
      AND name IN (SELECT name FROM deleted_parent)
      AND NOT (object_type = $1 AND id = $2)
    RETURNING id
)
SELECT
    EXISTS (SELECT 1 FROM deleted_parent) AS parent_deleted,
    (SELECT COUNT(*) FROM deleted_scoped) AS scoped_deleted
",
        )
        .bind(parent_type)
        .bind(parent_id)
        ...
        let parent_deleted: bool = row.get("parent_deleted");
        if parent_deleted {
            Ok(true)
        } else {
