...
fn runtime_binding_matches(
    ...
    if expected_driver.map(String::as_str) == Some(driver_name)
        && expected_runtime_identity.map(String::as_str) == Some(runtime_identity)
    {
        return Ok(true);
    }
    let phase = SandboxPhase::try_from(sandbox.phase()).unwrap_or(SandboxPhase::Unknown);
    if crate::compute::warm_pool::pending(sandbox)
        || matches!(phase, SandboxPhase::Provisioning | SandboxPhase::Starting)
    {
        return Ok(false);
    }
...
mod tests {
    ...
    #[test]
    fn registration_waits_for_startup_runtime_binding_and_rejects_a_completed_mismatch() {
        ...
        assert!(!runtime_binding_matches("kubernetes", "pod-a", &sandbox).unwrap());
        ...
        assert_eq!(
            runtime_binding_matches("kubernetes", "pod-a", &sandbox)
                .unwrap_err()
                .code(),
        ...
        annotations.insert(
            crate::compute::COMPUTE_DRIVER_ANNOTATION.into(),
            "kubernetes".into(),
        );
        annotations.insert(
        ...
        assert!(runtime_binding_matches("kubernetes", "pod-a", &sandbox).unwrap());
        assert_eq!(
            runtime_binding_matches("kubernetes", "pod-b", &sandbox)
                .unwrap_err()
                .code(),
            tonic::Code::PermissionDenied
        );
