...
use openshell_core::proto::{
    ...
    sandbox_reference, tcp_forward_init,
};
use openshell_core::proto::{
...
/// Resolve a canonical public sandbox reference and authorize its persisted
/// workspace. Missing and unauthorized objects deliberately share one response
/// so neither reference form can be used as an existence oracle.
pub(super) async fn resolve_and_authorize_sandbox_reference(
    state: &Arc<ServerState>,
    principal: &crate::auth::principal::Principal,
    sandbox_ref: Option<&openshell_core::proto::SandboxReference>,
    min_role: MinWorkspaceRole,
) -> Result<Sandbox, Status> {
    let sandbox_ref =
        sandbox_ref.ok_or_else(|| Status::invalid_argument("sandbox_ref is required"))?;
    let sandbox = match sandbox_ref.identifier.as_ref() {
        Some(sandbox_reference::Identifier::Id(id)) => {
            if id.is_empty() {
                return Err(Status::invalid_argument("sandbox_ref.id is required"));
            }
            if sandbox_ref.workspace_scope.is_some() {
                return Err(Status::invalid_argument(
                    "sandbox_ref.workspace_scope must be omitted for an ID reference",
                ));
            }
            state.store.get_message::<Sandbox>(id).await
        }
        Some(sandbox_reference::Identifier::Name(name)) => {
            if name.is_empty() {
                return Err(Status::invalid_argument("sandbox_ref.name is required"));
            }
            let workspace = crate::auth::workspace_authz::selected_workspace_name(
                sandbox_ref.workspace_scope.as_ref(),
            )?;
            state
                .store
        ...
        None => {
            return Err(Status::invalid_argument(
                "sandbox_ref identifier is required",
            ));
        }
    }
...
pub(super) async fn handle_get_sandbox(
    ...
    let sandbox = resolve_and_authorize_sandbox_reference(
        state,
        &principal,
        req.sandbox_ref.as_ref(),
        MinWorkspaceRole::User,
    )
    .await?;
    Ok(Response::new(SandboxResponse {
...
async fn handle_stop_sandbox_inner(
    ...
    let principal = super::extract_principal(&request)?;
    let req = request.into_inner();
    let resolved = resolve_and_authorize_sandbox_reference(
        state,
        &principal,
        req.sandbox_ref.as_ref(),
        MinWorkspaceRole::User,
...
async fn handle_start_sandbox_inner(
    ...
    let workspace = resolved.object_workspace();
    let name = resolved.object_name();
    let sandbox = state.compute.start_sandbox(workspace, name).await?;
...
mod tests {
    ...
    fn interactive_exec_rejects_missing_sandbox_id() {
        ...
        let err = validate_interactive_exec_start(Some(msg)).unwrap_err();
        assert_eq!(err.code(), tonic::Code::InvalidArgument);
        assert!(err.message().contains("sandbox_ref"));
    }
    ...
    fn interactive_exec_accepts_valid_start() {
        ...
        assert_eq!(
            req.sandbox_ref.unwrap().identifier,
            Some(sandbox_reference::Identifier::Id("test-id".to_string()))
        );
    ...
    async fn interactive_exec_rejects_sandbox_not_found() {
        ...
        let sandbox_id = match req.sandbox_ref.unwrap().identifier.unwrap() {
            sandbox_reference::Identifier::Id(id) => id,
            sandbox_reference::Identifier::Name(_) => unreachable!(),
        };
