...
async fn concurrent_exec_claims_open_one_relay_and_store_no_secrets() {
    ...
    let mut messages = register(&state, &req.sandbox_id);
...
async fn confirmed_exit_including_nonzero_and_124_finalizes_before_output_delivery() {
    for code in [0, 7, 124] {
        ...
        assert_eq!(
            sandbox::wait_for_exec_terminal(async { Ok(code) }, 0, Some(owner.clone()))
                .await
                .unwrap(),
            Some(code)
        );
...
async fn timeout_and_lost_exit_status_never_finalize_or_expire_pending_launches() {
    ...
    let owner = completion(&state).await;
    assert_eq!(
        sandbox::wait_for_exec_terminal(std::future::pending(), 1, Some(owner.clone()))
            .await
            .unwrap(),
        None
    );
    ...
    let status = sandbox::wait_for_exec_terminal(async { Ok(0) }, 0, Some(owner))
        .await
        .unwrap_err();
...
async fn exec_restart_preserves_pending_and_terminal_fences_and_reauthorizes() {
    ...
    state
        .store
        .delete(Sandbox::object_type(), &req.sandbox_id)
        .await
        .unwrap();
    assert_eq!(
...
async fn interactive_wire_start_is_admitted_under_its_own_method_namespace() {
    ...
    drop(client.exec_sandbox(req).await.unwrap());
    receive_relay(&state, &mut messages).await;
    assert_eq!(
        state
            .store
