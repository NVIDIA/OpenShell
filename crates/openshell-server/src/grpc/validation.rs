...
fn validate_resource_requirement_fields(spec: &SandboxSpec) -> Result<(), Status> {
    ...
    {
        let set = cpu.limit.is_some() || cpu.request.is_some();
        if let Some(limit) = cpu.limit.as_deref() {
            validate_cpu_quantity(limit, "spec.resource_requirements.cpu.limit")?;
        }
        if let Some(request) = cpu.request.as_deref() {
            validate_cpu_quantity(request, "spec.resource_requirements.cpu.request")?;
        }
        if !set {
            return Err(Status::invalid_argument(
                "spec.resource_requirements.cpu must set limit or request",
            ));
        }
    }
    ...
    {
        let set = memory.limit.is_some() || memory.request.is_some();
        if let Some(limit) = memory.limit.as_deref() {
            validate_memory_quantity(limit, "spec.resource_requirements.memory.limit")?;
        }
        if let Some(request) = memory.request.as_deref() {
            validate_memory_quantity(request, "spec.resource_requirements.memory.request")?;
        }
        if !set {
            return Err(Status::invalid_argument(
                "spec.resource_requirements.memory must set limit or request",
            ));
        }
    }
...
fn validate_cpu_quantity(value: &str, field_name: &str) -> Result<(), Status> {
    openshell_core::quantity::validate_cpu_quantity(value, field_name)
        .map_err(Status::invalid_argument)
}

fn validate_memory_quantity(value: &str, field_name: &str) -> Result<(), Status> {
    openshell_core::quantity::validate_memory_quantity(value, field_name)
        .map_err(Status::invalid_argument)
}
    ...
    for section_name in ["limits", "requests"] {
        ...
        for resource_name in ["cpu", "memory"] {
            if section.fields.contains_key(resource_name) {
                return Err(Status::invalid_argument(format!(
                    "template.resources.{section_name}.{resource_name} is no longer supported; use spec.resource_requirements.{resource_name}.request or .limit"
                )));
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn validate_sandbox_spec_accepts_request_only_cpu_and_memory() {
        let spec = SandboxSpec {
            resource_requirements: Some(openshell_core::proto::ResourceRequirements {
                gpu: None,
                cpu: Some(openshell_core::proto::CpuResourceRequirements {
    ...
    #[test]
    fn validate_sandbox_spec_rejects_invalid_memory_request() {
        let spec = SandboxSpec {
            resource_requirements: Some(openshell_core::proto::ResourceRequirements {
                gpu: None,
                cpu: None,
        ...
        assert!(
            err.message()
                .contains("spec.resource_requirements.memory.request")
        );
    }
    ...
    fn validate_sandbox_spec_rejects_legacy_template_cpu_memory_resources() {
        ...
        assert!(
            err.message()
                .contains("spec.resource_requirements.cpu.request or .limit")
        );
    }
