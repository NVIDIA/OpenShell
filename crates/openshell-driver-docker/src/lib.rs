...
fn docker_resource_limits(
    ...
    if resources
        .cpu
        .as_ref()
        .is_some_and(|cpu| cpu.request.is_some())
    {
        return Err(Status::failed_precondition(
            "docker does not support cpu.request",
        ));
    }
    if resources
        .memory
        .as_ref()
        .is_some_and(|memory| memory.request.is_some())
    {
        return Err(Status::failed_precondition(
            "docker does not support memory.request",
        ));
    }
    ...
    Ok(DockerResourceLimits {
        nano_cpus: parse_cpu_limit(
            resources
                .cpu
                .as_ref()
                .and_then(|cpu| cpu.limit.as_deref())
                .unwrap_or(""),
        )?,
        memory_bytes: parse_memory_limit(
            resources
                .memory
                .as_ref()
                .and_then(|memory| memory.limit.as_deref())
                .unwrap_or(""),
        )?,
    })
}
...
fn parse_cpu_limit(value: &str) -> Result<Option<i64>, Status> {
    ...
    if let Some(millicores) = value.strip_suffix('m') {
        let millicores = millicores.parse::<i64>().map_err(|_| {
            Status::failed_precondition(format!(
                "invalid docker cpu.limit '{value}'; expected an integer or millicore quantity",
            ))
        })?;
        if millicores <= 0 {
            return Err(Status::failed_precondition(
                "docker cpu.limit must be greater than zero",
            ));
        }
...
fn parse_memory_limit(value: &str) -> Result<Option<i64>, Status> {
    ...
    if !amount.is_finite() || amount <= 0.0 {
        return Err(Status::failed_precondition(
            "docker memory.limit must be greater than zero",
        ));
    }
    ...
    let multiplier = match suffix {
        ...
        _ => {
            return Err(Status::failed_precondition(format!(
                "invalid docker memory.limit suffix '{suffix}'",
            )));
        }
    };
