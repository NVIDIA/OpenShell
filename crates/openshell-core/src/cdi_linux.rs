...
pub fn resolve_cdi_context(context: &CdiContext) -> Result<CdiDerivedRequirements, CdiError> {
    ...
    accumulate_requirements(&edits, &mut accumulator)?;
    Ok(accumulator.build())
}

pub fn validate_cdi_requirements<S: BuildHasher>(
    requirements: &CdiDerivedRequirements,
    writable_file_allowlist: &HashSet<String, S>,
) -> Result<(), CdiError> {
    validate_cdi_requirements_with_path_kind(
        requirements,
        writable_file_allowlist,
        filesystem_path_kind,
    )
}

fn validate_cdi_requirements_with_path_kind<F, S>(
    requirements: &CdiDerivedRequirements,
    writable_file_allowlist: &HashSet<String, S>,
    path_kind: F,
) -> Result<(), CdiError>
...
mod tests {
    ...
    fn resolve_with_kind(
        ...
        let requirements = resolve_cdi_context(context)?;
        validate_cdi_requirements_with_path_kind(&requirements, &writable_file_allowlist, kind)?;
        Ok(requirements)
    }
    ...
    struct CdiRequirementsBaseline<'a> {
    ...
    fn assert_cdi_spec_matches_derived_requirements_baseline(spec: &str, expected: &str) {
    ...
    #[test]
    fn orin_cdi_spec_matches_derived_requirements_baseline() {
        assert_cdi_spec_matches_derived_requirements_baseline(
    ...
    #[test]
    fn spark_cdi_spec_matches_derived_requirements_baseline() {
        assert_cdi_spec_matches_derived_requirements_baseline(
    ...
    #[test]
    fn multi_gpu_cdi_spec_matches_derived_requirements_baseline() {
        assert_cdi_spec_matches_derived_requirements_baseline(
            include_str!("../testdata/multi-gpu-nvidia.yaml"),
            include_str!("../testdata/multi-gpu-nvidia-policy-baseline.json"),
        );
    }
...
"#,
        ...
        assert_eq!(requirements.read_only_paths, vec!["/usr/local/cuda/lib64"]);
    }
...
"#,
        ...
        assert_eq!(
            ...
            vec![
                ...
                "/etc/nvidia/runtime.json",
                "/opt/nvidia/debug/libunsupported.so.1beta",
                "/opt/nvidia/debug/libunsupported.so.debug",
                "/opt/nvidia/encode",
                "/opt/nvidia/lib",
