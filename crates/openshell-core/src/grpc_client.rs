...
//!    `RegisterSupervisor` once at startup.
//!
...
use crate::proto::{
    ...
    GetSandboxProviderEnvironmentRequest, NetworkActivitySummary, PolicyChunk, PolicySource,
    PolicyStatus, RefreshSandboxTokenRequest, RegisterSupervisorRequest, ReportPolicyStatusRequest,
    SandboxPolicy as ProtoSandboxPolicy, SubmitPolicyAnalysisRequest, SubmitPolicyAnalysisResponse,
    SupervisorActivationMessage, UpdateConfigRequest, inference_client::InferenceClient,
...
/// bootstrap path, which uses `plain_channel` to call `RegisterSupervisor`
...
async fn acquire_sandbox_token(endpoint: &str, plain_channel: &Channel) -> Result<AcquiredToken> {
    ...
    {
        return Ok(AcquiredToken {
            token: acquire_supervisor_activation(endpoint, plain_channel, &sa_path)
                .await?
                .token,
...
async fn acquire_k8s_sandbox_token(
    ...
    Ok(
        acquire_supervisor_activation(endpoint, plain_channel, sa_path)
            .await?
            .token,
    )
}

async fn acquire_supervisor_activation(
    endpoint: &str,
    plain_channel: &Channel,
    sa_path: &str,
) -> Result<SupervisorActivationMessage> {
    let sa_token = std::fs::read_to_string(sa_path)
        .into_diagnostic()
        .wrap_err_with(|| format!("failed to read K8s SA token from {sa_path}"))?
        .trim()
        .to_string();
    info!(endpoint = %endpoint, "registering supervisor for sandbox activation");
    ...
    let resp = client
        .register_supervisor(RegisterSupervisorRequest {})
        .await
        .into_diagnostic()
        .wrap_err("RegisterSupervisor bootstrap stream failed")?;
    let mut stream = resp.into_inner();
    let activation = stream
        .message()
        .await
        .into_diagnostic()
        .wrap_err("RegisterSupervisor activation stream failed")?
        .ok_or_else(|| miette::miette!("RegisterSupervisor stream closed before activation"))?;
    info!(
        sandbox_id = %activation.sandbox_id,
        sandbox_name = %activation.sandbox_name,
        "received supervisor activation"
    );
    Ok(activation)
}

/// Register a supervisor instance and install the activated gateway JWT.
///
/// Warm instances call this before policy loading. The registration stream remains
/// pending until the Kubernetes driver observes a claim binding and the gateway
/// sends activation.
pub async fn register_supervisor(endpoint: &str) -> Result<SupervisorActivationMessage> {
    let sa_path = std::env::var(sandbox_env::K8S_SA_TOKEN_FILE)
        .ok()
        .filter(|path| !path.is_empty())
        .ok_or_else(|| {
            miette::miette!(
                "{} must be set to register supervisor",
                sandbox_env::K8S_SA_TOKEN_FILE
            )
        })?;
        ...
        let activation = {
            ...
            if TOKEN_SLOT.get().is_some() {
                return Err(miette::miette!(
                    "supervisor registration was requested after sandbox token initialization"
                ));
            }

            match async {
                let plain_channel = build_plain_channel(endpoint).await?;
                acquire_supervisor_activation(endpoint, &plain_channel, &sa_path).await
            }
            .await
            {
                ...
                Err(err) => {
                    warn!(
                        ...
                        "supervisor registration failed; retrying: {err:#}"
                    );
                    drop(guard);
