...
- `raw` — direct access to the generated tonic clients for RPCs the curated
  surface doesn't yet cover (providers, policy, logs, settings, SSH, and
  forwarding).
...
The plain `raw_grpc` accessor does not refresh; it returns a client bound to the
current token. When a refresher is wired, use `raw_grpc_fresh` to refresh before
the call, and `force_refresh` to recover after a raw RPC returns
`Unauthenticated`.
