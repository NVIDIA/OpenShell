...
const SANDBOX_CLAIM_VERSION_V1ALPHA1: &str = "v1alpha1";
const SANDBOX_CLAIM_VERSIONS: &[&str] = &[
    SANDBOX_CLAIM_VERSION_V1BETA1,
    SANDBOX_CLAIM_VERSION_V1ALPHA1,
];
...
impl SandboxClaimActivationController {
            ...
            .await
        {
            ...
            Err(status) if status.code() == Code::NotFound => {
                debug!(
                    ...
                    sandbox_id = %request.sandbox_id,
                    instance_id = %request.instance_id,
                    "Warm-pool supervisor registration is not pending yet"
                );
            }
            Err(status) => {
                warn!(
    ...
    async fn supported_sandbox_claim_api(&self, client: Client) -> Result<SandboxClaimApi, String> {
        for version in SANDBOX_CLAIM_VERSIONS {
            let claim_api = self.sandbox_claim_api(client.clone(), version);
        ...
        Err(format!(
            "no supported SandboxClaim API version is available; tried {}",
            SANDBOX_CLAIM_VERSIONS.join(", ")
        ))
    }
...
fn parse_sandbox_claim(obj: &DynamicObject) -> Result<ParsedSandboxClaim, String> {
    Ok(ParsedSandboxClaim {
        ...
        warm_pool_name: string_at(&obj.data, &["spec", "warmPoolRef", "name"])
            .or_else(|| string_at(&obj.data, &["spec", "warmpool"])),
...
fn activation_request_from_claim_state(
    ...
    let sandbox_id = sandbox_cr
        ...
        .cloned()
        .ok_or_else(|| "Sandbox CR is missing OpenShell sandbox id label".to_string())?;
    let instance_id = pod
        .metadata
        .uid
        .as_ref()
...
mod tests {
    ...
    #[test]
    fn parses_v1alpha1_sandbox_claim_status() {
        let obj = dynamic_object(
            SANDBOX_CLAIM_GROUP,
            SANDBOX_CLAIM_VERSION_V1ALPHA1,
            SANDBOX_CLAIM_KIND,
            ...
            json!({
                "spec": {
                    "sandboxTemplateRef": {"name": "template-a"},
                    "warmpool": "pool-a"
                },
        ...
        assert_eq!(claim.warm_pool_name.as_deref(), Some("pool-a"));
        assert_eq!(claim.sandbox_template_name.as_deref(), Some("template-a"));
        assert_eq!(claim.sandbox_name.as_deref(), Some("sandbox-a"));
    }
