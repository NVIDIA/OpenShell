...
fn apply_workspace_persistence(
    ...
    if let Some(init_containers) = init_containers {
        ...
        // container runs the sandbox image itself — a minimal base such as
        // Alpine seeds with BusyBox tar, which rejects them and would fail the
        // init container, leaving the pod unable to start. The script probes
        // tar once and falls back to the previous flags when the extensions are
        // unavailable: such images keep seeding exactly as they do today rather
        // than regressing into a startup failure.
        //
    ...
    fn workspace_init_seeds_content_owned_by_the_sandbox_identity() {
        ...
        assert!(
            !script.contains(&format!(
                "tar -C {WORKSPACE_INIT_MOUNT_PATH} --no-same-owner"
            )),
            "extraction must not discard ownership unconditionally — that is the \
