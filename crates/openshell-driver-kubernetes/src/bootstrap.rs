...
//! `openshell.ai/sandbox-id` annotation, verifies the pod is controlled by the
...
pub const SANDBOX_ID_ANNOTATION: &str = "openshell.ai/sandbox-id";
...
impl LiveK8sResolver {
    ...
    fn sandboxes_apis(&self, namespace: &str) -> [Api<DynamicObject>; 2] {
    ...
    async fn get_sandbox_cr_for_owner(
        ...
        let [sandboxes_api_v1beta1, sandboxes_api_v1alpha1] = self.sandboxes_apis(namespace);
    ...
    async fn resolve(&self, token: &str) -> Result<Option<SupervisorBootstrapIdentity>, Status> {
        ...
        if !accepts_auth_namespace(
            ...
            warn!(
                ...
                "K8s TokenReview principal namespace is not accepted for supervisor bootstrap"
            );
            return Err(Status::permission_denied(
                "SA token namespace is not accepted by the driver",
            ));
        }
...
fn token_review_identity(
    ...
    let rest = username
        ...
        .ok_or_else(|| Status::permission_denied("credential is not a service account"))?;
    let (namespace, service_account) = rest
        .split_once(':')
        .filter(|(namespace, service_account)| !namespace.is_empty() && !service_account.is_empty())
        .ok_or_else(|| Status::permission_denied("credential has invalid service account"))?;
...
mod tests {
    ...
    fn token_review_identity_extracts_pod_binding() {
        ...
        let identity = token_review_identity(&status, "openshell-gateway", "default")
            .unwrap()
    ...
    fn token_review_identity_returns_none_when_not_authenticated() {
        ...
        assert!(
            token_review_identity(&status, "openshell-gateway", "default")
                .unwrap()
                .is_none()
        );
    }
    ...
    fn token_review_identity_requires_expected_audience() {
        ...
        let err = token_review_identity(&status, "openshell-gateway", "default")
            .expect_err("wrong audience must fail closed");
        assert_eq!(err.code(), tonic::Code::Unauthenticated);
    }

    #[test]
    fn token_review_identity_accepts_namespace_for_policy_validation() {
        let status = token_review_status(
            true,
            vec!["openshell-gateway"],
        ...
        let identity = token_review_identity(&status, "openshell-gateway", "default")
            .unwrap()
            .expect("namespace should be parsed for caller validation");
        assert_eq!(identity.namespace, "other");
    }
    ...
    #[test]
    fn bootstrap_namespace_policy_matches_workspace_modes() {
        ...
        config.workspace_mode = crate::config::WorkspaceMode::Managed;
        config.gateway_id = "gw".to_string();
        assert!(accepts_auth_namespace(&config, None, "openshell-gw-team-a"));
        assert!(!accepts_auth_namespace(
            &config,
            None,
            "openshell-other-team-a"
        ));

        config.workspace_mode = crate::config::WorkspaceMode::Operator;
    ...
    async fn provider_accepts_warm_pending_identity() {
        let resolved = bootstrap_identity(SupervisorBootstrapBinding::WarmPending {
            activation_guard: "cr-uid-a".to_string(),
        });
        let fake = Arc::new(FakeResolver::returning(Ok(Some(resolved.clone()))));
        let provider = KubernetesSupervisorBootstrapIdentityProvider::new(fake);
