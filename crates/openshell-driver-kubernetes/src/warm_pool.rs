    ...
    pub async fn claim_warm_pair(
        ...
        for _ in 0..4 {
            ...
            if annotation_or_label(&object, LABEL_SANDBOX_ID).as_deref() == Some(&sandbox.id) {
                let supervisor_uid =
                    annotation_or_label(&object, ANNOTATION_SANDBOX_RUNTIME_SUPERVISOR_UID)
                        .ok_or_else(|| {
                            tonic::Status::aborted("warm pair has no supervisor identity")
                        })?;
                self.schedule_pair_metadata(object);
                return Ok(super::kubernetes_runtime_identity(
                    &namespace,
                    &candidate.uid,
                    &supervisor_uid,
                ));
                ...
                .await
            {
                Ok(claimed) => {
                    let supervisor_uid =
                        annotation_or_label(&claimed, ANNOTATION_SANDBOX_RUNTIME_SUPERVISOR_UID)
                            .ok_or_else(|| {
                                tonic::Status::aborted("warm pair has no supervisor identity")
                            })?;
...
mod tests {
    ...
    async fn creation_uses_prepared_authentication_generation_for_metadata_and_secret_names() {
        ...
        let error = Box::pin(driver.create_sandbox_inner(&sandbox, Some(&preparation)))
            .await
            .unwrap_err();
        let requests = requests.lock().unwrap();
        assert_eq!(
    ...
    async fn claim_fixture(
        ...
        let object = serde_json::json!({"apiVersion":"agents.x-k8s.io/v1alpha1","kind":"Sandbox","metadata":{
            ...
            "annotations":{PAIR:"physical",FINGERPRINT:fingerprint, ANNOTATION_SANDBOX_RUNTIME_GENERATION:"generation",ANNOTATION_SANDBOX_RUNTIME_SUPERVISOR_UID:"supervisor-uid",REGISTERED:openshell_core::time::now_ms().to_string(),ANNOTATION_SANDBOX_RUNTIME_BOOTSTRAP_STARTED_AT:openshell_core::time::now_ms().to_string()}
        }});
