...
impl KubernetesComputeConfig {
    ...
    pub fn validate_service_account_names(&self) -> Result<(), String> {
        ...
        for (setting, names) in lists {
            for raw in names.iter().filter(|name| !name.is_empty()) {
                if raw != raw.trim() {
                    return Err(format!(
                        "{setting} entry '{raw}' has leading or trailing whitespace; \
                         a ServiceAccount name never does"
                    ));
                }
            }
...
mod tests {
    ...
    /// A padded config entry is a mistake worth failing on: the accepted set
    /// and the resolved pod name are both trimmed, so the untrimmed spelling
    /// would only ever appear in the `ServiceAccount` the driver provisions.
    #[test]
    fn validate_service_account_names_rejects_untrimmed_entries() {
        for cfg in [
            config_with_service_accounts(" openshell-sandbox ", &[], &[]),
            config_with_service_accounts("openshell-sandbox", [" padded "].as_slice(), &[]),
            config_with_service_accounts("openshell-sandbox", &[], ["padded\t"].as_slice()),
        ] {
