...
mod tests {
    ...
    fn delegated_credential_commands_parse() {
        ...
        match list.command {
            Some(Commands::DelegatedCredential {
                command:
                    Some(DelegatedCredentialCommands::List {
                        page_size,
                        page_token,
                        ids,
                        output,
                ...
                assert_eq!(page_token, "next-credential-page");
                assert!(ids);
                assert_eq!(output, OutputFormat::Table);
            }
            command => panic!("unexpected delegated credential list command: {command:?}"),
        }
