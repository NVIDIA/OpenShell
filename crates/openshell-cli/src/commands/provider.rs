...
pub async fn provider_list_profiles_text(
    ...
    loop {
        ...
        // The continuation token determines completion; a short page can still
        // have more results. Preserve every scope when profiles share a name.
        if response.next_page_token.is_empty() {
            break;
        }
        page_token = response.next_page_token;
...
fn format_provider_profile_details(profile: &ProviderProfile) -> String {
    ...
    for endpoint in &profile.endpoints {
        ...
        let tls = match endpoint.tls.as_str() {
            "" => "auto (default)",
            "skip" => "skip (raw tunnel; no L7 inspection or credential rewrite)",
            "terminate" => "terminate (automatic TLS detection)",
            "passthrough" => "passthrough (automatic TLS detection)",
            tls => tls,
        };
        ...
        } else if !endpoint.rules.is_empty() {
            "custom rules"
        } else if is_mcp && allow_all_known_mcp_methods == Some(true) {
            "all known MCP methods (subject to tool and deny rules)"
        } else {
            "not specified"
        };
        ...
        let enforcement = if endpoint.enforcement.is_empty() {
            "audit (default)"
        } else {
...
mod tests {
    ...
    fn profile_description_keeps_credential_values_out_of_human_output() {
        ...
        let mut profile = openshell_providers::builtin_profiles()
            .iter()
            .find(|profile| profile.id == "github")
            .expect("the built-in catalog contains GitHub")
            .to_proto();
        profile.source = "builtin".to_string();
        ...
        assert!(rendered.contains("Source: builtin\nScope: platform\n"));
