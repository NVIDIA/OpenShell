...
use openshell_ocsf::{
    ActivityId, AppLifecycleBuilder, ConfigStateChangeBuilder, EventContext, EventOrigin,
...
#[test]
fn linux_supervisor_events_report_linux_os() {
    let json = AppLifecycleBuilder::new(&sandbox_ctx())
        .activity(ActivityId::Open)
        .build()
        .to_json()
        .unwrap();
