...
fn main() {
    ...
    let result = match args.get(1).map(String::as_str) {
        ...
        Some("list-sandbox-namespaces") => openshell_cni::list_sandbox_namespaces(),
        Some("set-node-coverage") => openshell_cni::set_node_coverage(&args[2..]),
        Some("wait-coverage") => openshell_cni::wait_coverage(&args[2..]),
