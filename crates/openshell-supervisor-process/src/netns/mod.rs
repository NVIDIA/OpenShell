...
impl NetworkNamespace {
    ...
    ///
    /// See [`bind_tcp_in_netns_fd`] for the mechanism. Returns `Err` if the
    /// namespace has no fd, the address does not parse, `setns` fails, or bind
    /// fails.
    pub async fn bind_tcp_in_netns(&self, addr: &str) -> std::io::Result<tokio::net::TcpListener> {
        ...
        let addr: std::net::SocketAddr = addr
            .parse()
            .map_err(|e| std::io::Error::other(format!("invalid bind address {addr}: {e}")))?;
        bind_tcp_in_netns_fd(ns_fd, addr).await
    }
}

/// Bind a TCP listener inside the network namespace referenced by `ns_fd`.
///
...
///
/// Returns `Err` if `setns` fails or bind fails.
pub async fn bind_tcp_in_netns_fd(
    ns_fd: RawFd,
