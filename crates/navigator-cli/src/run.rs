...
pub async fn cluster_inference_set(
    ...
    let label = if configured.route_name == "sandbox-system" {
        "Sandbox system inference configured:"
    } else {
        "Cluster inference configured:"
    };
...
pub async fn cluster_inference_update(
    ...
    let configured = response.into_inner();
    let label = if configured.route_name == "sandbox-system" {
        "Sandbox system inference updated:"
    } else {
        "Cluster inference updated:"
    };
    println!("{}", label.cyan().bold());
    println!();
    println!("  {} {}", "Route:".dimmed(), configured.route_name);
...
pub async fn cluster_inference_get(
    server: &str,
    ...
    if let Some(name) = route_name {
        // Show a single route (--sandbox was specified).
        let response = client
            .get_cluster_inference(GetClusterInferenceRequest {
        ...
        print_inference_route(&mut client, "Cluster inference", "").await;
        println!();
        print_inference_route(&mut client, "Sandbox system inference", "sandbox-system").await;
    }
    Ok(())
}
