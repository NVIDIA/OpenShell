...
OpenShell uses the [Kubernetes Gateway API](https://gateway-api.sigs.k8s.io) for ingress. The chart creates a `GRPCRoute` that routes inbound gRPC traffic to the gateway pod. You need a Gateway API implementation installed on your cluster to fulfill the `GRPCRoute`. OpenShell tests this integration with [agentgateway](https://agentgateway.dev) and [Envoy Gateway](https://gateway.envoyproxy.io). The installation examples below pin known-good versions.

## Install agentgateway

Install the standard Gateway API resources, then install the pinned agentgateway charts:

```shell
export OPENSHELL_GATEWAY_API_VERSION=v1.6.2
export OPENSHELL_AGENTGATEWAY_VERSION=v1.5.0

kubectl apply --server-side -f "https://github.com/kubernetes-sigs/gateway-api/releases/download/${OPENSHELL_GATEWAY_API_VERSION}/standard-install.yaml"

helm upgrade --install agentgateway-crds \
  oci://cr.agentgateway.dev/charts/agentgateway-crds \
  --version "${OPENSHELL_AGENTGATEWAY_VERSION}" \
...
With agentgateway, an [`AgentgatewayPolicy`](https://agentgateway.dev/docs/kubernetes/latest/documentation/security/jwt/setup/) can validate the CLI's bearer token without a browser redirect. Set `spec.traffic.jwtAuthentication.preserveToken` to `true` so the token is forwarded to OpenShell for validation; agentgateway removes validated tokens by default.
