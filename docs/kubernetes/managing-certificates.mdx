...
You do **not** need to set `gatewayConfig.openshell.drivers.kubernetes.grpc_endpoint` to the external hostname.
Supervisors connect via the internal service name automatically. Setting
`grpc_endpoint` to an external hostname would cause supervisors to
