...
is set, and set `server.tls.clientCaSecretName` to a secret containing the
client CA — by default that's `certManager.caSecretName` (`openshell-ca-tls`),
the CA the chart issues the client certificate from.
