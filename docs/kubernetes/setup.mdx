...
![OpenShell on Kubernetes: the gateway provisions a Sandbox resource, separate supervisor and workload Pods, a boundary Service, NetworkPolicy, and a workspace PVC.](../images/openshell-kubernetes-runtime.svg)
...
| `podLifecycle.terminationGracePeriodSeconds` | Seconds Kubernetes waits before killing a stopping gateway pod. Defaults to `30` so gateways that use an external PostgreSQL database can move their supervisor sessions to other replicas. |
| `autoscaling.enabled` / `autoscaling.minReplicas` / `autoscaling.maxReplicas` | Render a HorizontalPodAutoscaler instead of a fixed `replicaCount`. Refer to [High Availability](/kubernetes/high-availability#scale-the-gateway). |
...
During shutdown, `/readyz` returns `503` with status `draining` while `/healthz` keeps returning `200`.
