...
The ClusterRole grants node inspection, token validation, namespace reads, and
warm-pool extension access. The chart grants `sandboxclaims` create/delete only
when `server.warmPooling.enabled=true`:
...
| `""` | `namespaces` | get |
| `extensions.agents.x-k8s.io` | `sandboxclaims` | get, list, watch; create, delete when warm pooling is enabled |
| `extensions.agents.x-k8s.io` | `sandboxtemplates`, `sandboxwarmpools` | get, list, watch; create, patch, update, delete when template reconciliation is enabled |
