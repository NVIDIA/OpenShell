...
| `supervisor.sidecar.proxyUid` | Non-root UID for the long-running network sidecar when `supervisor.topology` is `sidecar` or `network-sidecar`. The UID must not match the sandbox UID. |
...
- To choose between combined, sidecar, and network-sidecar sandbox pods, refer to [Topology](/kubernetes/topology).
