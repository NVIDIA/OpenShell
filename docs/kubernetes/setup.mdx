...
## Configure an HA database

The chart uses SQLite by default. SQLite is suitable for single-replica
evaluation, but HA gateway deployments need PostgreSQL so replicas share the
same persistence store. The OpenShell chart does not deploy PostgreSQL; install
or provision it separately, then create a Secret containing the connection URI:

```shell
kubectl create secret generic openshell-db -n openshell \
  --from-literal=uri="postgresql://user:pass@postgres.example.com:5432/openshell"
```

Reference that Secret when installing the chart:
...
helm upgrade --install openshell \
  ...
  --namespace openshell \
  --set replicaCount=2 \
  --set server.externalDbSecret=openshell-db
```
...
| `replicaCount` | Number of gateway replicas. Set `server.externalDbSecret` before running more than one replica. |
| `server.sandboxNamespace` | Namespace where sandbox pods are created. Defaults to the Helm release namespace when left empty. |
| `server.externalDbSecret` | Secret containing a PostgreSQL connection URI in the `uri` key. Required for HA. |
