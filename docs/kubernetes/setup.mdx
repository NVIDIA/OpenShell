...
| `cni.enabled` | Install the privileged OpenShell CNI DaemonSet (a per-cluster singleton). Enable in one release per cluster when `supervisor.topology=cni-sidecar`. |
| `cni.external` | Use a CNI singleton installed by another release instead of installing one here. Set on additional `cni-sidecar` gateway releases (with `cni.enabled=false`). |
...
- To choose between combined, sidecar, and cni-sidecar sandbox topology, refer to [Topology](/kubernetes/topology).
