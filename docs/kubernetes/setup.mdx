...
Apply the pinned release manifest and the extension CRDs:
...
This creates the `agent-sandbox-system` namespace, installs the CRDs (including the `extensions.agents.x-k8s.io` extension CRDs), and starts the controller.
