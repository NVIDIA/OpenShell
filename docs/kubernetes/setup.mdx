...
| `supervisor.topology` | Sandbox pod topology. Leave as `combined` for the original full-enforcement path, set `sidecar` when the agent container should run non-root without added Linux capabilities, or set `split-pod` to run network enforcement in a separate supervisor pod with NetworkPolicy isolation. Refer to [Topology](/kubernetes/topology). |
| `supervisor.sidecarProxyUid` | Non-root UID for the long-running network sidecar or split supervisor pod. The UID must not match the sandbox UID. |
...
- To choose between combined, sidecar, and split-pod sandbox topology, refer to [Topology](/kubernetes/topology).
