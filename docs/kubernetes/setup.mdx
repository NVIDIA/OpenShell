...
release namespace. The current gateway runtime still selects one
`server.sandboxNamespace`; mapping workspaces to several namespaces requires
the separate multi-namespace runtime feature.
