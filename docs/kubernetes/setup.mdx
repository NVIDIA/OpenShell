...
The gateway workload and the PKI bootstrap hooks satisfy the [Restricted Pod Security Standard](https://kubernetes.io/docs/concepts/security/pod-security-standards/), so the release installs into a namespace labeled `pod-security.kubernetes.io/enforce=restricted`.
