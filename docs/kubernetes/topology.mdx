...
pods) **and** whose namespace is in the plugin's `cni.sandboxNamespaces`
allowlist (default: the owner release's sandbox namespace). Pods in other
namespaces are passed through without an API lookup, so the allowlist bounds the
blast radius. Enable the installer (`cni.enabled=true`) in exactly one release
per cluster; additional gateway releases set `cni.enabled=false` and
`cni.external=true` to share it — and their sandbox namespace must be added to
the owner's `cni.sandboxNamespaces` first (see
[OpenShift](/kubernetes/openshift#enable-the-cni-sidecar-topology)). The installer
treats any `openshell-cni` chained entry as its own and upgrades it in place.
</Note>
