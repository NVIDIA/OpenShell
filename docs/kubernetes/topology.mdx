...
<Note>
The CNI installer is a **cluster singleton**. Its chained plugin enforces any
pod carrying the OpenShell annotations (set only by a gateway on its own sandbox
pods), so a single installation serves every OpenShell release across all
namespaces. Enable the installer (`cni.enabled=true`) in exactly one release per
cluster; additional gateway releases set `cni.enabled=false` and
`cni.external=true` to share it. The installer refuses to overwrite a chained
plugin entry with a non-OpenShell owner (fail closed).
</Note>
