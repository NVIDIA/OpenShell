...
| Network sidecar | UID 0 by default; `gatewayConfig.openshell.drivers.kubernetes.sidecar.proxy_uid` when binary-aware policy is disabled | Runs the proxy, enforces network policy, owns gateway authentication and the gateway session, and serves local policy/provider state over the sidecar control socket. |
...
`gatewayConfig.openshell.drivers.kubernetes.sidecar.process_binary_aware_network_policy=false` runs the sidecar as
...
`gatewayConfig.openshell.drivers.kubernetes.topology` unset or set it to `combined`.

Set `gatewayConfig.openshell.drivers.kubernetes.sidecar.process_binary_aware_network_policy=false` only when you
