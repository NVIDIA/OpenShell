...
/run/multus/cni/net.d/vendor-cni-chain`, a persistent `cni.stateDir`, and
`cni.openshift.privilegedSCC: true`. It requires a Multus thick daemon with
`auxiliaryCNIChainName` configured (default on OpenShift 4.x).
