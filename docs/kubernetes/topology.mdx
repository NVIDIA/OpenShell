...
description: "Choose between combined, sidecar, and split-pod supervisor topology for Kubernetes sandbox pods."
...
`sidecar`, or `split-pod` topology. Choose the topology based on which controls
...
| `split-pod` | You need network enforcement to run outside the agent pod and your cluster enforces Kubernetes NetworkPolicies. | Requires a NetworkPolicy-enforcing CNI or controller; process/filesystem enforcement stays network-only. |
...
| `sidecar` | Network supervisor sidecar | `sidecarProxyUid:sandbox_gid` | `false` | Drops `ALL` | Long-running proxy sidecar is also non-root without added capabilities. |
| `split-pod` | Agent pod container, process-only supervisor | `sandbox_uid:sandbox_gid` | `false` | Drops `ALL` | Agent and workload run without added Linux capabilities in their own pod. |
| `split-pod` | Supervisor pod container, network proxy only | `sidecarProxyUid:sandbox_gid` | `false` | Drops `ALL` | Long-running proxy runs outside the agent pod without added capabilities. |
...
| `split-pod` | Supervisor install init container | `0` | Not set | Not set | Copies the supervisor binary into the agent pod volume. |
| `split-pod` | Proxy CA install init container | `0:sandbox_gid` | `false` | Drops `ALL` | Copies proxy CA material into the agent pod TLS volume. |
...
Split-pod topology moves network enforcement and gateway forwarding into a
separate supervisor pod. The agent pod runs the process supervisor and reaches
the supervisor through a per-sandbox headless Service.

OpenShell creates these per-sandbox resources:

- Agent pod labeled `openshell.ai/sandbox-role=agent`.
- Supervisor pod labeled `openshell.ai/sandbox-role=supervisor`.
...
The supervisor pod has a controlling `Sandbox` ownerReference so Kubernetes
garbage collection removes the companion resources when the sandbox is deleted.

<Warning>
Split-pod topology requires NetworkPolicy enforcement. In local k3s/k3d testing,
keep the k3s network policy controller enabled or install a policy-enforcing CNI
before deploying the split-pod Skaffold profile. Without enforcement, the
agent-to-supervisor isolation policy is only declarative.
</Warning>

## Credential Exposure

Sidecar and split-pod topologies use pod `fsGroup` and group-readable projected
credentials so the non-root process supervisor can authenticate to the gateway.
This includes the projected ServiceAccount token used for sandbox token
bootstrap and the sandbox client TLS secret.
...
Set `supervisor.topology=split-pod` to use split-pod mode:
...
The same `sidecarProxyUid` value is used as the non-root UID for the split
supervisor pod and must not match the sandbox UID.
