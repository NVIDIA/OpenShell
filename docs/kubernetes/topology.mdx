...
| `proxy-pod` | Proxy CA install init container | `0:sandbox_gid` | `false` | Drops `ALL` | Uses the supervisor utility image to copy proxy CA material into the agent pod TLS volume. |
...
container. Consequently, proxy-pod topology provides network enforcement only:
