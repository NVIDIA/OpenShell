...
The singleton runs a single `openshell-cni` image. When gateways span multiple
OpenShell versions, keep the singleton at a version compatible with all of them;
the CNI-to-sandbox contract is not yet version-negotiated across releases.
