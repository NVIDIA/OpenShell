...
For a multi-gateway cluster, prefer installing the CNI as its own release with
`cni.only=true` (`-f deploy/helm/openshell/examples/cni-standalone.yaml`) so node
enforcement is not tied to any gateway's lifecycle; every gateway then runs
`cni.external=true`. See
[Running multiple gateways](/kubernetes/topology#running-multiple-gateways).
