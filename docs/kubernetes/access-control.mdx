...
The Helm chart generates a gateway TLS certificate and a user client certificate at install time. Sandbox Pods project only `ca.crt` from the client Secret so they can authenticate the gateway; the client certificate and private key are not exposed to sandboxes. Supervisors authenticate their RPCs with gateway-minted sandbox JWTs.
