...
Resource admission checks the requested external references before provisioning
and the actual workload Pod before releasing its scheduling gate. PVCs and
other supported external references require operator approval labels, including
same-namespace and read-only mounts. Unexpected references, changed resource
UIDs, and unsupported volume sources fail closed. GPU device attachments are
temporarily exempt. The driver rechecks persisted references on restart and
every 30 seconds during reconciliation; confirmed revocation suspends compute,
while transient lookup failures block new launches and recovery.
