...
6. Publish readiness only after the supervisor attaches, confirms enforcement,
   and registers the gateway relay. The supervisor accepts TCP connections on
   port 5501 only while its gateway session is ready, and the supervisor Pod's
   `tcpSocket` readiness probe checks that port.
...
- Kubelet reachability to supervisor Pods on TCP port 5501 for readiness probes.
  If you add a default-deny ingress policy to sandbox namespaces, allow that
  port from the node.
