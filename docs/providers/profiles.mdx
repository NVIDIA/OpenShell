...
The built-in `openai` and `anthropic` profiles use their public vendor
endpoints. Configuring an alternate base URL disables their fixed-vendor policy
and credential binding. Import an endpoint-bearing custom profile when a
sandbox needs direct access to an alternate upstream.
...
`inference_capable` identifies profiles intended for inference workflows. It is
descriptive metadata; attached providers use their declared native endpoints,
credentials, and binaries like any other profile.
