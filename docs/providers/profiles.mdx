...
- A user-managed profile shadows a built-in profile with the same ID in its applicable workspace or platform scope. The built-in remains the fallback outside that scope. Source-managed interceptor collisions still fail closed.
...
The built-in `openai` and `anthropic` profiles bind credentials to their public
vendor endpoints. To use a proxy or compatible API at another host, import a
custom profile that declares that host and attach a provider created from that
profile. OpenShell never broadens a built-in profile's credential boundary from
a base URL environment variable alone.
...
Use [Provider-backed Inference](/sandboxes/inference-routing) to attach an
...
Built-in provider profiles currently include:

| Profile ID | Category | Credential environment variables |
|---|---|---|
| `anthropic` | `inference` | `ANTHROPIC_API_KEY` |
...
For `token_exchange` profiles, choose the subject token source explicitly.
OpenShell trims surrounding whitespace from `subject_token.source` and
`subject_token.credential` before using them.
Use `source: provider_credential` when the provider record should hold a subject
credential named by `token_grant.subject_token.credential`. That credential is
gateway-only: the sandbox does not receive it as environment material, static
credential binding metadata, or workload placeholder ownership. Create or update
that provider credential from the current gateway OIDC login with `--from-oidc-token`.
This requires an active named gateway that was registered for OIDC. The CLI
copies the current OIDC access token and its expiry into the provider. If the
stored gateway access token is expired and a refresh token is available, the CLI
refreshes it first. OpenShell does not store the OIDC refresh token in the
provider. When the stored subject-token credential expires, the gateway rejects
intermediate token exchange until the provider is updated with a fresh token.
...
<Warning>
Delegate identity only through providers whose configuration administrators you
trust with your delegated access token. The gateway loads the current profile
for each exchange; creating a delegation does not pin its exchange configuration.
Workspace Admins can change workspace-scoped custom profiles, including
`token_grant.token_endpoint`, so they can redirect a subsequent exchange to an
endpoint they control. The gateway sends the delegator's raw access token as the
subject token to that endpoint, not the delegated refresh token or normal CLI
login credentials. The provider-credential subject-token path also trusts profile
administrators with its subject token. Delegated-sandbox ownership restricts
direct lifecycle and interactive access, but does not protect subject tokens from
administrators who control exchange configuration.
</Warning>

Use `source: sandbox_delegated_identity` when the subject token should be the
creating user's delegated OIDC identity. Create the provider with
`--runtime-credentials`, then create sandboxes with an explicit authorization
duration:

```shell
openshell provider create \
  --name protected-services \
  --type protected-services-profile \
  --runtime-credentials

openshell sandbox create \
  --provider protected-services \
  --use-my-identity-for=8h
```

Delegation must be enabled when the sandbox is created. A sandbox created
without `--use-my-identity-for` cannot add delegated identity later, although
the original delegator can extend or withdraw an existing delegation:

```shell
openshell sandbox delegated-identity status my-sandbox
openshell sandbox delegated-identity extend my-sandbox --for=24h
openshell sandbox delegated-identity withdraw my-sandbox
```

The gateway measures the requested duration from its own clock when it handles
create or extend and enforces its configured maximum delegation duration.

The status command reports `active`, `withdrawn`, `expired`, `revoked`, or
`credential-missing`. `revoked` means the sandbox delegation window still
exists, but the gateway-scoped delegated credential no longer authorizes token
exchange.

The first create or extend that needs delegated identity checks whether the
authenticated user already has a usable gateway-owned authorization. If not,
the CLI starts another Authorization Code flow with PKCE, opens the browser, and
requests only `openid` and `offline_access`. The identity provider may reuse the
existing browser SSO session. This authorization is separate from
`openshell gateway login`: the CLI transfers its access and refresh tokens to
the gateway without writing them to the local `oidc_token.json`, and the normal
CLI login remains the bearer for every gateway RPC. Later sandboxes reuse the
gateway-owned authorization without another prompt while it remains usable.

Configure the gateway OIDC client to allow the Authorization Code flow, the
CLI's loopback callback URI, and refresh tokens for `offline_access`. The IdP
must create a distinct grant and refresh-token chain for the second browser
authorization; rotating or single-use refresh tokens are supported. If the IdP
returns the normal CLI login's refresh token again, OpenShell rejects the
transfer to prevent two independent owners from rotating the same token. An IdP
may bind offline access to its login session, so ending that session can still
invalidate the delegated grant.

If refresh fails because the delegated grant is revoked or no longer active,
retry `openshell sandbox create --use-my-identity-for=...` or
`openshell sandbox delegated-identity extend ... --for=...`. The status
preflight starts a new browser authorization and replaces the unusable
gateway-owned credential. Running `openshell gateway logout` or
`openshell gateway login` changes only the normal CLI login and does not repair
or revoke the delegated grant.

Platform admins can inspect and revoke the gateway-scoped delegated credential
records:

```shell
openshell delegated-credential list
openshell delegated-credential status <credential-id>
openshell delegated-credential revoke <credential-id>
openshell delegated-credential delete <credential-id>
```

The list command returns one continuation-token page. Use `--page-size` to
control the page size and pass a non-empty `next_page_token` back through
`--page-token`. JSON and YAML output include both the `credentials` collection
and `next_page_token`; table output reports the token on stderr.

The gateway records failed OAuth material deletions for background retry. It
retries concurrent-update conflicts while recording staged material for cleanup.
If the credential record disappears or the gateway cannot persist the cleanup
record, it logs a cleanup warning; automatic cleanup cannot recover unrecorded
material.

Gateway replicas coordinate delegated credential refresh with a renewable lease.
A live owner renews through storage and commit, so a slow refresh does not lose
ownership just because its initial lease duration elapses. An expired lease lets
another replica recover after the owner fails. A crash or network partition after
the IdP rotates a refresh token but before the gateway commits it can still
require reauthorization.

Withdrawal is not instantaneous containment. The gateway rejects requests when
their authorization checks observe delegation expiry, withdrawal, or credential
...
minutes from insertion; this is not a five-minute deadline measured from
withdrawal. Stop or delete the sandbox when continued execution is unacceptable.
You can also revoke the upstream IdP grant, but an already issued access token
remains usable until its resource server rejects it or it expires unless that
system enforces immediate revocation.

These controls have different scopes: local gateway logout removes only the
CLI-owned login, sandbox withdrawal ends one sandbox's delegation window,
gateway credential revocation disables all sandboxes that reference that user's
gateway-owned grant, and IdP revocation invalidates the upstream OAuth grant.
...
| `cache_ttl_seconds` | No | Token cache TTL override. When omitted or `0`, OpenShell uses the token response `expires_in` with a 30-second safety margin and one-hour cap, or five minutes minus the margin if the response does not include an expiry. For `token_exchange`, OpenShell caps every cache entry at five minutes, including positive overrides; response-derived entries cache for at most four minutes and 30 seconds after the margin. `client_credentials` retains the general one-hour response cap and uncapped positive overrides. |
...
back to the retired provider registry. The built-in `openai` profile discovers
`OPENAI_API_KEY`; the built-in `anthropic` profile discovers
`ANTHROPIC_API_KEY`.
