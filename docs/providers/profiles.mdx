...
endpoints. Configuring an alternate base URL makes those providers route-only:
OpenShell withholds their static credentials and fixed-vendor policy from
direct sandbox traffic while gateway inference routing continues to use the
configured upstream.
