...
Withdrawing a sandbox delegation or revoking its gateway credential stops new
gateway exchanges immediately. A token already cached by the sandbox
supervisor can continue to be injected for no more than five minutes. Stop or
delete the sandbox when containment cannot wait for that cache window. You can
also revoke the upstream IdP grant, but an already issued access token remains
usable until its resource server rejects it or it expires unless that system
enforces immediate revocation.
