...
OpenShell emits OCSF v1.7.0 events internally, but many SIEMs only support older schema versions. The `ocsf_schema_version` setting tells the JSONL layer to downgrade events before writing, stripping fields and profiles that don't exist in the target version.
...
The setting can also be configured in `gateway.toml`:
...
When set, the JSONL layer applies the following transformations:
...
The shorthand log output is unaffected. The internal event model stays at v1.7.0; only the serialized JSONL is transformed.
...
| CrowdStrike FDR | v1.5.0 | `1.5` |
| Datadog Cloud SIEM | v1.5.0 (selectable) | `1.5` if needed |

<Note>
The core OCSF event structure (class UIDs, activity IDs, HTTP/network fields) is identical across v1.1 through v1.7. The differences are all profile-gated additions, so downgrading does not lose security-relevant data.
</Note>
...
|---|---|
| Splunk | Use the [Splunk OCSF Add-on](https://splunkbase.splunk.com/app/6943) to ingest OCSF JSONL files. Set `ocsf_schema_version = "1.3"` for CIM Add-On compatibility. |
| Amazon Security Lake | OCSF is the native schema for Security Lake. Set `ocsf_schema_version = "1.1"` for v1.1.0 compatibility. |
| CrowdStrike FDR | Ship JSONL files to Falcon Data Replicator. Set `ocsf_schema_version = "1.5"` for v1.5.0 compatibility. |
