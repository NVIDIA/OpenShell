...
Unix socket relay and relay-control notifications without a network endpoint
use Base Event (`class_uid: 0`). Include these classes when filtering supervisor
operational events. Relay-control records that previously appeared as `NET:*`
now appear as `EVENT`.
