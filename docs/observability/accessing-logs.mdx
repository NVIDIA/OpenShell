...
A cursor is one position shared by the log and platform sources; you track a single highest value, not one per source. If you request both logs and platform events through the SDK and set how far back each replays independently, an asymmetric depth can reduce how much either source delivers on connect — the gateway withholds events rather than hand out a cursor that one of the two sources cannot fully vouch for. `openshell logs` doesn't expose this dial and isn't affected; it applies to SDK callers who set both replay depths directly.
