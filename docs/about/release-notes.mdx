...
A stored global or sandbox policy that carries a removed `tls` value fails validation on read, so `GetSandboxConfig` returns `FAILED_PRECONDITION` and affected sandboxes cannot start. Run `openshell policy list --global` to see which revision is invalid and why, then re-apply a corrected policy with `openshell policy set --global`.
