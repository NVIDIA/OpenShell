...
against Anthropic. Review the
[example profile](https://github.com/NVIDIA/OpenShell/blob/main/providers/anthropic.yaml),
which allows `/usr/local/bin/opencode` to reach `api.anthropic.com`, then import
...
openshell profile import \
  --url https://raw.githubusercontent.com/NVIDIA/OpenShell/main/providers/anthropic.yaml
...
value in the credential store and sends it only to `api.anthropic.com`.
