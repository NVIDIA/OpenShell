...
<Note>
OpenShell 0.1.0 has landed. It brings:

- A smaller default workload image with a stricter fallback policy.
- Explicit provider profiles and provider attachment.
- Version 2 gateway configuration and the Helm `gatewayConfig` interface.
- Safer API contracts for workspaces, retries, pagination, exec sessions, and resumable watch streams.
