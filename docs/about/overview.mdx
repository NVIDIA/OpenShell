...
| Unauthorized API usage | Agent sends prompts or data to unapproved model providers. | Provider profiles and network policies control where inference traffic can go. |
...
| Credential binding | Keeps provider secrets outside the agent process and limits their use to approved endpoints. | Applied to attached providers. |
...
| Private enterprise development | Use provider profiles for self-hosted or private inference backends while keeping sensitive context under your control. |
