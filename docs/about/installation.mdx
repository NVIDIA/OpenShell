...
OpenShell supports several local compute drivers. Package-managed gateway configs leave the driver unset by default so the gateway can auto-detect an available driver. Set `compute_drivers` in the gateway TOML when you need to pin a specific driver.
...
The Homebrew service listens on `https://127.0.0.1:17670` and generates a local mTLS bundle on install. The service reads `~/.config/openshell/gateway.toml` when that file exists and otherwise uses the Homebrew-managed config under the Homebrew prefix, such as `/opt/homebrew/var/openshell/gateway.toml`.
...
The Linux user service listens on `https://127.0.0.1:17670`, writes gateway settings to `~/.config/openshell/gateway.toml`, and generates a local mTLS bundle before the gateway starts.
