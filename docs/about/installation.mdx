...
To remain on a specific release train, set `OPENSHELL_VERSION` to its rolling channel, such as `pre-0.1.0`. The channel advances only for a candidate that passes release validation, then post-publication canaries exercise the supported installation paths. The installer downloads only the checksums and packages required for your platform, then installs with Debian, RPM, or Homebrew. Installed packages retain the candidate's exact version, such as `0.1.0-pre.3`.
