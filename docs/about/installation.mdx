...
The snap stores optional gateway overrides in `$SNAP_COMMON/gateway.toml`, usually `/var/snap/openshell/common/gateway.toml`. The gateway service uses the VM compute driver by default.
