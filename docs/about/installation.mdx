...
without `snapd` (or with `OPENSHELL_INSTALL_METHOD=classic` set), the script
falls back to the classic package manager.
...
`log-observe`, and `system-observe` — still require manual connection:
