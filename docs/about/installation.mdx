...
Explicit image fields in the selected Docker, Podman, or Kubernetes TOML table
take precedence. Restart the user service after editing the file. Treat these
variables as trusted operator configuration; sandbox requests cannot set them,
and registry credentials must remain in the container runtime's credential
store or Kubernetes image-pull secrets.
