...
Tagged prereleases are stored as GitHub Actions artifacts for 90 days and require an authenticated [GitHub CLI](https://cli.github.com/) session:
...
The `pre` alias selects the highest prerelease version with a successful release workflow. To install a specific prerelease instead, set `OPENSHELL_VERSION` to its full tag, such as `v0.1.0-pre.1`. The installer downloads the release bundle, verifies the package checksums, and installs with Debian, RPM, or Homebrew for your platform.
