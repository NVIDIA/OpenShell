...
The script detects your operating system and installs the OpenShell CLI, standalone policy prover, and gateway with your native package manager. On Linux, the snap path is preferred when `snapd` is available; otherwise, the script uses the DEB or RPM package. It then starts the local gateway server so you can begin creating sandboxes.
...
described below, provided no non-snap Docker is detected. On hosts without
`snapd`, or if a native Docker package is present, the installer silently falls
back to DEB or RPM.
