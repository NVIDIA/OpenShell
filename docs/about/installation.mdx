...
You can run the gateway without Homebrew, RPM, Debian packages, or Helm by downloading the release binary and starting `openshell-gateway` yourself. This is useful on immutable or image-based systems where package-manager installation is not the right fit.

Choose the gateway release artifact that matches your host:
...
Set `VERSION` to a release tag from [OpenShell GitHub Releases](https://github.com/NVIDIA/OpenShell/releases). For production installs, download and verify the release checksum files before installing the artifact. To configure the gateway process, refer to [Configuring the Gateway](/sandboxes/manage-gateways#configuring-the-gateway).
