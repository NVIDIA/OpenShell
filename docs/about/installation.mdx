...
The Homebrew service listens on `https://[::1]:17670` and generates a local mTLS bundle on install. The installer registers `https://localhost:17670` with the CLI so TLS uses a DNS name while macOS resolves the connection to IPv6 loopback. The formula creates a Homebrew prefix config, such as `/opt/homebrew/var/openshell/gateway.toml`, with this IPv6 loopback default so Docker Desktop and Podman Machine can use separate IPv4 loopback callback listeners. The gateway reads `~/.config/openshell/gateway.toml` instead when that file exists. Homebrew preserves existing prefix and user configs during upgrades.
