...
The script detects your operating system and installs the OpenShell CLI, standalone policy prover, and gateway. On Linux, it installs from the Snap Store whenever the `snap` command is available, except for prereleases, which use the native Debian or RPM package. It then starts the local gateway server so you can begin creating sandboxes.

Snap installs use `latest/stable` by default and `latest/edge` when
`OPENSHELL_VERSION=dev`. Other non-prerelease `OPENSHELL_VERSION` values also
use the stable Snap channel because Snap Store channels do not select an exact
GitHub release.
...
snap for stable and development installs. Prerelease installs
(`OPENSHELL_VERSION=pre` or a numbered prerelease tag) use Debian or RPM
packages even when `snap` is available. For Snap installs, install and start
Docker Engine from a system package or Docker's package repository first. The
Docker snap is not currently compatible with OpenShell.

On Fedora and RHEL without `snap`, or for prereleases, the install script uses RPM packages. The RPMs install the `openshell` CLI, `openshell-prover`, the `openshell-gateway` daemon, and a systemd user service.

On Debian and Ubuntu without `snap`, or for prereleases, the install script uses a Debian package. The Debian package installs the `openshell` CLI, `openshell-prover`, the `openshell-gateway` daemon, VM sandbox support, and a systemd user service.
