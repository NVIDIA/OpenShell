...
On Linux systems with the `snap` command, the install script uses the OpenShell snap. If Docker is absent, it installs the Docker snap and waits for the Docker daemon before installing OpenShell.
...
compatible installation method. The install script reuses an existing Docker
daemon. If Docker is absent, it installs the Docker snap and waits for the
daemon before installing OpenShell.
...
which exposes the installed Docker daemon regardless of how Docker was
packaged. The `log-observe` and `system-observe` plugs are also automatically
connected through the OpenShell snap's store assertions.
