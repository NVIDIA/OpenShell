...
The GitHub provider's rule, `_provider_my_github`, allows connections to
`github.com` but not push requests. Do not filter this log with `--level warn`,
which hides policy events.
...
- Review [Policies](/sandboxes/policies) for incremental policy updates and policy history.
