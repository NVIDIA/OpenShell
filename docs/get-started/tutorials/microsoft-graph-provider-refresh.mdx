...
Provider refresh updates the provider record at the gateway. The gateway pushes the new provider environment to running sandboxes, but already-running processes keep the environment they started with.
