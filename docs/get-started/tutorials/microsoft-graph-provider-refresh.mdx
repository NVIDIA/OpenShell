...
```shell
openshell profile lint -f microsoft-graph-mail.yaml
openshell profile import -f microsoft-graph-mail.yaml
```
...
Provider refresh updates the provider record at the gateway. Running sandboxes receive provider environment revisions over the supervisor configuration stream, but already-running processes keep the environment they started with.
