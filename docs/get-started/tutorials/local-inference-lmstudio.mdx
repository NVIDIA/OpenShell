...
The active `inference.local` route is the gateway default. Re-run `openshell inference set` whenever you want to switch the default between OpenAI-compatible and Anthropic-compatible clients. Use `openshell inference sandbox set` when only one sandbox should use a different provider and model.
