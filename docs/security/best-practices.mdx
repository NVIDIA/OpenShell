...
OpenShell enforces sandbox security across three layers: network, filesystem, and process.
...
| Provider credentials | Credential exposure and unauthorized service access. | Endpoint-bound placeholder resolution in the policy proxy | Yes. Update the provider or attached profile policy. |
