...
| What you can change | Add `protocol: rest` to enable per-request HTTP method/path inspection, `protocol: websocket` to inspect RFC 6455 upgrade handshakes and client text messages, or `protocol: graphql` to inspect GraphQL operation type, operation name, and root fields. Pair inspected protocols with `rules` or access presets (`full`, `read-only`, `read-write`). |
