...
ASCII case only when the containing directory is confirmed case-insensitive.
Case-sensitive, indeterminate, and globbed paths retain case so a case-distinct
executable cannot inherit another executable's grant. Verbatim `\\?\` paths are
matched exactly without separator or case rewriting. Prefer ordinary absolute
paths; when a verbatim path is necessary, copy the exact runtime-reported form
into the policy.
