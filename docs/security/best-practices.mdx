...
OpenShell applies seccomp in two phases. A narrow supervisor-startup prelude runs before CLI parsing and async runtime initialization. The parent compiles the broader runtime filter before forking, and the child installs that prepared filter after privilege drop.
...
This ordering is intentional: named network-namespace setup still relies on privileged helpers, and the supervisor must resolve named identities from `/etc/group` and `/etc/passwd` before Landlock restricts child access.
...
3. Parent-side child preparation, including executable PATH resolution, named user, group, and supplementary membership resolution, Landlock path handles, and compiled runtime seccomp programs.
4. Network and child-only mount namespace entry (`setns`) in child `pre_exec`.
5. Prepared numeric privilege drop (`setgroups` + `setgid` + `setuid`).
6. Core-dump hardening (`RLIMIT_CORE=0`, plus `PR_SET_DUMPABLE=0` on Linux).
7. Landlock filesystem restrictions.
8. Prepared runtime seccomp socket domain and syscall filter installation.
