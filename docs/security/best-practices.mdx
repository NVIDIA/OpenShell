...
On Windows, OpenShell normalizes `/` and `\` for ordinary binary paths and folds
ASCII case only when trusted runtime evidence confirms that every parent
directory uses case-insensitive lookup. A sensitive or indeterminate component
keeps the whole match case-exact, so a case-distinct executable cannot inherit
another executable's grant. Policy loading never opens policy-authored paths.
UNC, device, and verbatim namespace paths are matched exactly without separator
or case rewriting; prefer ordinary absolute paths.
