...
| OpenCode | Partial | Add `opencode.ai` endpoint to the policy for full functionality. |
| Codex | None | Provide a complete custom policy with OpenAI endpoints. |

<Info>
If you run a non-Claude agent without a custom policy, the agent's API calls are denied by the proxy. You must provide a policy that declares the agent's endpoints.
