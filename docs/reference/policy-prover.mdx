...
The `boundary-v2` containment model covers filesystem paths, process
identity settings, Landlock compatibility requirements, L4 destination authority,
...
Policies whose overlapping endpoints select different destination restrictions
remain unsupported because the runtime uses the selected endpoint's address
filter. CIDR unions within a supported endpoint are checked by the solver.
Unsupported IP-literal wildcard selectors and ranges rejected by the runtime
also cannot produce a successful proof.

Consumers must explicitly accept model `boundary-v2` and its additional
`process` and `landlock` domains. Do not treat a v1 result as covering these
fields.
