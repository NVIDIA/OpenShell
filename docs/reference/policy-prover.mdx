...
Create `maximum.yaml` with this maximum policy:
...
with a policy that makes `/etc` writable even though the maximum grants only
read access:
...
|---|---|
| `counterexample` | An object for `exceeds_max`; otherwise `null`. |
