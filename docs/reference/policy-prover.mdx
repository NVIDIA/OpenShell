...
`scope.domains` is the machine-readable coverage declaration. An exceeding
result also includes a typed counterexample. Automation should use `result` and
`reason_code` instead of parsing the human-readable explanation.
...
| Exit | Result | Meaning |
|---|---|---|
| `0` | `within_boundary` | Containment was established for the reported scope. |
...
object reports the domains used for each check. Policies that use recognized
authority outside that scope return `unsupported` rather than silently ignoring
it.
...
Consumers must inspect `scope.domains` and require every domain relevant to
