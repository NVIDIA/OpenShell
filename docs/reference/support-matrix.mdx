...
OpenShell uses different libc targets for host-executed artifacts:

| Artifact | Linux libc target | Role |
|---|---|---|
| `openshell` | Static musl | Standalone CLI and package-managed CLI. |
| `openshell-gateway` | GNU/glibc 2.31 | Host gateway daemon and gateway container image binary. |
| `openshell-driver-vm` | GNU/glibc 2.31 | Host subprocess for VM-backed sandboxes. |
| `openshell-sandbox` | Runtime-specific | Supervisor that usually runs inside controlled containers, pods, or VM guests. |
