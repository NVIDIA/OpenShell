...
| Apple Container | Supported for local macOS workflows on Apple silicon. | Requires Apple's `container` CLI on the gateway host. |
...
| Apple Container CLI | 1.0.0 tested | Required for Apple Container-backed gateways. The driver uses `container system status --format json`, `container list --all --format json`, and `container network list --format json`; no older minimum version is declared yet. |
