...
For `kubernetes-secrets`, `namespace` sets where OpenShell-managed provider Secret objects are stored. When omitted, the driver uses the in-cluster ServiceAccount namespace when available, otherwise `default`. The gateway's RBAC Role grants read/write access to all Secrets in the configured namespace because Kubernetes does not allow `resourceNames` restrictions on create operations. Use a dedicated namespace (e.g., `openshell-credentials`) to isolate managed Secrets from other workloads.
