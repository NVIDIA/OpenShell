...
The driver assigns a distinct loopback listener port to each MXC sandbox, but the listener does not authenticate its TCP peer. Treat Windows processes that can connect directly to gateway-owned loopback ports as part of the trusted host boundary.
