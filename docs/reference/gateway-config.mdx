...
# "network-sidecar" runs only network enforcement in a sidecar and leaves the
...
# capabilities into the effective set. In sidecar topologies the network init
# container installs nftables rules that exempt the effective sidecar UID.
proxy_uid                      = 1337
# Keep process/binary-aware network policy enabled in sidecar topologies. Set
