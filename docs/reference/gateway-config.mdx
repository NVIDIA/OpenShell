...
[openshell.drivers.kubernetes.disruption_protection]
# Disabled by default. Enabling this permits, but does not automatically add,
# per-sandbox PodDisruptionBudgets. New requests are rejected while disabled;
# previously accepted PDBs remain until their deadlines but are not repaired.
enabled      = false
# Maximum duration accepted from the first-class Sandbox API request. This is a
# fail-closed expiration target: if every gateway is unavailable at the
# deadline, the PDB remains until reconciliation resumes or an operator removes
# it.
max_duration = "24h"
```
...
Custom non-Helm deployments that enable this feature must grant the gateway
service account `create`, `delete`, `get`, `list`, and `patch` on `policy/v1`
PodDisruptionBudgets in the sandbox namespace.
