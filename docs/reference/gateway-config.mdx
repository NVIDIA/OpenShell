...
# When enabled, compatible creates may use v1beta1 SandboxClaim resources
# backed by the driver's cache of OpenShell-enabled SandboxWarmPool resources.
# Requires the Agent Sandbox extensions.yaml APIs. If they are unavailable at
# gateway startup, OpenShell disables warm pooling and template reconciliation
# for this driver instance and falls back to direct Sandbox creation.
enabled = true

[openshell.drivers.kubernetes.warm_pooling.templates]
# Optional OpenShell SandboxTemplate-backed reconciler for generated warm pools.
enabled = true
...
key used for driver-owned sandbox template config such as
`SandboxTemplate.spec.driver_config.<name>`. Attach driver-owned config through
`openshell sandbox template create --driver-config-json`.
