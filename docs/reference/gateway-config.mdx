...
# ... governed policy/provider profile source ...
...
[openshell.gateway.policies]
# Filesystem directory, file:// URI, or grpc+unix:// socket.
location = "grpc+unix:///var/run/openshell/policy-source.sock"
# Applied to new sandboxes only when CreateSandbox omits spec.policy.
...
`[openshell.gateway.policies]` configures an optional startup-loaded source for governed sandbox policies and provider type profiles. Set `location` to a filesystem directory, a `file://` URI, or a `grpc+unix://` Unix socket. Set `default_policy` to the policy document name the gateway should apply when `CreateSandbox` omits `spec.policy`; an explicit request policy always wins.

Filesystem sources use this layout:
...
Both `.yaml` and `.yml` are accepted. Document names are derived from the file stem.

gRPC sources implement `openshell.policy_source.v1.PolicySource` from `proto/policy_source.proto`:
...
`GetPolicy` and `GetProvider` return only `bytes document = 1`. Documents must be UTF-8 YAML. The gateway stores the exact YAML bytes with a SHA-256 digest for later verification, parses policy YAML into the sandbox policy model, and imports provider YAML as source-backed provider profiles. Source-backed provider profiles can be updated by the source on restart, but regular provider-profile import and delete APIs cannot overwrite or remove them.
