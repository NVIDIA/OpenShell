...
# the gateway host's loopback is reachable only through the host alias
# host.openshell.internal (gvproxy NATs 192.168.127.254 to the host's
# 127.0.0.1). Address it as http://host.openshell.internal:<port>, not
# http://127.0.0.1:<port>. A proxy on the corporate network is dialed out
# through the host process and needs no alias. The guest's callback to the
# gateway is unaffected and never traverses the proxy.
#
