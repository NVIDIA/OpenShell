...
# "split-pod" moves network enforcement and gateway forwarding into a separate
# supervisor pod and uses NetworkPolicy to force agent egress through it. In
# sidecar and split-pod modes, the agent container runs non-root with no added
# Linux capabilities and process/filesystem enforcement is network-only.
supervisor_topology            = "combined"
# UID used by the long-running network sidecar or split supervisor pod. In
