...
# Defaults to the gateway version. The gateway accepts its current internal
# supervisor protocol revision, the immediately previous revision through
# polling compatibility, and pre-handshake supervisors for one release. Other
# revisions reject the session. Compatibility sessions emit gateway warnings.
