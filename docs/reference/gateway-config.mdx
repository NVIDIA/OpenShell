...
# Optional keys used only to decrypt envelopes written with an earlier key.
decrypt_only_key_encryption_key_paths = [
  "/var/lib/openshell/credentials/previous-key-encryption-key.bin",
]
# Rewrap existing data-encryption keys with the active key during startup.
...
For the default encrypted database store, OpenShell stores provider credentials as JSON envelopes encrypted with AES-256-GCM in the gateway database. Each credential gets a random data-encryption key; the gateway wraps that key with a local key-encryption key. By default, the active key-encryption key is created at `$XDG_STATE_HOME/openshell/gateway/credentials/key-encryption-key.bin` with owner-only permissions. Use `[openshell.gateway.credential_storage] key_encryption_key_env` instead of `key_encryption_key_path` to load a base64-encoded 32-byte active key from an environment variable.

`decrypt_only_key_encryption_key_paths` and `decrypt_only_key_encryption_key_envs` add keys that may decrypt older envelopes but are never used for new writes. OpenShell derives the key identifier from the key material and rejects duplicate keys. Unknown key identifiers fail closed. Set `rewrap_on_startup = true` to scan the encrypted credential records at gateway startup and rewrap their data-encryption keys with the active key. Rewrap preserves the credential ciphertext and uses compare-and-swap updates so it cannot overwrite a concurrent credential change. The sweep is idempotent and a later startup can resume after interruption.

Back up the database and every key required by retained backups together; losing a required key makes those stored credentials unrecoverable. A rewrap changes the live envelopes but does not change historical backups. Keep the previous key until a backup created after rewrap has been restore-tested and older backups have reached the end of their retention period.

In Kubernetes, the Helm chart creates a retained Secret containing the shared active key, injects it as `OPENSHELL_GATEWAY_CREDENTIAL_KEY_ENCRYPTION_KEY`, and renders `key_encryption_key_env` for the gateway when no external credential driver is enabled. Multi-replica deployments need every replica to use the same database and keyring.

Rotate the key in an HA deployment with staged rolling updates:

1. Set `server.credentialStorage.existingSecret` to the current key Secret and add the future key Secret to `decryptOnlyExistingSecrets`. Leave `rewrapOnStartup` disabled and wait for every replica to become ready.
2. Set `existingSecret` to the future key Secret and put the previous Secret in `decryptOnlyExistingSecrets`. Keep `rewrapOnStartup` disabled and wait until every replica uses the future key for new writes.
3. Enable `rewrapOnStartup` without changing the keyring and complete another rolling update. Check the completion events; the final replica should report zero remaining rewraps after all earlier replicas finish.
4. Create and restore-test a new backup. After older backups have reached the end of their retention period, remove the previous Secret from `decryptOnlyExistingSecrets`, disable `rewrapOnStartup`, and complete one final rolling update.

Each referenced Secret must contain a base64-encoded 32-byte key under `key-encryption-key`. Do not change the active key without first staging both keys on every replica.
...
server:
  credentialStorage:
    ...
    decryptOnlyExistingSecrets: []
    rewrapOnStartup: false
```
