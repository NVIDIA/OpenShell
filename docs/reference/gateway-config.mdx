...
supported `compute_driver.proto` gRPC service on a Unix socket. The selected
...
At launch time, `--compute-driver-socket` can override the implementation for
any explicitly selected name, including `docker`, `podman`, `kubernetes`, and
`vm`. The override takes precedence over built-in construction:

```shell
openshell-gateway --drivers docker \
  --compute-driver-socket /run/openshell/docker-driver.sock
```

Do not use `[openshell.drivers.docker].socket_path` for this purpose; that
existing field selects the Docker daemon socket. Operator-supplied driver
processes and Unix sockets are not created or supervised by the gateway.
