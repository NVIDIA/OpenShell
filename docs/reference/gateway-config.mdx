...
# Optional. Defaults to 1000. Must be >= 1. Root (0) is always rejected.
# Lower these for enterprise UIDs/GIDs such as GID 30.
min_sandbox_uid       = 1000
min_sandbox_gid       = 1000
...
`[openshell.gateway] min_sandbox_uid` and `min_sandbox_gid` are the inclusive minima for numeric `run_as_user` and `run_as_group` values. Both default to `1000`. Set them to `1` to accept enterprise system IDs such as GID `30`. The value `0` (root) is always rejected. The gateway injects the configured minima into Docker, Podman, Kubernetes, and VM sandbox environments so the in-sandbox supervisor uses the same range.
