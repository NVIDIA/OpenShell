...
Set `egress_proxy = true` with a loopback seed such as `egress_proxy_addr = "127.0.0.1:18080"` to enable the Windows Pattern-C split. MXC redirects sandbox traffic to a sandbox-unique port on the configured loopback IP, where the host CONNECT proxy enforces the trimmed network policy. Governed egress requires `process_container` until MXC supports `network.proxy` for isolation sessions.
