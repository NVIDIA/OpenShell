...
# Set this only after verifying that the cluster CNI enforces both ingress and
# egress NetworkPolicy in every sandbox namespace.
...
# The workload runtime and supervisor companion use separate images. Both
# default to the gateway version; override either to pin a specific build.
...
# Omit this field to keep Docker's runtime-selected AppArmor profile.
# Localhost/<name> requires an operator-loaded profile; Unconfined is an
# explicit operator opt-out.
# app_armor_profile = "Localhost/openshell-sandbox"
