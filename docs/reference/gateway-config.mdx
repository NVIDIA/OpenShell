...
Each interceptor must also complete [extension protocol negotiation](/extensibility/extension-negotiation) during `Describe`. Upgrade the interceptor first when moving from the legacy manifest: the gateway rejects missing metadata, incompatible protocol majors, and unmet requirements before serving requests.
