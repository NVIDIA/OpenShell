...
# Omit for the platform default. On Linux the supervisor always shares the
# gateway's host network namespace, so the default resolves to loopback
# regardless of host topology. On macOS Podman machine, the default is
# gvproxy's host-loopback IP (192.168.127.254). Set explicitly only to
# override with a different reachable address (e.g. a containerized or
# remote gateway).
