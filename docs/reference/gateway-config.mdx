...
The native Windows gateway links the MXC compute driver in-process. Gateway configuration contains host runtime settings only; workload command, working directory, and environment are sandbox-scoped.
...
# process_container is the default and enforces default-deny filesystem access.
# isolation_session is an explicit grant-only compatibility mode.
...
Unknown MXC fields are rejected. Network policies and live policy mutations fail closed until the driver has a bound enforcement path. See [Sandbox Compute Drivers](sandbox-compute-drivers.mdx#windows-mxc-driver) for per-sandbox workload configuration.
