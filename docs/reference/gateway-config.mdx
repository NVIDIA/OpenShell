...
`/etc/openshell-tls/network-additional-ca.crt`. The Kubernetes driver creates a
managed ConfigMap containing `ca.crt` in each target namespace and mounts it
only into the combined supervisor or network sidecar. The VM driver writes the
same path into each sandbox overlay. Every driver passes
`--network-additional-ca-bundle` as an operator-controlled supervisor argument.
...
There is no hot reload. Restart the gateway after changing a source file, then
recreate or restart affected sandboxes. Running supervisors retain the trust
loaded at their startup. Removing the table and restarting returns new
sandboxes to default destination trust.
...
the global table, and grants the Kubernetes driver ConfigMap apply permissions
for the selected workspace mode. Updating only the source ConfigMap does not
trigger the gateway workload checksum, so restart the gateway explicitly.
