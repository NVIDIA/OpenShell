...
account's user SID and may retain group SIDs that are disabled for access
checks, so seeing the same username or `BUILTIN\\Administrators` in
`whoami /all` does not mean the sandbox has effective administrator access.
Validate the boundary with the token APIs (`TokenIsAppContainer`,
`TokenAppContainerSid`, and `CheckTokenMembership`) instead; the MXC real-binary
integration suite checks all three for the default configuration.
