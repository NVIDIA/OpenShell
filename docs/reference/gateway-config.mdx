...
commands and working directories remain sandbox-scoped. Supply the command with
the generic trailing-command syntax or through the `mxc` driver configuration
described below.
...
Supply the workload command with the generic `sandbox create -- <COMMAND>` syntax. Use `sandbox create --driver-config-json` for an optional working directory or the legacy MXC-specific command form, for example `{"mxc":{"command":["cmd","/c","echo hello"],"cwd":"C:\\work"}}`. If both forms supply a command, `mxc.command` wins. Supply workload environment through `sandbox create --env` or `--env-from`; it is not part of gateway configuration.
