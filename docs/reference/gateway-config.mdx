...
Package-managed gateways create a TOML file on first start and launch the gateway with `--config`. Set `OPENSHELL_GATEWAY_CONFIG` in the launch environment to use a different file.

| Package | Gateway TOML location |
|---|---|
| Homebrew | `$XDG_CONFIG_HOME/openshell/gateway.toml` when it exists, otherwise the Homebrew-managed `var/openshell/gateway.toml` under the Homebrew prefix, such as `/opt/homebrew/var/openshell/gateway.toml`. |
| Debian/Ubuntu | `%E/openshell/gateway.toml`, usually `~/.config/openshell/gateway.toml` for the systemd user service. |
| Fedora/RHEL RPM | `%E/openshell/gateway.toml`, usually `~/.config/openshell/gateway.toml` for the systemd user service. |
