...
namespace                      = "agents"
service_account_name           = "openshell-sandbox"
# Extra ServiceAccounts accepted when a sandbox bootstraps, on top of
# service_account_name. Only needed when something other than the driver
# assigns the pod's ServiceAccount. Matched exactly, namespace-independent.
# additional_bootstrap_service_account_names = ["openshell-sandbox-2"]
# ServiceAccounts a caller may request per sandbox via
# SandboxTemplate.service_account_name. Also accepted at bootstrap.
