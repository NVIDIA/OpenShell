...
## Supervisor connections and policy recovery

The first supervisor connection offers the policy found in the sandbox image. A policy already stored by the gateway takes precedence. The gateway sends the selected policy back on the same stream for image-specific filesystem preparation, validates and persists any proposed change, then accepts the session with a fresh authoritative configuration bootstrap. A preparation or bootstrap failure rejects the session before runtime initialization. Reconnects skip image-policy preparation and receive the current gateway bootstrap directly.

On startup, the gateway repairs missing legacy policy history and skips invalid stored policies. A valid global policy can still override an invalid local policy; otherwise, that sandbox's configuration reads continue to report the validation failure.
...
# Defaults to the gateway version. The current internal supervisor protocol
# streams authoritative configuration snapshots. The immediately previous
# revision remains polling-compatible, and pre-handshake supervisors remain
# compatible for one release. Other revisions reject the session.
...
# Defaults to the gateway version. Custom builds must match the gateway's
# internal supervisor protocol revision; mismatched peers reject the session.
# Supervisors from releases before the handshake existed remain compatible for
# one release.
# supervisor_image             = "ghcr.io/nvidia/openshell/supervisor:<version>"
...
# sandbox_runtime_image = "ghcr.io/nvidia/openshell/sandbox:<version>"
# Supervisor runtime defaults to the gateway version. The current internal
# protocol streams authoritative configuration snapshots; the immediately
# previous revision uses polling compatibility. Pre-handshake supervisors
# remain compatible for one release. Other revisions reject the session.
# supervisor_image = "ghcr.io/nvidia/openshell/supervisor:<version>"
...
# sandbox_runtime_image = "ghcr.io/nvidia/openshell/sandbox:<version>"
# Dynamically linked supervisor runtime defaults to the gateway version. The
# current internal protocol streams authoritative configuration snapshots; the
# immediately previous revision uses polling compatibility. Pre-handshake
# supervisors remain compatible for one release. Other revisions reject the
# session.
# supervisor_image      = "ghcr.io/nvidia/openshell/supervisor:<version>"
