...
# Defaults to the gateway version. The gateway and supervisor use a
# release-matched internal protocol revision; every other revision rejects the
# session so configuration is never applied through a removed fallback path.
