...
`[openshell.gateway.auth] allow_oidc_auth_only = true` permits OIDC authentication-only mode when `admin_role` and `user_role` are both empty. This skips role checks for tokens validated by the configured issuer while retaining configured scope checks. The gateway rejects this setting when OIDC is absent or RBAC roles are configured, and logs a startup warning when the mode is active. Shared gateways fail startup unless OIDC RBAC roles are configured or this flag is set explicitly.
