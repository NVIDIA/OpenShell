...
`pc_least_privilege` requests a Less-Privileged AppContainer and defaults to
`false`. Leave it enabled. A sandbox without it can reach host services that
issue network requests on its behalf, so a workload sends outbound HTTP even
when it declares no `network_policies`. Enabling it requires
`pc_capabilities = ["registryRead"]`, without which the sandbox cannot
initialize Winsock and most runtimes exit during startup. `registryRead` grants
read access to the Winsock catalog keys and no network access.

Grant network access with `egress_proxy`, which applies gateway network policy.
Adding `internetClient` to `pc_capabilities` instead grants unrestricted
outbound access that a sandbox's
[`network_policies`](/reference/policy-schema#network-policies) cannot
constrain.
