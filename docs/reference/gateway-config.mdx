...
Each sandbox runs inside its own libkrun microVM managed by one `openshell-driver-vm` subprocess spawned by the gateway. Configure VM driver settings in this gateway TOML file under `[openshell.drivers.vm]`.
...
default_image  = "ghcr.io/nvidia/openshell/sandbox:latest"
# Guest mTLS material is gateway-level input inherited by local compute drivers.
...
# Optional bootstrap image. When unset, the VM driver uses default_image, then
# the requested sandbox image.
# bootstrap_image = "ghcr.io/nvidia/openshell/sandbox-bootstrap:latest"
...
# Hypervisor helper verbosity. Distinct from [openshell.gateway].log_level,
# which controls gateway and driver process logs.
hypervisor_log_level = 1

# GPU passthrough support. Leave disabled unless the host is prepared for VFIO.
gpu_enabled = false
gpu_mem_mib = 8192
gpu_vcpus   = 4
```
