...
# plugin to install the pod-network rules before the workload starts. In sidecar
# and cni-sidecar modes, the agent container runs non-root with no added Linux
# capabilities and process/filesystem enforcement is network-only.
topology                       = "combined"
...
# container installs nftables rules that exempt the effective sidecar UID; in
# cni-sidecar topology the OpenShell CNI plugin installs equivalent rules.
proxy_uid                      = 1337
