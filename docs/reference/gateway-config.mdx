...
Each sandbox runs inside its own libkrun microVM managed by one `openshell-driver-vm` subprocess spawned by the gateway. Configure VM driver settings through the gateway TOML file; `openshell-driver-vm` does not read a separate `--config` file.
