...
| `mcp.versions` | list of string | No | Nonempty allowlist of exact supported MCP core revisions: `2025-03-26`, `2025-06-18`, `2025-11-25`, and `2026-07-28`. Omission resolves to `["2025-11-25"]`; it never means latest or all known revisions. The key must be absent to use this default; `versions: null` and `versions: []` are invalid. Values must be unique, contain no extra whitespace, and name a revision in this closed set. OpenShell selects each request's revision from `MCP-Protocol-Version` and requires it to appear in the allowlist, except during a valid legacy `initialize`. The 2025 revisions permit a missing-header fallback to `2025-03-26`. The sessionless `2026-07-28` revision requires the header and matching per-request metadata; see [MCP allow and deny rules](#mcp-allow-and-deny-rules-protocol-mcp). Malformed or unsupported version headers receive `400 Bad Request`; a supported revision outside the allowlist receives `403 Forbidden`. |
...
| `mcp.allow_all_known_mcp_methods` | bool | No | Defaults to `false`. When `true`, enables the endpoint MCP method profile: omitted `rules` allow methods available in the selected core revision and all tools before `deny_rules`, and omitted rule `method` uses that profile. Unknown extensions still require an exact literal method rule. When unset or `false`, explicit MCP method rules are required; rules with `tool` or `params.name` must set `method: tools/call`. |
...
MCP rules match sandbox-to-server MCP Streamable HTTP request bodies by MCP method and optional tool selectors. OpenShell uses `tower-mcp-types`, pinned to `0.22.2`, for JSON-RPC structure and MCP method, direction, message-kind, parameter, and metadata type checks against the selected revision. Type validation follows Tower's deserialization and inspection APIs; it does not provide complete JSON-schema validation. OpenShell enforces the revision allowlist, HTTP/body consistency, request limits, and policy rules. Methods known but unavailable in the selected revision are rejected. Unknown extension methods require an exact literal method rule; method globs and `mcp.allow_all_known_mcp_methods` never authorize them.

Omitting `mcp.versions` or the entire `mcp` object resolves to `["2025-11-25"]`. Stored and serialized policy data contain this explicit list, so adding another supported revision cannot widen a normalized policy. Set `versions: ["2026-07-28"]` to opt into the sessionless revision. A policy version selects core request inspection; it does not claim conformance for separately named protocol extensions.

For the 2025 revisions, a valid standalone `initialize` proposes a version in its body. Subsequent HTTP requests select their revision with exactly one `MCP-Protocol-Version` header. An absent header selects `2025-03-26`, which must appear in the endpoint allowlist; it does not select the policy default. OpenShell stores no connection or session state and does not inspect the server's initialization response. The `2025-03-26` profile permits nonempty same-side top-level JSON-RPC batches capped at 64 members; later supported profiles prohibit top-level batches.

For `2026-07-28`, each HTTP POST contains one JSON-RPC request or explicitly allowed extension notification. For JSON-RPC requests, OpenShell checks the following before policy evaluation and repeats the checks after middleware changes the request:

- `params._meta` contains `io.modelcontextprotocol/protocolVersion` set to `2026-07-28` and `io.modelcontextprotocol/clientCapabilities`. Metadata, including optional `io.modelcontextprotocol/clientInfo`, must pass Tower's request-metadata type checks. These requirements also apply to unknown extension requests.
- Exactly one `MCP-Protocol-Version` header matches the body version, and exactly one `Mcp-Method` header matches the JSON-RPC method.
- `Mcp-Name` is required for `tools/call` and `prompts/get`, where it matches `params.name`, and for `resources/read`, where it matches `params.uri`. OpenShell decodes the standard `=?base64?...?=` header encoding before comparison.

Missing required metadata, values rejected by Tower's type checks, and missing, malformed, or mismatched required headers receive `400 Bad Request`. A supported revision outside the endpoint allowlist receives `403 Forbidden`. The sessionless profile has no `initialize` or `notifications/initialized` handshake. A client may call `server/discover` first, and `subscriptions/listen` receives notifications on its POST response stream. Standalone GET streams, DELETE session operations, client JSON-RPC response frames, and the stdio-only `notifications/cancelled` method are rejected.

Extension notifications require an exact method allow rule and `MCP-Protocol-Version: 2026-07-28` to select this profile. They do not require `params._meta`, `Mcp-Method`, or `Mcp-Name`; this revision defines no metadata-header contract for notification POSTs. Deny rules still take precedence over the exact allow rule.

OpenShell forwards custom headers without matching `Mcp-Param-*` fields against tool schemas. JSON-RPC responses and server-to-client MCP messages on response bodies or SSE streams, including multi-round tool result payloads, are relayed without policy parsing.

`mcp.allow_all_known_mcp_methods` defaults to `false`, so endpoints require explicit MCP method rules. Set it to `true` to enable methods available in the selected core profile; rules can then omit `method`, and tool selectors are normalized to `tools/call` internally. By default, `tools/call` `params.name` must match the MCP-recommended tool-name pattern `^[A-Za-z0-9_.-]{1,128}$`; configure `mcp.strict_tool_names: false` only for a server that intentionally uses names outside that pattern. Wildcard `tool` matchers require `mcp.strict_tool_names` to remain enabled.

Use `rules` for MCP allow rules and `deny_rules` for MCP deny rules. Deny rules take precedence over allow rules. If an MCP endpoint sets `mcp.allow_all_known_mcp_methods: true` and omits `rules`, OpenShell allows methods available in the request's selected MCP core profile and all tools, then applies any `deny_rules`; extensions still require exact literal method rules. Otherwise, the endpoint must define explicit rules. A broad allow or deny rule whose method matcher includes `tools/call` cannot be combined with tool-specific allow rules because it would bypass or erase the tool filter; add `tool` or `params.name` to scope `tools/call`, or remove the tool-specific rules. In a batch request, one denied call denies the full batch.
...
OpenShell canonicalizes an explicit list in semantic order. Each request selects one allowed revision and is inspected with only that profile. For a `2026-07-28` policy example, see [MCP and JSON-RPC matching](/sandboxes/policies#mcp-and-json-rpc-matching).
