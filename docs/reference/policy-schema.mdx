...
| `protocol` | string | No | Set to `tcp` for explicit L4 TCP passthrough, `rest` for HTTP method/path inspection, `websocket` for RFC 6455 upgrade and client text-message inspection, `graphql` for GraphQL-over-HTTP operation inspection, `mcp` for MCP Streamable HTTP request inspection, or `json-rpc` for generic JSON-RPC-over-HTTP method inspection. WebSocket endpoints can also use GraphQL operation rules for GraphQL-over-WebSocket traffic. Omitting the field remains equivalent to `tcp`. Direct sandbox DNS and transparent TCP capture are not enabled yet. Provider-credentialed endpoints require an inspected protocol unless `allow_uninspected_credentials` is explicitly set. |
...
- `protocol: tcp` is L4-only and rejects the L7-only `access`, `rules`, and `deny_rules` fields.
