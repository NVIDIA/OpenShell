...
Ordinary runtime policy uses the strict profile. Unknown keys in closed schema
objects are rejected with their field path. The following maps are intentionally
...
The containment profile used by managed maximum-policy tooling retains unknown
closed-object fields for a fail-closed support audit. It also recognizes
top-level managed `metadata` and endpoint or allow-rule `review` annotations.
Those annotations describe workflow, not authority, and ordinary runtime policy
rejects them. Maximum-policy containment support lands separately from this
schema boundary.
