...
| `websocket_credential_rewrite` | bool | No | When `true` on a `protocol: rest` endpoint, OpenShell rewrites `openshell:resolve:env:*` placeholders in client-to-server WebSocket text messages after an allowed HTTP `101` upgrade. Defaults to `false`. |
