...
| Field | Type | Required | Description |
|---|---|---|---|
| `method` | string | Yes | JSON-RPC method name or glob to deny. A value without glob metacharacters matches that exact method name. In glob patterns, `*` matches any character sequence, so `method: "*"` denies any JSON-RPC method rather than a literal method named `*`. To deny a literal method named `*`, escape the glob character as `method: "\\*"`. |
| `params` | map | No | Params matchers keyed by flattened object-param path. Omit to deny every call matching `method`. Strings, numbers, and booleans are converted to strings; arrays, `null`, and non-object top-level params do not produce matcher keys. |
