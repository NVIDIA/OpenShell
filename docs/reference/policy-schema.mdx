...
numeric UID/GID in the allowed sandbox range. Docker and Podman may select
other named identities or non-root system IDs only through OCI `USER`
fallback. Root identities are always rejected.
