...
Sandbox delegated identity uses another Authorization Code flow with PKCE when
the authenticated user has no usable gateway-owned delegated credential. The
CLI opens the browser and may reuse the existing identity-provider SSO session,
but obtains a token bundle separate from the normal CLI login. The delegated
request uses only `openid` and `offline_access`, plus the gateway audience. The
CLI transfers the returned bundle to the gateway and never writes it to
`oidc_token.json`; the gateway then owns and rotates that refresh-token chain.
The IdP must enable the authorization code flow and offline access for the
configured public client, allow the CLI's loopback callback URI, and issue a
distinct refresh-token chain for each authorization transaction.

OpenShell continues to use the normal CLI access token for status,
authorization, create, and extend RPCs. A long browser flow or image build may
outlive that token, so the CLI reloads or refreshes the normal login immediately
before constructing the final gRPC client. A healthy delegated grant is reused
without another prompt. A missing, revoked, or inactive delegated grant prompts
for a replacement without changing the local login.
