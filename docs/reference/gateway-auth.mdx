...
`openshell.gateway.oidc` TOML table:
...
When a gateway is deployed without a TLS table in `gatewayConfig`, TLS is disabled entirely. The CLI connects over plain HTTP/2. This mode is intended for local port-forwarding or gateways behind a trusted reverse proxy or tunnel that handles TLS termination externally.
