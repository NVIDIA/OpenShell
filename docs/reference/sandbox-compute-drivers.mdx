...
create response cannot poison a retry. The resolved workspace is the cwd and
`HOME` for direct and SSH children. The supervisor itself starts from `/`, so a
missing or invalid workspace is handled during readiness instead of preventing
the container runtime from starting it.
