...
| `disruption_protection.enabled` | `server.disruptionProtection.enabled` | Allow individual sandboxes to request a time-bounded PodDisruptionBudget. Disabled by default. Enabling it does not protect sandboxes that omit a duration request. |
...
Operators can allow sandboxes to request time-bounded protection from
voluntary Kubernetes evictions:
...
PDB from that deadline after restarts and deletes it after expiration. If every
gateway is unavailable when the deadline passes, protection remains active
until a gateway resumes reconciliation.
