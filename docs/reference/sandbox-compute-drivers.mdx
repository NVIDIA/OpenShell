...
A driver may instead advertise driver-owned runtime control. In that mode, the
gateway sends the canonical policy in the validation and creation requests, the
driver applies it before launch, and driver-reported readiness does not require
a supervisor session. The Windows MXC driver uses this mode.
