...
| `topology` | `supervisor.topology` | Set `combined` for the default single supervisor path, `sidecar` to move pod-level network enforcement and the gateway session into a dedicated sidecar, or `network-sidecar` to run only network enforcement in a sidecar while preserving the agent image entrypoint. |
| `sidecar.proxy_uid` | `supervisor.sidecar.proxyUid` | UID used by the long-running network sidecar in sidecar topologies. The network init container exempts this UID from proxy redirection. |
| `sidecar.process_binary_aware_network_policy` | `supervisor.sidecar.processBinaryAwareNetworkPolicy` | Keep process/binary-aware network policy enabled in sidecar topologies. Set false to drop the sidecar's `SYS_PTRACE` capability used for cross-UID `/proc` inspection and enforce endpoint/L7 policy without matching `policy.binaries`. |
...
long-running sidecar runs non-root, drops default capabilities, and adds only
`SYS_PTRACE` for workload process identity resolution through shared `/proc`. The
`sidecar.process_binary_aware_network_policy = false` setting removes that
sidecar capability and relaxes network policy to endpoint/L7 matching only. The
...
In `network-sidecar` topology, the agent container runs the image entrypoint
directly and does not start an OpenShell process supervisor. OpenShell forces
egress through the sidecar proxy and can perform best-effort process/binary-aware
network policy when shared PID namespace discovery and `/proc` access work. If
that discovery fails, the sidecar logs a warning and evaluates endpoint/L7
policy without process or binary matching for that request. This topology does
not provide OpenShell SSH, exec, file sync, Landlock filesystem policy, child
seccomp policy, or provider environment injection into the workload.
