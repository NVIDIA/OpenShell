...
| `client_tls_secret_name` | `server.tls.clientTlsSecretName` | Project only `ca.crt` from the Kubernetes TLS Secret into supervisor Pods so they can authenticate the gateway. |
