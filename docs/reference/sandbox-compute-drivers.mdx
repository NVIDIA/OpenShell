...
by Docker Desktop and Podman Machine, set `bind_address = "[::1]:17670"` for the primary listener
and register `https://localhost:17670` as the CLI endpoint. The hostname matches
the generated certificate and is the package-managed default. The CLI also
normalizes raw IPv6-literal endpoints for TLS. Do not broaden the primary
listener to `0.0.0.0`.
...
through an IPv4 host-gateway alias. When the primary gateway listener does not
already cover IPv4 loopback, the Docker driver requests a separate
