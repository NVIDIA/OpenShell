...
while its Kubernetes watch is healthy. If every gateway is unavailable when
the deadline passes, protection remains active until a gateway resumes
reconciliation or an operator deletes the PDB. Treat the requested duration as
a fail-closed expiration target, not a Kubernetes-enforced TTL. If the persisted
deadline becomes malformed, OpenShell retains the PDB until an operator repairs
or removes the annotation.
