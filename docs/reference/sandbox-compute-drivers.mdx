...
When `openshell.drivers.kubernetes.warm_pooling.profiles.enabled = true`, the
driver reconciles labelled ConfigMaps into generated `SandboxTemplate` and
`SandboxWarmPool` resources. This is enabled by default with the Helm chart.
The reconciler uses the same workspace-to-namespace mapping as sandbox
creation.
Each profile ConfigMap must contain
`data["warm-pool.toml"]` with a narrow CLI-shaped schema:
...
matching path discovers them like any externally managed warm pool.
