...
When either identity component falls back to OCI `USER`, OpenShell creates the
fixed `/sandbox` workspace if it is absent and transfers ownership of only that
directory to the completed UID/GID. Existing image contents and nested bind or
volume mounts retain their ownership. Sandbox startup fails before readiness
when the workspace root cannot be prepared, including when it is read-only.
Images only need to pre-own files beneath `/sandbox` when the agent must modify
...
adopt the image's OCI working directory.
