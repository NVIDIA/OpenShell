...
supervisor resolves any required names inside that image, then uses the numeric
UID/GID for both direct and SSH children. It does not modify `/etc/passwd` or
`/etc/group`.
