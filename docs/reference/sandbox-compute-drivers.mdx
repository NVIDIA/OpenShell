...
`SandboxTemplate.spec.driver_config`. The value is a JSON object keyed by driver
name. The gateway forwards only the block for the active driver, so a
Kubernetes gateway receives the `kubernetes` object from a value such as:
