...
validation. Podman checks the same
pinned image ID in a temporary networkless probe without the workspace volume,
token, or TLS secrets. The probe retains only `SETUID` and `SETGID`, drops to
