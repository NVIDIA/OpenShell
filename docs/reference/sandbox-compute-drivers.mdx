...
| `supervisor_topology` | `supervisor.topology` | Set `combined` for the default single supervisor path, `sidecar` to move pod-level network enforcement and gateway forwarding into a dedicated sidecar, or `split-pod` to run network enforcement in a separate supervisor pod with NetworkPolicy isolation. |
| `sidecar_proxy_uid` | `supervisor.sidecarProxyUid` | UID used by the long-running network sidecar or split supervisor pod. In `sidecar` topology, the network init container exempts this UID from proxy redirection. |
...
and `split-pod` topology, the agent container runs as the resolved sandbox
UID/GID with no added Linux capabilities. Sidecar mode uses a root init
container for nftables setup and a long-running non-root sidecar. Split-pod mode
creates a separate non-root supervisor pod, a headless Service, a proxy CA
Secret, and per-sandbox NetworkPolicies. Both modes keep gateway session and SSH
behavior, but the process supervisor runs in network-only mode: filesystem
policy, process privilege dropping, and
