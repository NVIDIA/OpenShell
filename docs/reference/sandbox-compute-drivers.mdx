...
the default sidecar path. The network sidecar receives provider environment
updates over its gateway session and streams them to the process supervisor so
future child processes can see updated provider env without gateway access in
the agent container.
