...
| `proxy_uid` | `supervisor.proxyUid` | UID used by the long-running network sidecar or proxy supervisor pod. In `sidecar` topology, the network init container exempts this UID from proxy redirection. In `cni-sidecar` topology, the OpenShell CNI plugin installs equivalent nftables or iptables exemptions. |
...
OpenShell CNI DaemonSet to install nftables or iptables pod-network rules before
the workload starts.
...
and process/binary identity checks are not applied by the process container. Set
