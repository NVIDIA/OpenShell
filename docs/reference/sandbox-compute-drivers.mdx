...
Two distinct settings control which accounts those are.
...
They are separate settings because a pod spec has a single `serviceAccountName`
field, so a list has no valid rendering into it. Leave the additional list
empty, which is the default, and the gateway accepts only the ServiceAccount it
assigns. That is the behavior of releases before this setting existed.
...
entries are neither validated as DNS-1123 names nor checked against existing
ServiceAccounts, so a typo surfaces only as a rejected bootstrap. Surrounding
whitespace is trimmed. Matching also ignores the namespace, which is validated
...
bootstrap path is disabled and these settings have no effect.
