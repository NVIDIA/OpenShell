...
runs in `network-only` mode: filesystem policy, process privilege dropping, and
process/binary identity checks are not applied by the process container. Network
policy still runs in the sidecar and remains endpoint/L7 scoped.
