...
reconstruct argv with shell parsing. Native runtime restart policies remain
disabled. The gateway either records the exit as terminal or replaces the
whole compute resource according to the sandbox's `never`, `on-failure`, or
`always` policy.
