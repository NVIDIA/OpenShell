...
| `disruption_protection.enabled` | `server.disruptionProtection.enabled` | Allow individual sandboxes to request a PodDisruptionBudget with a fail-closed expiration target. Disabled by default. Enabling it does not protect sandboxes that omit a duration request. |
