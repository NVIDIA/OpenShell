...
| `image_pull_policy` | `sandboxRuntime.image.pullPolicy` | Set the canonical isolated sandbox-runtime pull policy: `always`, `if_not_present`, or `never`. `newer` is Podman-only. Helm derives the runtime field. |
...
| `supervisor_image` | `supervisor.image.repository` / `supervisor.image.tag` | Set the isolated supervisor image. Helm derives the runtime field. |
| `supervisor_image_pull_policy` | `supervisor.image.pullPolicy` | Set the canonical supervisor pull policy: `always`, `if_not_present`, or `never`. `newer` is Podman-only. Helm derives the runtime field. |
| `sandbox_runtime` | `supervisor.sandboxRuntime` | Configure the isolated runtime's NetworkPolicy acknowledgement and boundary port. |
| `https_proxy`, `no_proxy`, `proxy_auth_secret_name`, `proxy_auth_secret_key`, `proxy_auth_allow_insecure`, `proxy_connect_by_hostname` | `upstreamProxy` | Set the chart-owned corporate proxy URL, bypass list, credential Secret reference, cleartext-auth acknowledgement, and hostname CONNECT option. Helm derives all runtime fields. |
