...
The gateway talks to the Docker daemon to create sandbox containers. The Docker driver does not currently implement gateway-owned builds from local directories or Dockerfiles; use a registry image reference.
...
The Podman driver supports gateway-owned builds for `openshell sandbox create --from <Dockerfile-or-directory>` when the operator sets `enable_image_builds = true` in `[openshell.drivers.podman]`. The option is disabled by default because authenticated sandbox users can cause the gateway's Podman service to execute arbitrary Dockerfile build steps. The CLI streams a tar-formatted build context to the gateway, and the driver builds through its configured Podman socket. The workflow sends the build context, not an exported image archive, and the resulting tag remains in the selected Podman image store.
