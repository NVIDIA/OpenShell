...
| `disruption_protection.enabled` | `server.disruptionProtection.enabled` | Allow individual sandboxes to request a PodDisruptionBudget with a fail-closed expiration target. Disabled by default. Disabling it rejects new requests; existing PDBs remain until their deadlines but are not repaired while mutation RBAC is disabled. |
...
namespace. The OpenShell Helm chart grants mutation permissions when
`server.disruptionProtection.enabled` is true and retains cleanup permissions
when it is false. Turning the gate off rejects new requests without shortening
previously accepted intervals. Existing PDBs expire normally, but OpenShell does
not recreate or patch them while mutation permissions are disabled.
