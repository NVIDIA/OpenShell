...
It supports MXC process containers and isolation sessions. For the MXC 0.8
host-proxy path, use `process_container`: the driver injects an authenticated
per-sandbox HTTP proxy address and denies direct Internet access at the MXC
layer. The current generated policy permits all host-loopback ports; proxy
authentication does not isolate other services bound to host loopback.
See [MXC gateway configuration](/reference/gateway-config#mxc) for workload,
relay, environment, and auditing options.
