...
Pass `userns_mode` in per-sandbox driver config to run a sandbox container inside a user namespace. Accepts the same values as `podman run --userns`, such as `"keep-id"`, `"keep-id:uid=998,gid=998"`, `"auto"`, `"nomap"`, or `"host"`. Omitting it leaves Podman's default behavior unchanged.
