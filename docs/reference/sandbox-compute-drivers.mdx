...
fixed `/sandbox` workspace if it is absent and transfers ownership of its
existing image contents to the completed UID/GID. The ownership walk skips
symlinks and nested bind or volume mounts. Sandbox startup fails before
readiness when the workspace root cannot be prepared, including when it is
read-only. Images may pre-own `/sandbox` as a build optimization, but this is
not required.
