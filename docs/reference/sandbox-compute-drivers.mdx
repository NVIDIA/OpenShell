...
| `disruption_protection.enabled` | `server.disruptionProtection.enabled` | Allow individual sandboxes to request a PodDisruptionBudget with a fail-closed expiration target. Disabled by default. Disabling it rejects new requests; existing PDBs remain until their deadlines but are not repaired. Gateway ownership metadata can still be adopted safely after a gateway ID change. |
| `disruption_protection.max_duration` | `server.disruptionProtection.maxDuration` | Set the maximum accepted per-sandbox protection duration. The default is `24h`; values support positive integer seconds, minutes, or hours. |
...
Operators can allow sandboxes to request fail-closed protection from voluntary
Kubernetes evictions with an expiration target:
...
The sandbox must also opt in through the portable sandbox API:
...
OpenShell rejects sandbox creation if the operator gate is disabled, the
duration is invalid, or the request exceeds `maxDuration`.

The request is part of `SandboxSpec`, not the immutable sandbox template or
driver JSON. Compute drivers advertise whether they support, approximate, or
reject the intent. Kubernetes supports it with a PDB; Docker, Podman, and the
VM driver currently reject it as unsupported.

OpenShell creates a `policy/v1` PodDisruptionBudget with `minAvailable: 1` and
persists an absolute UTC deadline on the Sandbox CR. The gateway reconciles the
PDB from that deadline after restarts and schedules deletion at expiration
while its Kubernetes watch is healthy. Gateway-scoped PDB management labels
prevent one OpenShell installation from reconciling another installation's
PDBs. If every gateway is unavailable when the deadline passes, protection
remains active until a gateway resumes reconciliation or an operator deletes
the PDB. Treat the requested duration as a fail-closed expiration target, not a
Kubernetes-enforced TTL. If the persisted deadline becomes malformed,
OpenShell retains the PDB until an operator repairs or removes the annotation.

This protects only against voluntary evictions that honor PDBs. It does not
prevent node failure, preemption, direct pod or Sandbox deletion, or platform
outages, and it is not a backup or restore mechanism. Unhealthy sandbox pods
remain evictable through `unhealthyPodEvictionPolicy: AlwaysAllow` so a broken
pod does not indefinitely block node maintenance.

Non-Helm deployments must grant the gateway service account `create`, `delete`,
`get`, `list`, and `patch` on `policy/v1` PodDisruptionBudgets in the sandbox
namespace. The OpenShell Helm chart grants `create` when
`server.disruptionProtection.enabled` is true and always retains cleanup plus
`patch` permission for metadata-only gateway ownership adoption. Turning the
gate off rejects new requests without shortening previously accepted intervals.
Existing PDBs expire normally, but OpenShell does not recreate or repair them
