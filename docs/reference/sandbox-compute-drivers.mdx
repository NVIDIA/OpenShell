...
| `runtime_class_name` | `server.runtimeClassName` | Set the default Kubernetes RuntimeClass for sandbox pods, such as `gvisor` for minikube/GKE Sandbox or a Kata RuntimeClass name. The gateway validates this class at startup when configured. Per-sandbox template runtime classes override this default and are validated during sandbox admission/create. |
| `runtime_class_outer_isolation` | `server.runtimeClassOuterIsolation` | Treat the configured RuntimeClass as the sandbox's outer kernel isolation boundary. Set explicitly for gVisor, GKE Sandbox, or Kata RuntimeClasses where supervisor-managed netns/seccomp controls are unavailable. |
