...
Guest egress leaves through gvproxy, so a proxy listening on the gateway host's loopback is reachable only through the host alias: configure `https_proxy = "http://host.openshell.internal:<port>"` rather than a `127.0.0.1` URL. A proxy on the corporate network needs no alias, and the guest's callback to the gateway never traverses the proxy.
