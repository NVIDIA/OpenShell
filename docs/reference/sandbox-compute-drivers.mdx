...
`enable_bind_mounts = true` in `gateway.toml`. Host bind mounts expose gateway
host paths to sandbox requests, so they are disabled by default. Image mounts
are not accepted in Docker sandbox driver config.
...
| Type | Fields |
|---|---|
| `bind` | `source`, `target`, optional `read_only` (`false` by default). `source` must be an absolute host path. Requires `enable_bind_mounts = true`. |
...
`enable_bind_mounts = true` in `gateway.toml`. Host bind mounts expose gateway
host paths to sandbox requests, so they are disabled by default.
