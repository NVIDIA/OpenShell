...
- `cpu.limit` contains a portable CPU quantity such as `500m` or `2`.
- `memory.limit` contains a portable memory quantity such as `512Mi` or `4Gi`.
...
| Kubernetes | Applies each typed value as both the container request and limit. |
