...
OIDC is the recommended authentication method for Kubernetes deployments. It integrates with your existing identity provider, supports role-based access control, and gives each user their own identity without requiring manual certificate distribution.

The Helm chart also generates mTLS certificates at install time, which the gateway uses for transport-layer security regardless of which authentication method you choose. The mTLS client bundle in the `openshell-client-tls` secret is used internally by sandbox supervisors to communicate back to the gateway, not for distributing access to individual users.
