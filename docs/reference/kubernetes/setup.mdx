...
| cert-manager | No | See [Use cert-manager for PKI](/reference/kubernetes/cert-manager) — only needed if you prefer cert-manager over the built-in PKI job |
| Kubernetes Gateway API | No | See [Enable Gateway API Access](/reference/kubernetes/gateway-api) — only needed for external ingress without port-forwarding |
...
<Warning>
The port-forward is for local evaluation only. For shared environments, expose the gateway through your ingress controller or access proxy. See [Enable Gateway API Access](/reference/kubernetes/gateway-api) for an external ingress option.
</Warning>
...
- To enable automatic certificate rotation, see [Use cert-manager for PKI](/reference/kubernetes/cert-manager).
- To expose the gateway externally without port-forwarding, see [Enable Gateway API Access](/reference/kubernetes/gateway-api).
- To configure OIDC or reverse-proxy authentication, see [Configure Gateway Authentication](/reference/kubernetes/authentication).
