...
An interceptor implements the [`openshell.gateway_interceptor.v1.GatewayInterceptor` gRPC service](https://github.com/NVIDIA/OpenShell/blob/main/proto/gateway_interceptor.proto#L14-L26):
...
Each [`InterceptorEvaluation`](https://github.com/NVIDIA/OpenShell/blob/main/proto/gateway_interceptor.proto#L39-L60) identifies the configured interceptor, manifest binding, public OpenShell service and method, authenticated principal, and active phase. The phase determines whether the payload contains a proposed operation, optional current state, or committed response.

The interceptor returns an [`InterceptorResult`](https://github.com/NVIDIA/OpenShell/blob/main/proto/gateway_interceptor.proto#L79-L91) with an allow or deny decision, an optional denial status and reason, JSON patches during `modify_operation`, and non-secret log annotations.

## Declare and Authorize Bindings

The interceptor declares bindings in its [`InterceptorManifest`](https://github.com/NVIDIA/OpenShell/blob/main/proto/gateway_interceptor.proto#L93-L103) returned by `Describe`. Each binding selects one public OpenShell RPC and one or more phases. The gateway validates these declarations at startup against its compiled protobuf descriptors and explicit interceptable RPC allowlist.
...
The following unary mutation RPCs are interceptable:

| Area | RPC selector |
| --- | --- |
| Sandbox | `openshell.v1.OpenShell/CreateSandbox` |
| Sandbox | `openshell.v1.OpenShell/AttachSandboxProvider` |
| Sandbox | `openshell.v1.OpenShell/DetachSandboxProvider` |
| Sandbox | `openshell.v1.OpenShell/DeleteSandbox` |
| Sandbox access | `openshell.v1.OpenShell/CreateSshSession` |
| Sandbox access | `openshell.v1.OpenShell/RevokeSshSession` |
| Service exposure | `openshell.v1.OpenShell/ExposeService` |
| Service exposure | `openshell.v1.OpenShell/DeleteService` |
| Provider | `openshell.v1.OpenShell/CreateProvider` |
| Provider | `openshell.v1.OpenShell/UpdateProvider` |
| Provider | `openshell.v1.OpenShell/DeleteProvider` |
| Provider profile | `openshell.v1.OpenShell/ImportProviderProfiles` |
| Provider profile | `openshell.v1.OpenShell/UpdateProviderProfiles` |
| Provider profile | `openshell.v1.OpenShell/DeleteProviderProfile` |
| Provider credential | `openshell.v1.OpenShell/ConfigureProviderRefresh` |
| Provider credential | `openshell.v1.OpenShell/RotateProviderCredential` |
| Provider credential | `openshell.v1.OpenShell/DeleteProviderRefresh` |
| Policy | `openshell.v1.OpenShell/UpdateConfig` |
| Policy analysis | `openshell.v1.OpenShell/SubmitPolicyAnalysis` |
| Policy draft | `openshell.v1.OpenShell/ApproveDraftChunk` |
| Policy draft | `openshell.v1.OpenShell/RejectDraftChunk` |
| Policy draft | `openshell.v1.OpenShell/ApproveAllDraftChunks` |
| Policy draft | `openshell.v1.OpenShell/EditDraftChunk` |
| Policy draft | `openshell.v1.OpenShell/UndoDraftChunk` |
| Policy draft | `openshell.v1.OpenShell/ClearDraftChunks` |

The gateway maintains the canonical [interceptable route allowlist](https://github.com/NVIDIA/OpenShell/blob/main/crates/openshell-gateway-interceptors/src/routes.rs#L17-L43). Read the interceptor manifest and gateway startup diagnostics when selecting bindings. Unknown, streaming, read-only, and non-allowlisted RPCs cannot be intercepted.
...
An interceptor can advertise `provider_profiles = true` in its manifest and implement [`SnapshotProviderProfiles`](https://github.com/NVIDIA/OpenShell/blob/main/proto/gateway_interceptor.proto#L18-L21). The RPC returns a [`ProviderProfileSnapshot`](https://github.com/NVIDIA/OpenShell/blob/main/proto/gateway_interceptor.proto#L105-L110). Add that interceptor to `provider_profile_sources` to include its profiles in the gateway's effective catalog.
