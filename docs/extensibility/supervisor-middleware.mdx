...
The supervisor must connect to every remote middleware service required by its
initial effective policy before it starts the sandbox workload. A connection
failure blocks sandbox startup even when a matching config uses `fail_open`.
The supervisor cannot acknowledge a bootstrap that omitted a required runtime.
After startup, a failed registry reload keeps the last-known-good registry and
reports the new configuration as rejected. Existing workload processes keep
running under that retained configuration.
