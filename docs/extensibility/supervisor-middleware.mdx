...
Operator-run services expose bindings for supported operations and phases. HTTP bindings are identified by operation and phase. The experimental agent-conversation binding also identifies a harness, hook, and schema version. Policies attach the complete middleware by its operator-owned gateway registration name.
