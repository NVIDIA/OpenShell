...
| `max_body_bytes` | Operator limit applied to HTTP bindings exposed by the service, up to the 4 MiB platform maximum. A WebSocket-only service sets this to `0`; each WebSocket binding declares `max_message_bytes` in its manifest. |
