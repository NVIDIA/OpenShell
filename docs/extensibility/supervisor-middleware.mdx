...
- Operator-run services use TLS `https://` when gateway JWT signing is enabled. Certificates must chain to the configured custom CA or platform roots, and the endpoint hostname must match.
