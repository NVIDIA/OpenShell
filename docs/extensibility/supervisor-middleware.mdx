...
Response middleware advertising `HTTP_RESPONSE/PRE_RETURN` inspects the matching final response before delivery to the sandbox. Its preflight includes the upstream `Content-Length`, `Content-Encoding`, and `Content-Range` as read-only metadata, except when a field is named by `Connection`. Middleware cannot write or remove these fields. OpenShell computes the downstream framing separately, so the final response can have a different length or use chunked transfer encoding after body processing.
