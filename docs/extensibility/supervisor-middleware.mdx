...
Writes and removals may target middleware-visible end-to-end request headers without a required prefix. Protected credential, routing, framing, and hop-by-hop headers are always rejected. Header values must not contain control characters.
