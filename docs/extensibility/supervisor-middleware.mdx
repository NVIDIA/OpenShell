...
Rust services can enable the `extension` Cargo feature on `openshell-sdk` for common extension interfaces, verification, and observability. Caller verification is available today through `openshell_sdk::extension::GatewayJwtAuthenticator`; additional SDK support will follow.
