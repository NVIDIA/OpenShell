...
4. Deploy the upgraded extension before upgrading the gateway. A gateway with mandatory negotiation rejects an older extension that omits metadata.
5. Run mixed-minor tests with required capabilities present and absent. Verify that a major mismatch and missing metadata fail before runtime traffic.
