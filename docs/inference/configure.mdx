...
Point `inference.local` at that provider and choose the default model for the gateway:
...
## Override One Sandbox's Backend

The gateway default applies to every sandbox that does not have its own override. To give one sandbox a different backend while keeping the same `https://inference.local` endpoint inside the sandbox, configure that sandbox by name:

```shell
openshell inference sandbox set \
    --sandbox <sandbox> \
    --provider my-cloud-provider \
    --model provider/model-name
```

Show the sandbox override:

```shell
openshell inference sandbox get --sandbox <sandbox>
```

Remove a sandbox override and return that sandbox to the gateway default:

```shell
openshell inference sandbox clear --sandbox <sandbox>
```
...
- Gateway default: Every sandbox using the active gateway sees the default `inference.local` backend unless it has a sandbox override.
- HTTPS only: `inference.local` is intercepted only for HTTPS traffic.
- Hot reload: Provider, model, timeout, and sandbox override changes are picked up by running sandboxes within about 5 seconds by default. No sandbox recreation is required.
