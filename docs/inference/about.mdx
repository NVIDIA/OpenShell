...
| Configuration | A gateway-level route defines the default backend. Operators can configure an individual sandbox to use a different provider and model without changing sandbox code. |
...
| Hot-refresh | OpenShell picks up provider credential changes, route changes, and sandbox override changes without recreating sandboxes. Changes propagate within about 5 seconds by default. |
