...
bound to a different host, port, or path. For example, a documentation example
referencing an unknown `KEY`, or a GitHub placeholder in an OpenAI conversation,
does not trigger a body denial. OpenShell does not substitute the secret.

A placeholder bound to the destination still fails closed, including inside quoted
tool output. Known invalid or revoked references and unavailable classification
