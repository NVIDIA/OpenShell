...
- an update gives one host and port two different MCP inspection contracts, including through separate rules or different paths.
...
The sandbox picks among the endpoints matching a request by most-specific path, so a broad REST endpoint and a narrower GraphQL endpoint on the same host and port stay unambiguous. MCP options are not selected that way, so MCP endpoints on one host and port have to agree on strict-tool-name, method-profile, and body-limit settings, even under different paths or in separate rules. An update that would create a disagreement is rejected. A policy that already contains one is left alone so unrelated updates still apply, but it should be repaired with full YAML replacement.
