...
openshell settings set --global \
  --key policy_validation_failure_mode \
  --value retain_last_valid
```

In `retain_last_valid` mode, the rejected candidate remains inactive and the previous valid generation remains active. If no previous valid generation exists, such as during initial startup, OpenShell still fails closed. The setting is gateway-global and cannot be overridden by an individual sandbox.
