...
You can preview the effects of a policy update before applying it:

```shell
openshell policy set <name> --policy current-policy.yaml --dry-run
```

`--dry-run` runs validation checks and shows which network rules would be added or removed, but does not apply the policy or notify the sandbox. The `--wait` flag is incompatible with `--dry-run`.
