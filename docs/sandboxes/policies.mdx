...
| `options` | No | Comma-separated endpoint options. Currently only `websocket-credential-rewrite` is supported, and only for `protocol: websocket` or REST compatibility endpoints that perform a WebSocket upgrade. |
