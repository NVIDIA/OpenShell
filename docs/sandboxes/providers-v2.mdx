...
- Built-in provider profiles stored under the repository `providers/` directory.
...
- Built-in refresh-backed provider profiles for `outlook` and `google-drive`.
...
`binaries` contains the executable paths allowed to reach the profile endpoints when the profile contributes policy to a sandbox. Entries can be strings or objects with `path` and `harness`.
