...
credentials:
  - name: user_oidc_token
    description: User OIDC token used as a token-exchange subject token
    env_vars: [CUSTOM_API_USER_OIDC_TOKEN]
    required: true
    auth_style: bearer
...
credential named by `token_grant.subject_token.credential`. Create or update that
provider credential from the current gateway OIDC login with `--from-oidc-token`.
