...
| `xaa-dev` | `other` | `XAA_DEV_ACCESS_TOKEN` |

The built-in `xaa-dev` profile demonstrates the sample 2-step XAA flow and includes the sample identity provider, resource authorization server, and resource API endpoints. For the full setup, see [XAA.dev Sample XAA](/get-started/tutorials/xaa-dev).
