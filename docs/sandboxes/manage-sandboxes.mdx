...
The CLI sends piped stdin in bounded frames, including for commands without a
TTY. Input is not limited by the gateway's per-message request size. The CLI
closes remote stdin when the pipe reaches EOF and continues reading command
output until the command finishes.
