...
retained sandbox in `Completed`; a nonzero exit leaves it in `Stopped` with a
...
`Stopped/MainProcessFailed` sandbox to launch a fresh instance of its canonical
command. Delete an inactive sandbox normally when you no longer need its
retained state.
...
| Stopped      | Compute is stopped and access is unavailable. This also represents a nonzero canonical-main result when the condition is `MainProcessFailed`.       |
...
| Error        | Provisioning, driver, or supervisor infrastructure failed. Check logs with `openshell logs`.                                                    |
...
`MainProcessFailed` and the `Stopped` phase. It also sets `status.exit_code`;
