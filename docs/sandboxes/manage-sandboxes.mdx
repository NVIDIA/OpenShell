...
The built-in default policy includes the filesystem access CUDA needs after GPU
devices are injected. If you use a custom policy with `--gpu`, keep the injected
GPU device nodes and `/proc` in `filesystem_policy.read_write`; CUDA
initialization may fail when `/proc` remains read-only.
