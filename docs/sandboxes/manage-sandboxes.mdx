...
Use `--from` to create a sandbox from the base image, another pre-built sandbox name, a local directory, a rootfs tar archive, or a container image:
...
Local directories and Dockerfiles require a local gateway because the CLI
builds images through the local Docker daemon. Rootfs tar archives
(`.tar`, `.tar.gz`, `.tgz`) also require a local gateway and are passed
directly to the VM compute driver. Use a registry image reference for remote
gateways.
