...
The sandbox table’s NOTES column shows `Config invalid` and the rejection reason when policy or provider configuration blocks provisioning. Open the sandbox detail view to see the Notes field. The diagnostic clears after the configuration is repaired; active port forwards remain listed.
