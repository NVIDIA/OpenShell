...
Set portable CPU and memory limits with `--cpu` and `--memory`:
...
OpenShell stores these values as typed portable resource requirements. Docker
and Podman apply them as runtime limits. Kubernetes uses each limit as its
request only when no request was set separately. The VM and MXC drivers reject
CPU or memory requirements because they do not currently support per-sandbox
sizing. For VM gateways, configure `vcpus` and `mem_mib` on the driver instead.

API and SDK callers can set `cpu.request` and `memory.request` independently
of their optional limits. Kubernetes honors those typed requests. It also
accepts requests in its driver configuration for workloads that need
Kubernetes-specific settings; a typed request takes precedence over the driver
configuration, and a driver-config request takes precedence over the
limit-to-request fallback. The `--cpu` and `--memory` flags continue to set
limits only.

<Warning>
CPU and memory values under `SandboxTemplate.resources.limits` or
`SandboxTemplate.resources.requests` are no longer supported. API clients must
use `SandboxSpec.resource_requirements.cpu.request` or `.limit` and
`SandboxSpec.resource_requirements.memory.request` or `.limit`. The gateway
rejects legacy requests with a migration error.
</Warning>
...
When the gateway creates a sandbox from a template, it resolves the template's
CPU, memory, and GPU settings into the sandbox's typed `resource_requirements`.
The reusable template field `SandboxWorkloadConfig.resources` uses the same
`ResourceRequirements` message as `SandboxSpec.resource_requirements`, so API
and SDK callers use `cpu.request`/`cpu.limit`, `memory.request`/`memory.limit`,
and `gpu` in both places.
...
JSON and YAML output include an `endpoint_statuses` list. The API and SDK expose it as `Sandbox.status.endpoint_statuses`. Each record contains `endpoint_id`, `host`, `ports`, `path`, `last_result`, and `last_reported_at`. Select the endpoint by its address to read the result directly. Treat `endpoint_id` as opaque.
