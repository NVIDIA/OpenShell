...
`--timeout <seconds>` to override the terminal wait timeout; otherwise the CLI
uses `OPENSHELL_LIFECYCLE_TIMEOUT`, which defaults to 300 seconds. A timeout
returns an error while deletion continues asynchronously.
