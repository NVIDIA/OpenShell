...
sandbox create requests. Template `driver_config` values are JSON objects keyed
by driver name. The gateway forwards only the block for its configured compute
driver:
