...
| Restarting   | The main process exited and its restart policy selected a restart. OpenShell is waiting for backoff or for the new supervisor session.                 |
...
the sandbox is `Restarting`. Connect and exec operations remain unavailable
