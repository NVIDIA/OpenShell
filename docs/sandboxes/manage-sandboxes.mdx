...
Sandbox service routing is enabled for gateways by default. Loopback gateways use `openshell.localhost` service URLs. Custom HTTPS service domains use the gateway server SAN configuration: add a wildcard DNS SAN such as `*.apps.example.com` to the gateway certificate and pass the same SAN to the gateway with `--server-san` or `OPENSHELL_SERVER_SAN`.
...
Use `openshell svc` as shorthand for `openshell service`.
