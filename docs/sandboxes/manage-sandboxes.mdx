...
Sandbox service routing is enabled for gateways by default. HTTPS service URLs need the service base domain in the gateway certificate SANs, so recreate older gateways before using remote or non-loopback service URLs:
