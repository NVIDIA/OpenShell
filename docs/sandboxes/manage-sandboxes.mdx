...
size. Piped input is limited to 4 MiB; use `sandbox upload` for larger files.
The CLI closes remote stdin when the pipe reaches EOF and continues
reading command output until the command finishes.
...
Run `exit` or press `Ctrl-D` to close this separate shell.
