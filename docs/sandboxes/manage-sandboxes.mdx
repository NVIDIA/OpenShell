...
`--no-wait` to return after the gateway accepts the asynchronous request. Set
`--timeout <seconds>` with a positive value to override the terminal wait
...
an error while deletion continues asynchronously.
