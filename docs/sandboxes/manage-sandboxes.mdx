...
`MainProcessFailed` condition. With no trailing command, OpenShell
...
Request bounded protection from voluntary platform disruption:

```shell
openshell sandbox create --disruption-protection 4h -- claude
```

The gateway rejects the request when the selected compute driver does not
support it, the administrator has not enabled it, or it exceeds the configured
maximum. Kubernetes implements the request with a PodDisruptionBudget. Docker,
Podman, and VM drivers currently report it as unsupported. This does not protect
against node failure, direct deletion, workload failure, or resource pressure.
...
Prefer stable flags such as `--cpu`, `--memory`, `--gpu`, and
`--disruption-protection` when they cover the same behavior.
