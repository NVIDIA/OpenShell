...
For local directories and Dockerfiles, the CLI streams the build context to the
gateway. A Podman gateway with operator-controlled `enable_image_builds = true`
builds it through the Podman socket selected by the driver, so explicit
`socket_path` configuration and auto-detection use the same image store that
starts the sandbox. Other compute drivers do not currently
support gateway-owned builds; use a registry image reference with those drivers.
