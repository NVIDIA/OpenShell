...
For a TTY main process, `Ctrl-C` interrupts a foreground job such as `tail -f`.
When the canonical main process itself owns the terminal foreground, `Ctrl-C`
disconnects the attachment without terminating the main process. Non-TTY
connections retain normal signal behavior.
