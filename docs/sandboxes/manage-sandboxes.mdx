...
Mount a host directory into a new sandbox when you need live file sharing:

```shell
openshell sandbox create --mount ./project:/sandbox/project -- claude
```

The mount source must be an existing host directory and the sandbox path must be absolute.
OpenShell resolves relative host paths before sending the create request.
