...
## Proxy Remote Service URLs

Run a local browser proxy for the active remote gateway:

```shell
openshell gateway proxy
```

The proxy binds an available local port and prints URLs like `http://<sandbox>--<service>.<gateway>.openshell.localhost:<port>/`. It forwards HTTP and WebSocket traffic to the selected remote gateway over the gateway's configured transport. Use this when remote wildcard DNS is not configured.
...
| `--local-domain` | Enable local browser URLs for exposed sandbox HTTP services, such as `https://my-sandbox--web.openshell.openshell.localhost:8080`. This is currently enabled by default for new local gateways during the local-domain routing spike. Recreate gateways created before this feature so the generated TLS certificate includes the local-domain SANs. |
| `--local-domain-suffix` | Override the local-domain suffix. Defaults to `openshell.localhost`. |
