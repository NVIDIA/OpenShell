...
| Runtime | Where sandboxes run | Best for |
|---|---|---|
...
The Helm gateway container uses a read-only image filesystem. SQLite-backed
StatefulSets write persistent state to the volume mounted at `/var/openshell`.
If a gateway reports a write failure, check that volume and its permissions.
...
- To configure workspace membership and roles, refer to [Workspaces](/sandboxes/manage-workspaces).
