...
**Why a placeholder credential?** `provider create` requires a non-empty `credentials` map. The bridge-fronted shape ignores AWS credentials at the router level (the bridge holds operator-side auth in its own pod), so any string value satisfies the structural requirement; `unused-bridge-fronted-shape` makes the intent obvious in `openshell provider get` output. When the SigV4 follow-up lands, this becomes a real key.
...
**About `--no-verify`.** The default validation probe does not yet recognize the `aws_bedrock_invoke` and `aws_bedrock_invoke_stream` protocols, so without `--no-verify` the `inference set` call would fail before it could mint a route. The first sandbox round-trip is the real verification today.
