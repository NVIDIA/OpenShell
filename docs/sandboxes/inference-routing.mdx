...
OpenShell matches the real path of each executable, not a symlink such as
`/usr/bin/python3`, so the profile uses globs that match versioned interpreters
such as `/usr/bin/python3.12`. To find an interpreter's real path, run
`readlink -f /usr/bin/python3` inside the sandbox, and adjust `binaries` to
match the executables in your image.
