...
Public requests do not accept canonical sandbox IDs as references. Fields that
previously accepted `sandbox_id` are removed and their protobuf field numbers
are reserved. Resolve stored IDs to sandbox names before calling the public
API. List RPCs that are not filtered to one sandbox continue to use only the
request-level `workspace_scope` selector.
