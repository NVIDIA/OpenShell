...
Single-workspace public requests use a non-empty string `workspace`, including
the literal `default`. Only the sandbox, sandbox template, provider, and service
collection list requests use the `WorkspaceSelector` oneof because those APIs
also accept `all_workspaces`. The all-workspaces selection requires Platform
Admin access. For services, choose either a sandbox name or `--all-workspaces`;
...
list RPCs use the request-level `workspace_scope` selector.
