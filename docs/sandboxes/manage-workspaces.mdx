...
### Delegated sandbox exceptions

A sandbox created with delegated identity acts on behalf of its delegating
user, so its lifecycle and interactive access are narrower than the ordinary
workspace role model.

| Operation | Delegating user | Workspace Admin | Platform Admin | Other Workspace User |
|---|---|---|---|---|
| Start | Yes | No | No | No |
| Stop | Yes | Yes | Yes | No |
| Delete | Yes | Yes | Yes | No |
| Connect, exec, file transfer, forwarding, services, providers, policy, or settings | Yes | No | No | No |

Admins can stop a delegated sandbox for immediate containment or delete it to
release resources, but they cannot reactivate a workload that acts for another
user. Only the original delegator can restart it, including after delegation
expiry, withdrawal, or credential revocation. These exceptions do not change
access to sandboxes created without delegated identity.

These restrictions govern direct sandbox access, not confidentiality against
administrators who control attached providers and profiles. Workspace Admins can
change workspace-scoped custom profiles to redirect token exchanges and receive
the delegator's subject access token. Delegate identity only when you trust those
administrators. Refer to [Profiles](/providers/profiles) for the token-exchange
trust requirements.
