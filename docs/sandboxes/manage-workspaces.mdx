...
Workspace-scoped public requests use the `workspace_scope` field with a
`WorkspaceSelector`. Single-workspace requests select a non-empty canonical
name, including the literal `default`. Only sandbox, sandbox template,
provider, and service collection list requests also accept `all_workspaces`.
The all-workspaces selection requires Platform Admin access. For services,
choose either a sandbox name or `--all-workspaces`; the options are mutually
exclusive.

Every sandbox-scoped RPC identifies the sandbox by its human-readable name and
keeps workspace selection in a separate field:

```json
{
  "workspaceScope": { "workspace": "team-ml" },
  "name": "agent"
}
```

Public requests do not accept canonical sandbox IDs as references. This is a
breaking API change: regenerate protobuf clients and replace stored ID
references with sandbox names before calling the public API. The four collection
list RPCs use the request-level `workspace` selector.
...
The TUI starts in the `default` workspace and sends that name explicitly. Its
all-workspaces collection views send the selector marker instead.
