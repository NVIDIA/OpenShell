...
and service list requests, and it requires Platform Admin access. For services,
choose either a sandbox name or `--all-workspaces`; the options are mutually
exclusive.
...
Sandbox-scoped RPCs use a `SandboxReference` instead of separate sandbox and
workspace fields. Use the name variant for human-readable lookup and include a
named workspace selector. Use the ID variant when the canonical sandbox ID is
already known; ID references must omit `workspace_scope`.

| Previous request fields | Sandbox reference |
| --- | --- |
| `name: "agent"`, `workspace_scope.workspace: "team-ml"` | `sandbox_ref.name: "agent"`, `sandbox_ref.workspace_scope.workspace: "team-ml"` |
| `sandbox_name: "agent"`, `workspace_scope.workspace: "team-ml"` | `sandbox_ref.name: "agent"`, `sandbox_ref.workspace_scope.workspace: "team-ml"` |
| `sandbox_id: "<canonical-id>"` | `sandbox_ref.id: "<canonical-id>"` |

Do not set both `sandbox_ref` and a request-level `workspace_scope`. List RPCs
that are not filtered to one sandbox continue to use the request-level
`workspace_scope` selector.

The Rust and Python SDKs expose separate all-workspaces list methods. The Go
SDK uses `ListAll`, and the TypeScript SDK uses a discriminated option type, so
a caller cannot select a named workspace and all workspaces in one typed call.
