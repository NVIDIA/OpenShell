...
| `aws-bedrock` | `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_SESSION_TOKEN`, `AWS_REGION` | Declarative schema for future direct Bedrock support. The built-in profile grants no endpoint access; use a custom endpoint-bearing profile for a bridge. |
