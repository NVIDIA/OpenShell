...
read-only. Update accepts one file at a time and rejects stale resource versions.
When `providers_v2_enabled=true`, updated profile policy applies to all provider
