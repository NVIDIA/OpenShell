...
Inference resolves through the gateway's catalog: OpenShell matches the command
against each imported profile's ID and the binaries it authorizes. A command no
profile claims, or one that several claim, infers nothing. Import a profile that
names the binary first, or pass an existing provider with `--provider <name>`.
