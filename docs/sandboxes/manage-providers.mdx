...
Provider profile metadata is available for known provider types. A sandbox uses
provider profile network policy only when you opt in at sandbox creation:

```shell
openshell sandbox create --provider my-claude --provider-profile-policy -- claude
```

Without `--provider-profile-policy`, provider behavior remains credential-only.
