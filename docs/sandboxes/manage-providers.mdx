...
Update a custom provider profile after editing its endpoints, binaries, or
credential metadata:

```shell
openshell provider profile update -f my-api-profile.yaml
```

Import remains create-only and fails if the profile ID already exists. Use
`provider profile update` for existing custom profiles. Built-in profiles are
read-only. When `providers_v2_enabled=true`, updated profile policy applies to
all provider instances of that type on the next sandbox config sync.
