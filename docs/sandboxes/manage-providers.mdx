...
via sandbox network policy. Use `aws-s3` for S3-specific endpoint rules. Use
`aws-bedrock` for native regional Bedrock Runtime endpoints:
...
The Bedrock profile allows regional `bedrock-runtime` endpoints and applies
proxy-side SigV4 body signing. Set `AWS_REGION` or `AWS_DEFAULT_REGION` as an
application environment variable so the AWS SDK selects the intended native
endpoint; the endpoint hostname also determines the signing region.
...
| `aws-bedrock` | `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_SESSION_TOKEN` | Native regional Bedrock Runtime APIs with proxy-side SigV4 body signing. Set the SDK region through the sandbox environment. |
...
| `openai` | `OPENAI_API_KEY` | OpenAI API at `api.openai.com`. A base URL override does not authorize or bind credentials to an alternate host; use a custom endpoint-bearing profile for compatible providers. |
...
Inference providers use the same attachment and native endpoint flow as every
other provider. Built-in profiles cover OpenAI, Anthropic, NVIDIA API Catalog,
DeepInfra, and Google Vertex AI. Attach the provider to a sandbox; the profile
contributes its endpoint policy, binary allowlist, and credential bindings.

OpenAI-compatible services on other hosts are not automatically covered by the
`openai` profile. Import a custom profile that declares the alternate endpoint
and required client binaries, then create and attach a provider of that type.
See [Inference Providers](/sandboxes/inference-routing) for an example.
