...
Set the value before creating a sandbox when you want the first denied request to include policy advisor guidance. The gateway pushes setting changes to running sandboxes, so they can enable the surface after startup, but startup enablement gives the agent the clearest first-denial path.
