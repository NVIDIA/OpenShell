...
- **Check admission policy requirements.** In managed and operator workspace modes, the chart installs a `ValidatingAdmissionPolicy` that limits gateway workspace requests to namespaces it owns or selects. This requires Kubernetes 1.30 or later. The chart fails to render with `operatorNamespaceFile` or a set-based `operatorNamespaceLabel` while the policy is enabled; set `admissionPolicy.enabled=false` to keep those configurations ([PR #3630](https://github.com/NVIDIA/OpenShell/pull/3630)).
