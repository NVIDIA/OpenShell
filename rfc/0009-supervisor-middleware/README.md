...
- **Header mutation.** Headers preserve duplicates and wire order. External writes and removals may target visible end-to-end headers without a required prefix. Writes support append, overwrite, or skip. Credential-bearing, routing, framing, hop-by-hop, and `Connection`-nominated headers remain protected. Each stage's mutations are atomic.
