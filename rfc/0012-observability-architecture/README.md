...
| CLI | None (not needed, see below) | Passive `TRACEPARENT` forwarding proposed | Lightweight |
...
A log push mechanism already exists. The `LogPushLayer` in `crates/openshell-supervisor-process/src/log_push.rs` captures tracing events from the supervisor and streams them to the gateway via the `PushSandboxLogs` client-streaming RPC. Each `SandboxLogLine` carries the sandbox ID, timestamp, level, target, message, and structured key-value fields. The push is best-effort (`try_send`; if the channel is full, the event is dropped). The gateway stores these logs and exposes them through `GetSandboxLogs`.
...
The CLI needs only passive `traceparent` propagation, not its own OTel provider. If `TRACEPARENT` is set in the environment (e.g., by a CI pipeline), the CLI forwards it to the gateway via `TraceContextInterceptor` on the gRPC channel. This is lightweight (no `SdkTracerProvider`, no collector connectivity required) and gives CI pipelines end-to-end trace continuity. The gateway remains the instrumentation boundary for platform tracing.
