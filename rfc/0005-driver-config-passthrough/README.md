...
This RFC records the implemented baseline after PRs #1744, #1785, #1815, and
PR #1861. It also keeps the remaining follow-up questions visible: driver
identity aliases, schema discovery, no-match warnings, and the future of
legacy Kubernetes platform resource passthrough.
...
| Docker | `cdi_devices` for exact GPU selection and `mounts` for `bind`, `volume`, and `tmpfs` mounts. |
| Podman | `cdi_devices` for exact GPU selection and `mounts` for `bind`, `volume`, `tmpfs`, and `image` mounts. |
...
Documented driver config keys are driver-owned public API. Users, templates,
and automation can depend on them. Drivers should evolve those keys
...
explicit versioned shape over changing an existing key in place.
