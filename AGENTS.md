...
| `crates/openshell-crypto/` | Crypto backend | Backend selection for the process-default provider (`ring` by default, AWS-LC FIPS under the `fips` feature); TLS provider, PKI/JWT key generation, AEAD, SSH algorithm sets. `sqlx` and the AWS SDK select their own and are handled in `openshell-server` |
