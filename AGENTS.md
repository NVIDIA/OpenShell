...
This environment is pre-provisioned via a VM snapshot (the startup update script runs `mise install protoc uv python rust` + `uv sync --group dev`). System deps already installed in the snapshot: `mise` (at `/usr/local/bin/mise`), Docker CE, `libz3-dev`, `iproute2`. Toolchain commands are standard (see [CONTRIBUTING.md](CONTRIBUTING.md) and `tasks/*.toml`); the notes below are the non-obvious caveats.

- **Always prefix mise with `MISE_AUTO_INSTALL=0 MISE_VERSION_CHECK=0`.** Egress is restricted: `nodejs.org`, `ziglang.org`, `get.helm.sh`, `dl.k8s.io`, and npm are blocked, so `node`, `zig`, `helm`, `kubectl`, and `markdownlint-cli2` are *not* installed. Without `MISE_AUTO_INSTALL=0`, `mise run`/`mise exec` abort trying to (re)install those tools. `MISE_VERSION_CHECK=0` silences the blocked self-update check. Consequently `mise run lint`/`mise run ci` fail on the `markdown:lint`/`helm:*` sub-tasks; run the working pieces directly (e.g. `mise run rust:format:check`, `rust:lint`, `python:lint`, `python:format:check`, `python:typecheck`, `license:check`, `test:rust`, `test:python`).
- **`cc`/`c++` are pointed at gcc** (via `update-alternatives`). The default Ubuntu `clang` cannot find libstdc++ headers, which breaks the from-source `protobuf-src` build. Keep gcc as the `cc`/`c++` provider.
- **Cargo/sccache:** `mise.toml` sets `RUSTC_WRAPPER=sccache`; sccache is installed and works. If you bypass mise (run `cargo` directly), either keep sccache on `PATH` or `unset RUSTC_WRAPPER`, and add the mise `protoc` bin dir to `PATH` (`export PATH="$(MISE_AUTO_INSTALL=0 mise which protoc | xargs dirname):$PATH"`).
- **Docker must be started manually each session** (no systemd): `sudo dockerd >/tmp/dockerd.log 2>&1 &`. The daemon is preconfigured for `fuse-overlayfs` + `iptables-legacy` (`/etc/docker/daemon.json`); the `ubuntu` user is in the `docker` group so fresh shells reach the socket without `sudo`.
- **Container registry egress is blocked** (`ghcr.io`, Docker Hub blob CDN, `quay.io`). The default sandbox image `ghcr.io/nvidia/openshell-community/sandboxes/base:latest` cannot be pulled, so sandbox-create/e2e tests that pull images will fail on network. A locally-built glibc image **`openshell-localsandbox:latest`** (host rootfs + `iproute2` + a `sandbox` UID-1000660000 user + `/run`) exists for offline sandboxes. Run the gateway with it via:
  `OPENSHELL_SANDBOX_IMAGE=openshell-localsandbox:latest OPENSHELL_SANDBOX_IMAGE_PULL_POLICY=Never mise run gateway:docker`
  then drive it with `./target/debug/openshell --gateway docker-dev <cmd>`. `sandbox create -- <cmd>` runs a command in the sandbox and prints its output; the interactive `sandbox exec` SSH relay can hang on this minimal image, and a benign `/home/sandbox/.profile: Permission denied` line is expected. If the image is missing, rebuild it by importing the host rootfs (`tar -C / ... | docker import - openshell-localbase:raw`) and adding `/run`, `iproute2`, and the `sandbox` user.
- To run the real community sandbox image or any registry-backed e2e (`mise run e2e`), request egress for `ghcr.io` and the Docker Hub registry/CDN in the Cloud Agent network settings.
