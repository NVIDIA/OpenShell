...
| Sandbox pod rejected by PodSecurity in a namespace enforcing `restricted` | Sandbox pods require `SYS_ADMIN`, `NET_ADMIN`, `SYS_PTRACE`, `SYSLOG`, and a UID 0 supervisor, which exceed Baseline | Point `server.sandboxNamespace` at a namespace that does not enforce Restricted |
