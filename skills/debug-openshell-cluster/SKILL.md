...
| HTTP response returns `response_delivery_failed` or ends after partial delivery | A fail-closed `HTTP_RESPONSE/PRE_RETURN` stage failed before or after response commitment | Sandbox HTTP activity and detection findings; middleware failure category; response framing; service timeout, capacity, and transport logs. Retrying may repeat upstream side effects |
