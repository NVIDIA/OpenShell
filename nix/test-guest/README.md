...
conformance binary are copied, `openshell-snap` detects the prebuilt gateway
binary and uses it to generate the PKI and sandbox JWT bundle after removing
any previous Snap and before installing the candidate. It then connects the
required interfaces, registers the local gateway, and waits for `openshell
status`. The guest command runs smoke conformance.
