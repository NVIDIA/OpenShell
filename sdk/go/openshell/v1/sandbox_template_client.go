...
func (s *sandboxTemplateClient) Create(ctx context.Context, workspace string, template *SandboxTemplate) (*SandboxTemplate, error) {
	if template == nil {
		return nil, &StatusError{Code: ErrorInvalidArgument, Message: "template must not be nil"}
	}
	protoTemplate, err := converter.SandboxTemplateToProto(template)
	if err != nil {
	...
	return converter.SandboxTemplateFromProto(resp.GetTemplate()), nil
}

func (s *sandboxTemplateClient) Get(ctx context.Context, workspace, name string) (*SandboxTemplate, error) {
...
func (s *sandboxTemplateClient) List(ctx context.Context, workspace string, opts ...ListOptions) ([]*SandboxTemplate, error) {
	...
	templates := make([]*SandboxTemplate, 0, len(resp.GetTemplates()))
	for _, proto := range resp.GetTemplates() {
		templates = append(templates, converter.SandboxTemplateFromProto(proto))
	}
	return templates, nil
}

func (s *sandboxTemplateClient) Delete(ctx context.Context, workspace, name string) error {
	_, err := s.client.DeleteSandboxTemplate(ctx, &pb.DeleteSandboxTemplateRequest{
		Name:      name,
		Workspace: workspace,
	})
	if err != nil {
