...
func resourceRequirementsFromProto(rr *pb.ResourceRequirements) *types.ResourceRequirements {
	...
	if cpu := rr.GetCpu(); cpu != nil {
		result.CPU = &types.CPUResourceRequirements{Limit: cpu.GetLimit(), Request: cpu.GetRequest()}
	}
	if mem := rr.GetMemory(); mem != nil {
		result.Memory = &types.MemoryResourceRequirements{Limit: mem.GetLimit(), Request: mem.GetRequest()}
	}
	return result
}
...
func resourceRequirementsToProto(rr *types.ResourceRequirements) *pb.ResourceRequirements {
	...
	if rr.CPU != nil {
		result.Cpu = &pb.CpuResourceRequirements{
			Limit:   StringPtr(rr.CPU.Limit),
			Request: StringPtr(rr.CPU.Request),
		}
	}
	if rr.Memory != nil {
		result.Memory = &pb.MemoryResourceRequirements{
			Limit:   StringPtr(rr.Memory.Limit),
			Request: StringPtr(rr.Memory.Request),
		}
	}
	return result
}
