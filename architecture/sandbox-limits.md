...
| Token-exchange token cache TTL | 5 min maximum per entry from insertion | The cap applies to positive profile overrides. Response-derived TTLs apply the 30 s expiry margin after the cap, for a 4 min 30 s maximum. Already-authorized exchanges may finish and populate caches after delegation withdrawal or credential revocation, so this is not a withdrawal-to-containment deadline. |
