...
Local-domain sandbox service routing is always configured for gateway deployments. Bootstrap injects the gateway name as the local-domain cluster label, and fast deploy passes the same label through Helm so incremental deploys do not reset it.
