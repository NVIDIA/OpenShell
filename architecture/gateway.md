...
- Authorize Kubernetes proxy registration against driver-verified physical
  ...
  waits while a provisioning or starting runtime binding is being persisted,
  then requires an exact identity match. It supplies initial credentials; it
  does not bypass generation checks or recover a credential lineage that has
  already advanced through refresh.
...
to the selected compute driver's `AuthenticateSandbox` RPC. The gateway requires
a matching durable sandbox record and the exact driver/runtime assignment
recorded at provisioning before returning operational credentials. Session
authentication checks the durable runtime generation and token lineage for every
sandbox RPC, so a replaced runtime and legacy unbound tokens cannot retain
provider or control-plane access. The Kubernetes driver uses its own named
configuration to run TokenReview and verify the live proxy Pod, workload Pod,
and controlling Sandbox CR. The runtime assignment binds their immutable
Kubernetes identities. The bootstrap path accepts
