...
boundaries, not public gateway RPCs. The descriptor-derived test is the
authority for current counts and fingerprints.
...
Mutation request IDs extend public request fields without changing the durable
protobuf schema. The descriptor-derived test records the resulting public and
durable closures.
...
builds the latest full snapshot for each affected active sandbox. Repeated
mutations coalesce, concurrency is bounded, and periodic reconciliation repairs
build, route, or session failures. An async router owns session lookup, message
