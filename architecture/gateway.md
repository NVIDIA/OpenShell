...
| Dual-use | `upstreamProxy.{url,noProxy,authSecret.name,authSecret.key,authAllowInsecure,connectByHostname}`, `supervisor.image.pullPolicy` | Chart-owned `upstreamProxy.*` and `supervisor.image.*` inputs derive their Kubernetes-driver runtime values. |
