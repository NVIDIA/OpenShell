...
- Rust-heavy Dockerfiles use BuildKit cache mounts for cargo registry directories, keyed by image name and `TARGETARCH`, with `sharing=locked` to prevent concurrent cache corruption in parallel CI builds. The `target/` directory is not cached via BuildKit mounts — sccache handles compilation caching externally (via memcached in CI, local disk for dev).
