...
- Rust-heavy Dockerfiles use BuildKit cache mounts for cargo registry and sccache local disk directories, keyed by image name and `TARGETARCH`, with `sharing=locked` to prevent concurrent cache corruption in parallel CI builds. sccache uses memcached in CI (`SCCACHE_MEMCACHED_ENDPOINT`) and falls back to the local disk cache mount for local dev builds. The `target/` directory is not cached via BuildKit mounts to avoid stale artifact issues across dependency changes.
