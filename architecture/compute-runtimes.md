...
single GPU. CPU and memory requirements carry independent optional request and limit
quantities (e.g. `"500m"`, `"4Gi"`). Docker and Podman apply limits and reject independent
requests; Kubernetes applies both, falling back from a limit to a request only when no
typed or driver-config request is set. The VM and MXC drivers reject typed CPU/memory
requirements with a clear error until sizing support lands there, rather than silently
ignoring them.
