...
The ownership rewrite uses GNU tar extensions, and the init container runs the
sandbox image itself. Images whose tar lacks them — a minimal base such as
Alpine, which provides BusyBox tar — seed with the previous flags instead, so
they keep the behavior they have today rather than failing to start.
