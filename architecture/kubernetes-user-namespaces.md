...
**No changes to:** seccomp filters (CLONE_NEWUSER block stays), Landlock, supervisor privilege-drop logic, init containers, and volume ownership semantics (ID-mapped mounts handle ownership transparently).
...
### 7. Standalone driver binary: wire CLI arg
