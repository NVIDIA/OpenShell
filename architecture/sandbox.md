...
socket inode. If fork or descriptor passing creates multiple owners, they must
have compatible policy identities; the proxy denies ambiguous shared ownership
before middleware evaluation or upstream connection.
...
duration; the gateway derives the absolute authorization window from its own
clock. Delegated access and refresh tokens live behind opaque
...
workspace role model. These ownership restrictions govern direct sandbox access,
not confidentiality against configuration administrators. Exchanges load the
current provider profile; delegation does not pin its exchange configuration.
Workspace admins can change workspace-scoped custom profiles, including the
token endpoint, and redirect a subsequent exchange. The gateway sends the raw
delegated access token as the subject token, not the delegated refresh token or
normal CLI login credentials. Both provider-credential and delegated-identity
subject-token paths therefore trust administrators who control exchange
configuration with their subject tokens.
The gateway rejects requests whose authorization checks
observe delegation expiry, withdrawal, missing credential state, or credential
revocation. Withdrawal is not instantaneous containment. Already-authorized
refreshes or exchanges may finish and populate token caches. Provider profile
lowering trims surrounding whitespace from subject-token source and credential
identifiers before the gateway resolves them with exact matching.
...
an override, is capped at five minutes from insertion. This limits reuse of each
entry, not elapsed time from delegation withdrawal or credential revocation;
in-flight exchanges can populate caches afterward. Response-derived entries
apply the 30-second margin after that cap and therefore live for at most four
minutes and 30 seconds. This bound does not invalidate an access token that an upstream
