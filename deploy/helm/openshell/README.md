...
| supervisor.sidecarProxyUid | int | `1337` | UID for the long-running network sidecar/split supervisor. In sidecar topology, the network init container installs nftables rules that exempt this UID. |
...
| supervisor.topology | string | `"combined"` | Supervisor pod topology for Kubernetes sandboxes. "combined" runs the current single supervisor container in the agent pod. "sidecar" runs network enforcement in a dedicated sidecar and the process supervisor as a low-capability wrapper in the agent container. "split-pod" runs network enforcement in a separate supervisor pod and restricts the agent pod to that supervisor through NetworkPolicy. |
