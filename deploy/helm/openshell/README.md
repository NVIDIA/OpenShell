...
| grpcRoute.backendTLSPolicy.caCertificateConfigMapName | string | `""` | Name of the ConfigMap containing the CA certificate (key: ca.crt) used to validate the gateway pod's TLS certificate. Defaults to <fullname>-backend-ca when empty. The certgen hook auto-creates this: with pkiInitJob (default), immediately on install/upgrade; with cert-manager, the hook waits for cert-manager to issue the server certificate first, so on first install the ConfigMap is created during the subsequent `helm upgrade` (run upgrade after cert-manager reconciles). |
