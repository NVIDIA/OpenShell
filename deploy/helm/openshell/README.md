...
| server.dbMaxConnections | int | `0` | Connection ceiling for the gateway's database pool, shared by every database-backed request. 0 (default) uses the gateway's backend default: 10 for PostgreSQL, 5 for on-disk SQLite. On PostgreSQL each replica opens its own pool, so keep dbMaxConnections * replicaCount below the server's own max_connections. |
