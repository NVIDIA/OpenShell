...
| server.drivers.kubernetes.operatorNamespaceFile | operator mode | `""` | Path to a drop-in JSON file mapping workspace names to namespace names. Hot-reloaded on change. |
| server.drivers.kubernetes.operatorNamespaceLabel | operator mode | `""` | K8s label selector for namespace discovery. The driver watches namespaces matching this label. |
