...
wrappers. A repository-only value inherits `IMAGE_TAG`; a value with an
explicit tag or `@sha256:` digest is used as-is. When these variables are
unset, the existing `OPENSHELL_REGISTRY` plus `IMAGE_TAG` behavior is retained.
