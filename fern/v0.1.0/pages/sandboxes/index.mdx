...
For the complete structure and every field, refer to the [Policy Schema Reference](/reference/policy-schema).
...
- The proxy queries the [policy engine](/about/architecture) with the destination (host and port) and the calling binary. A connection is allowed only when both match an entry in the same policy block.
...
- To create your first sandbox, refer to [Create and Manage](/sandboxes/create-and-manage).
