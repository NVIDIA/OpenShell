---
title: "Set Up a Sandbox of Claude Code with a Custom GitHub Policy"
...
Create a [credential provider](/sandboxes/providers) that injects your GitHub token into the sandbox automatically. The provider reads `GITHUB_TOKEN` from your host environment and sets it as an environment variable inside the sandbox:
...
For details on policy block structure, refer to [Network Access Rules](/sandboxes/index#network-access-rules).
...
- To inject credentials automatically instead of pasting tokens, refer to [Providers](/sandboxes/providers).
