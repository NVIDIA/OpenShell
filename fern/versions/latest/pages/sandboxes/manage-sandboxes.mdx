...
This page covers creating sandboxes and managing them. For background on what sandboxes are and how isolation works, refer to [About Sandboxes](/latest/sandboxes).
...
If you plan to run sandboxes on a remote host or a cloud-hosted gateway, set up the gateway first. Refer to [Manage Gateways](/latest/sandboxes/manage-gateways) for deployment options and multi-gateway management.
...
Use the terminal to spot blocked connections marked `action=deny` and inference-related proxy activity. If a connection is blocked unexpectedly, add the host to your network policy. Refer to [Policies](/latest/sandboxes/policies) for the workflow.
...
- To follow a complete end-to-end example, refer to the [Github Sandbox](/latest/tutorials/github-sandbox) tutorial.
- To supply API keys or tokens, refer to [Manage Providers](/latest/sandboxes/manage-providers).
- To control what the agent can access, refer to [Policies](/latest/sandboxes/policies).
- To use a pre-built environment, refer to the [Community Sandboxes](/latest/sandboxes/community-sandboxes) catalog.
