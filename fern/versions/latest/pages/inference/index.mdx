...
| Path | How It Works |
|---|---|
| External endpoints | Traffic to hosts like `api.openai.com` or `api.anthropic.com` is treated like any other outbound request, allowed or denied by `network_policies`. Refer to [Policies](/latest/sandboxes/policies). |
| `inference.local` | A special endpoint exposed inside every sandbox for routing inference traffic locally, preserving privacy and security. The [privacy router](/latest/about/architecture) strips the original credentials, injects the configured backend credentials, and forwards to the managed model endpoint. |
...
- To set up the backend behind `inference.local`, refer to [Configure](/latest/inference/configure).
