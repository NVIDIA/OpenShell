...
For the full policy YAML schema, refer to the [Policy Schema](/latest/reference/policy-schema).
For the architecture of each enforcement layer, refer to [Architecture](/latest/about/architecture).
...
- [Policies](/latest/sandboxes/policies) for applying and iterating on sandbox policies.
- [Policy Schema](/latest/reference/policy-schema) for the full field-by-field YAML reference.
- [Default Policy](/latest/reference/default-policy) for the built-in default policy breakdown.
- [Gateway Auth](/latest/reference/gateway-auth) for gateway authentication details.
- [Architecture](/latest/about/architecture) for the system architecture.
