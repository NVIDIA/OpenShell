...
- A working OpenShell installation. Complete the [Quickstart](/latest/get-started/quickstart) before proceeding.
...
In terminal 2, create a new sandbox with Claude Code. The [default policy](/latest/reference/default-policy) is applied automatically, which allows read-only access to GitHub.

Create a [credential provider](/latest/sandboxes/manage-providers) that injects your GitHub token into the sandbox automatically. The provider reads `GITHUB_TOKEN` from your host environment and sets it as an environment variable inside the sandbox:
...
The following YAML shows a complete policy that extends the [default policy](/latest/reference/default-policy) with GitHub access for a single repository. Replace `<org>` with your GitHub organization or username and `<repo>` with your repository name.
...
The remaining blocks (`claude_code`, `nvidia_inference`, `pypi`, `vscode`) are identical to the [default policy](/latest/reference/default-policy). The default policy's `github_ssh_over_https` and `github_rest_api` blocks are replaced by the `github_git` and `github_api` blocks above, which grant write access to the specified repository. Sandbox behavior outside of GitHub operations is unchanged.

For details on policy block structure, refer to [Policies](/latest/sandboxes/policies).
</Accordion>
...
- To add per-repository access levels (read-write vs read-only) or restrict to specific API methods, refer to the [Policy Schema Reference](/latest/reference/policy-schema).
- To learn the full policy iteration workflow (pull, edit, push, verify), refer to [Policies](/latest/sandboxes/policies).
- To inject credentials automatically instead of pasting tokens, refer to [Manage Providers](/latest/sandboxes/manage-providers).
