...
class Openshell < Formula
  ...
  def post_install
    ...
    unless gateway_config.exist?
      ...
      # Migrate only the exact config generated by the affected formula. Keep
      # any user-edited config untouched.
      if gateway_config.read == legacy_ipv6_gateway_config_contents
        gateway_config.write gateway_config_contents
      end
    end
