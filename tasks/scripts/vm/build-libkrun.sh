...
ensure_cargo_for_libkrun() {
  ...
  echo "       Typical fix: run vm:setup via mise from the repo so Rust 1.88.0 is on PATH," >&2
  echo "       or:  rustup toolchain install 1.88.0 && rustup default 1.88.0 && export PATH=\"\$HOME/.cargo/bin:\$PATH\"" >&2
