...
TARGET=${1:?"Usage: container-build-image.sh <gateway|supervisor|supervisor-builder|supervisor-output> [extra-args...]"}
shift

CONTAINERFILE=$(ce_resolve_containerfile deploy/container images)
...
if [[ "${USE_PREBUILT_BINARIES:-}" == "true" ]]; then
  case "${TARGET}" in
    gateway|supervisor|supervisor-output)
      if [[ ! -d deploy/container/.build/prebuilt-binaries ]]; then
        echo "Error: USE_PREBUILT_BINARIES=true but deploy/container/.build/prebuilt-binaries/ does not exist" >&2
