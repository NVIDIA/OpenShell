...
if ! cargo tree -p "${PACKAGE}" --features fips -e normal -i aws-lc-fips-sys >/dev/null 2>&1; then
...
echo "  (expected: the AWS SDK's rustls 0.21 leg, and OpenShell crates via"
echo "   additive sibling default features — see docs/security/fips.mdx)"
echo
if cargo tree -p "${PACKAGE}" --features fips -e normal -i ring 2>/dev/null | sed 's/^/  /'; then
  :
else
...
cargo tree -p "${PACKAGE}" --features fips -e normal 2>/dev/null \
